 ...
 * @author          lalala
 */
class Bargain_base_model extends MY_Model
{
  ...
  public function count_by($where = '')
  {
     ...
     * @return int
     */
    public function signing_count_by($where)
    {
        //查询字段
        $this->dbback_city->select(array('count(distinct(broker_name_a)) as num'));
        if ($where) {
            //查询条件
            $this->dbback_city->where($where);
        }
        $arr_num = $this->dbback_city->get($this->_tbl)->row_array();
        return $arr_num['num'];
    }
  /**
   * 获取合同列表页
                             ...
                             $order_key = 'id', $order_by = 'DESC')
  {
     ...
     * @return array 返回多条记录组成的二维数组
     */
    public function get_signing_data_by($where, $start = -1, $limit = 20,
                                        $order_key = 'id', $order_by = 'DESC')
    {

        $this->dbback_city->select(array('agency_id_a', 'agency_name_a', 'company_name_a', 'count(*) as signing_num'));
        if ($where) {
            //查询条件
            $where .= ' group by agency_id_a';
            $this->dbback_city->where($where);
        }
        //排序条件
        //$this->dbback_city->order_by($order_key, $order_by);
   ...
   * @return array 合同记录组成的一维数组
   */
  public function get_by_bargain_number($bargain_no)
  {
    //查询字段
    if ($this->_select_fields) {
      $this->dbback_city->select($this->_select_fields);
    ...
    //查询条件
    $this->dbback_city->where('number', $bargain_no);
    return $this->dbback_city->get($this->_tbl)->row_array();
  }
...
/* End of file bargain_base_model.php */
/* Location: ./applications/models/bargain_base_model.php */
