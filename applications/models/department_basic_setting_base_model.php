...
/**
 * Agency_basic_setting_base CLASS
 *
 * 基本设置管理相关功能
 *
 * @package         MLS
 ...
 * @author          xz
 */
class Department_basic_setting_base_model extends MY_Model
{
  /**
   * 基础设置表名称
   * @var string
   */
  private $_bs_tbl = 'department_basic_setting';
   ...
   * @return array
   */
  public function get_data_by_department_id($id)
  {
  ...
  {
    $this->dbback_city->select('*');
    $this->dbback_city->where(array('company_id' => $id, 'department_id' => 0));
    return $this->dbback_city->get($this->_bs_tbl)->result_array();
  }
  ...
  {
    $this->dbback_city->select('department_id , sell_house_nature_public , rent_house_nature_public , buy_customer_nature_public , rent_customer_nature_public , house_public_time , customer_public_time');
   ...
   */
  public function judge_by_id($paramArray, $department_id)
  {
    $sql = "select id from department_basic_setting where department_id = " . $department_id;
    $result = $this->db_city->query($sql)->result_array();
    if (!empty($result)) {
      return $this->update_by_id($paramArray, $department_id);
    } else {
      $paramArray['department_id'] = $department_id;
