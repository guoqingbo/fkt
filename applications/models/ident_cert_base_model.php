 ...
 * @author          sun
 */
class Ident_cert_base_model extends MY_Model
{
   ...
   * @return array 返回多条证书信息记录组成的二维数组
   */
   ...
   * @return array 身份认证信息记录组成的一维数组
   */
  public function get_by_broker_id($broker_id, $start = 0, $limit = 1, $order_key = 'id', $order_by = 'DESC')
  {
    //查询字段
    if ($this->_select_fields) {
      $this->dbback_city->select($this->_select_fields);
  ...
  /**
   * 根据查询条件返回一条身份认证信息表的记录
   * @param string $where 查询条件
   * @return array 返回一条一维数组的证书信息表记录
   */
   ...
   * @return int 成功 返回插入成功后的证书信息id 失败 false
   */
...
/* End of file Ident_cert_base_model.php */
/* Location: ./applications/models/ident_cert_model.php */
