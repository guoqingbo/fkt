...
/**
 * purview_company_role_base_model CLASS
 *
 * 公司权限角色添加、删除、修改管理功能
 *
 * @package         MLS
 ...
 * @author          sun
 */
class Purview_company_group_base_model extends MY_Model
{
   ...
   * @var string
   */
  private $_tbl = 'purview_company_group';
  ...
  public function get_all_by($where)
  {
    ...
    $sql = "select b.truename,a.name as store_name,s.name as name,b.role_id,s.id as sid from broker_info b
                ...
                left join purview_company_group p on b.role_id = p.id
  ...
  public function get_by_id($id)
  {
    ...
    foreach ($result_new as $key => $val) {
      $sql = "select name from purview_modules where id= " . $key . ' and status = 1';
      $info = $this->dbback_city->query($sql)->row_array();
      $result_array[$key]['name'] = $info['name'];
      foreach ($val as $k => $v) {
        $sql = "select pname from purview_list where pid = " . $v . " and status = 1";
      ...
      if (!empty($result)) {
          foreach ($result as $key => $val) {
              ...
              if (is_array($result_new_item) && !empty($result_new_item)) {
                  foreach ($result_new_item as $key1 => $val1) {
                      $result_func_auth = array_merge($result_func_auth, $val1);
                  }
              }
          }
      }
  ...
  public function get_menu_by($id, $cid)
  {
    $id = $id;
    $company_id = $cid;
    $sql = "select func_auth from purview_company_group where id = " . $id . " and company_id = " . $company_id;
    $result = $this->dbback_city->query($sql)->row_array();
  ...
  public function save_group_func($equipment, $id)
  {
    ...
    $sql = "update purview_company_group set func_auth = '" . $func_auth . "' where id ='" . $id . "'";
  ...
  {
    $sql = "select func_auth from purview_company_group where id = " . $id;
  ...
  public function get_all_company_id()
  {
    ...
    $sql = "SELECT DISTINCT company_id FROM `purview_company_group`";
