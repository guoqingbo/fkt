...
class house_base_model extends MY_Model
{
  ...
  {
    parent::__construct();
    $this->wuba_street_fkt = 'wuba_street_fkt';
  }
   ...
   */
  public function get_count_by_cond($cond_where = '', $cond_or_like = array())
  {
    $count_num = 0;
  ...
  public function get_list_by_cond($cond_where = '', $offset = 0, $limit = 10,
                                   $order_key = 'updatetime', $order_by = 'DESC', $order_key2 = '', $order_by2 = '', $order_key3 = '', $order_by3 = '')
  {
    //房源需求信息表
    $tbl_demand = $this->get_tbl();
    ...
    if (!empty($order_key2) && !empty($order_by2)) {
      $this->dbback_city->order_by($order_key2, $order_by2);
    }

    if (!empty($order_key3) && !empty($order_by3)) {
      $this->dbback_city->order_by($order_key3, $order_by3);
    }
   ...
   * @return  array   出售出租信息列表
   */
  public function get_list_by_cond_or_like($cond_where = '', $cond_or_like = array(), $offset = 0, $limit = 10,
                                           $order_key = 'updatetime', $order_by = 'DESC', $order_key2 = '', $order_by2 = '', $order_key3 = '', $order_by3 = '')
  {
    //房源需求信息表
    $tbl_demand = $this->get_tbl();
   ...
   * @return  array   出售出租信息列表
   */
  public function get_list_by_cond_or_like_export($cond_where = '', $cond_or_like = array(),
                                                  $order_key = 'updatetime', $order_by = 'DESC', $order_key2 = '', $order_by2 = '', $order_key3 = '', $order_by3 = '', $limit = 0, $offset = 1000)
  {
    //房源需求信息表
    $tbl_demand = $this->get_tbl();
    ...
    $this->dbback_city->limit($offset, $limit);
  ...
  public function get_wuba_street_id($where = array(), $database = 'db_city')
  {
    $result = $this->get_data(array('form_name' => $this->wuba_street_fkt, 'where' => $where, 'select' => array('wuba_dist_id,wuba_street_id')), $database);
    return $result;
  }
  ...
  //获取佣金分配，默认为5：5
  public function get_commission_ratio_id($commission_ratio_id)
  {
    $commission_ratio_id = intval($commission_ratio_id);
    return $commission_ratio_id > 0 ? $commission_ratio_id : 5;
  }

  //获取佣金分配百分比
  public function get_commission_ratio($commission_ratio)
  {
    $commission_ratio_arr = explode(':', $commission_ratio);
    return array('house' => ($commission_ratio_arr[0] / 10 * 100) . '%',
      'customer' => ($commission_ratio_arr[1] / 10 * 100) . '%');
  }
}
