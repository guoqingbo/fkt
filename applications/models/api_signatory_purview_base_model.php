 ...
 * @author          sun
 */
class Api_signatory_purview_base_model extends MY_Model
{
  ...
  public function __construct()
  {
    ...
    $this->_mem_key = $city . '_api_signatory_purview_base_model_';
    //初始化经纪人基本信息
    if (is_full_array($this->signatory_info)) {
      //引用模块权限
      $this->load->model('purview_module_base_model');
      //引入菜单权限
      // $this->load->model('purview_menu_base_model');
      //引入功能权限
      // $this->load->model('purview_func_base_model');
      //初始化用户权限
      $this->_init_user_purview();
    }
  }

  //根据公司编号和权限编号获取相应的菜单和功能权限
  private function _init_user_purview()
  {
    //判断经纪人是否是联网经纪人
    if ($this->signatory_info['package_id'] == 2 && $this->signatory_info['area_id'] == 2) {
      $this->load->model('purview_system_role_base_model');
      $purview = $this->purview_system_role_base_model->get_by_id(2);
    } else {
      //根据公司编号和经纪人角色获取相应的权限
      $company_id = $this->signatory_info['company_id'];
      $role_id = $this->signatory_info['role_id'];
      $this->load->model('purview_company_role_base_model');
      $purview = $this->purview_company_role_base_model->
      get_by_company_id_role_id($company_id, $role_id);
    }
    $this->user_purview = array(
      'menu_auth' => isset($purview['menu_auth']) ?
        unserialize($purview['menu_auth'])
        : array(),
      'func_auth' => isset($purview['func_auth']) ?
        unserialize($purview['func_auth'])
        : array(),
    );
  }
  ...
  {
    $all_module = $this->purview_module_base_model->get_all();
    //经纪人的菜单和功能权限
    $menu_auth = $this->user_purview['menu_auth'];
    $func_auth = $this->user_purview['func_auth'];
    //模块菜单
    $module_menu = array();
    $none_purview_url = '/purview/none/';
    //根据权限判断显示的菜单
    foreach ($all_module as $v_module) {
      ...
      {
        $v_module['url'] = $none_purview_url;
        //查看模块下有多少菜单 以order排序
        $menu = $this->purview_menu_base_model->get_by_module_id($module_id);
        $is_auth = 0;
        foreach ($menu as $v_m) {
          $menu_id = $v_m['id'];
          //菜单地址无需权限控制跳出下一个模块
          ...
          if ($v_m['init_auth'] == 1
            ...
            //获取此类和方法的功能ID
            $func = $this->purview_func_base_model->get_by_class_method($class, $method);
            if (is_full_array($func)) {
              if (in_array($func['id'], array_keys($func_auth))) //勾起了权限
              {
                ...
                {
                  //查询菜单下所有功能
                  $func_menu = $this->purview_func_base_model->get_func_menu_by_menu_id($menu_id);
          ...
          //找到地址直接进入下个模块判断
          if ($v_module['url'] != $none_purview_url) {
            break;
          }
        }
        if ($is_auth == 1) {
  ...
  //获取某个模块下的菜单权限
  public function get_menu_purview($class, $method)
  {
    $user_menu = ''; //用户菜单
    ...
    //先根据菜单地址获取菜单信息
    $menu = $this->purview_menu_base_model->get_by_url($url);
    //根据功能去查是否存在菜单功能
    $func = $this->purview_func_base_model->get_by_class_method($class, $method);
    if (!is_full_array($menu) && is_full_array($func)) {
      $menu = $this->purview_menu_base_model->get_by_id($func['menu_id']);
    }
    //查看模块是否需要做权限
    if (is_full_array($menu)) {
      $module = $this->purview_module_base_model->get_by_id($menu['module_id']);
    }
    //菜单功能
    if (is_full_array($func) && $func['is_menu'] == 1) {
      //查询菜单下所有功能
      $func_menu = $this->purview_func_base_model->get_func_menu_by_menu_id($func['menu_id']);
    ...
    if (is_full_array($menu)) //菜单不需要做权限控制
    {
      ...
      $menu_id = $menu['id'];
      //获取模块下所有菜单
      $module_menu = $this->purview_menu_base_model->get_by_module_id($module_id);
      //根据菜单权限筛选出合法的菜单
      foreach ($module_menu as $v) {
        ...
        {
          //看看有没有菜单功能
          $func_menu = $this->purview_func_base_model->get_func_menu_by_menu_id($v['id']);
          if (is_full_array($func_menu)) //此菜单下有菜单功能
          {
            foreach ($func_menu as $v_k => $v_f) {
              ...
              {
                $menu = $this->purview_menu_base_model->get_by_id($v_f['menu_id']);
  ...
  public function get_batch_func_purview($arr_func)
  {
    ...
    if (is_full_array($arr_func)) {
      foreach ($arr_func as &$v) {
        $func_auth_result = $this->get_func_purview($v['class'], $v['method']);
  ...
  public function compare_has_purview($area_auth, $compare_param)
  {
    ...
    if ($area_auth['area'] == 1 && isset($compare_param['signatory_id'])) //本人
    {
      $compare_result = $compare_param['signatory_id']
      == $this->signatory_info['signatory_id'] ? true : false;
    } else if ($area_auth['area'] == 2) //门店
    {
      if (!isset($compare_param['agency_id'])) {
        //通过经纪人编号查找门店编号
        //@todo
        $this->load->model('api_signatory_base_model');//经纪人接口模型类
        $signatory_info = $this->api_signatory_base_model->get_by_signatory_id($compare_param['signatory_id']);
        $compare_param['agency_id'] = $signatory_info['agency_id'];
      }
      $compare_result = $compare_param['agency_id']
      == $this->signatory_info['agency_id'] ? true : false;
    } else if ($area_auth['area'] == 3) //公司
    {
      ...
      if (!isset($compare_param['company_id'])) {
        if (isset($compare_param['agency_id'])) //设置了门店
        {
          ...
          $company = $this->api_signatory_base_model->get_by_agency_id($compare_param['agency_id']);
          ...
          $this->load->model('api_signatory_base_model');//经纪人接口模型类
          $signatory_info = $this->api_signatory_base_model->get_by_signatory_id($compare_param['signatory_id']);
          $compare_param['company_id'] = $signatory_info['company_id'];
        }
      }
      $compare_result = $compare_param['company_id']
      == $this->signatory_info['company_id'] ? true : false;
    }
