...
/**
 * attendance_config_base_model CLASS
 *
 * 考勤设置查询、添加、删除、修改管理功能
 *
 * @package         MLS
 ...
 * @author          sun
 */
class Attendance_config_base_model extends MY_Model
{
   ...
   * @var string
   */
  private $_tbl = 'attendance_config';
   ...
   * @param int $order_attendance_config_base 排序字段
   * @param string $order_by 升序、降序，默认降序排序
   * @return array 返回多条记录组成的二维数组
   */
  public function get_all_by($where, $start = 0, $limit = 20,
                             $order_attendance_config_base = 'id', $order_by = 'DESC')
  {
    //查询字段
    ...
    //排序条件
    $this->db_city->order_by($order_attendance_config_base, $order_by);
    if ($start >= 0 && $limit > 0) {
      $this->db_city->limit($limit, $start);
    }
    //返回结果
    return $this->db_city->get($this->_tbl)->result_array();
  }
...
/* End of file attendance_config_base_model.php */
/* Location: ./applications/models/attendance_config_base_model.php */
