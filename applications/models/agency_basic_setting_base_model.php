 ...
 * @author          xz
 */
class Agency_basic_setting_base_model extends MY_Model
{
  /**
   * 基础设置表名称
   * @var string
   */
  private $_bs_tbl = 'agency_basic_setting';
   ...
   * @return array
   */
  public function get_data_by_agency_id($id)
  {
  ...
  {
    $this->dbback_city->select('*');
    $this->dbback_city->where(array('company_id' => $id, 'agency_id' => 0));
    return $this->dbback_city->get($this->_bs_tbl)->result_array();
  }
   ...
   * @return array
   */
  public function get_all_agency_data()
  {
    $this->dbback_city->select('agency_id , sell_house_nature_public , rent_house_nature_public , buy_customer_nature_public , rent_customer_nature_public , house_public_time , customer_public_time');
   ...
   */
  public function judge_by_id($paramArray, $agency_id)
  {
    $sql = "select id from agency_basic_setting where agency_id = " . $agency_id;
    $result = $this->db_city->query($sql)->result_array();
    if (!empty($result)) {
      return $this->update_by_id($paramArray, $agency_id);
    } else {
      $paramArray['agency_id'] = $agency_id;
