...
/**
 * Api_broker_sincere_base_model CLASS
 *
 * 经纪人增加，扣除积分接口
 *
 * @package         MLS
 ...
 * @author          sun
 */
class Api_broker_sincere_base_model extends MY_Model
{
  /**
   * 类初始化
   */
  public function __construct()
  {
    ...
    $this->_mem_key = $city . '_api_broker_sincere_base_model_';
  }
   ...
   */
  function update_trust($broker_id, $func_alias, $score = '')
  {
    ...
    if ($score < 0) //扣分
    {
      $trust = $this->broker_info_base_model->get_trust_by_broker_id($broker_id);
      if ($trust < abs($score)) {
        $score = -($trust);
      }
    }
    ...
    //更新分值
    $this->broker_info_base_model->update_self_trust_by_broker_id($broker_id, $score);
    $trust = $this->broker_info_base_model->get_trust_by_broker_id($broker_id);
    //插入记录
    $this->load->model('sincere_trust_record_base_model');
    $action_id = $operator_trust_action[$func_alias]['id'];
    $trust_record = array(
      'broker_id' => $broker_id, 'action_id' => $action_id,
   ...
   * @return {'trust' : '', 'level' : '' }
   */
  public function get_trust_level_by_broker_id($broker_id)
  {
    $this->load->model('broker_info_base_model');
    $trust = $this->broker_info_base_model->get_trust_by_broker_id($broker_id);
    //根据分值获取等级
  ...
  {
    $this->load->model('broker_info_base_model');
    return $this->broker_info_base_model->get_trust_by_broker_id($broker_id);
  }
   ...
   */
  public function get_trust_appraise_count($broker_id)
  {
    $mem_key = $this->_mem_key . 'get_trust_appraise_count_' . $broker_id;
    ...
    if (isset($cache['is_ok']) && $cache['is_ok'] == 1) {
      ...
      $trust_count = $this->sincere_trust_count_base_model->get_by_broker_id($broker_id);
   ...
   * @param int $broker_id
   */
  public function good_avg_rate($broker_id)
  {
    $mem_key = $this->_mem_key . 'good_avg_rate_' . $broker_id;
    $cache = $this->mc->get($mem_key);
    if (isset($cache['is_ok']) && $cache['is_ok'] == 1) {
      $result = $cache['data'];
    } else {
      ...
      //经纪人好评率
      $good_rate = $this->get_trust_appraise_count($broker_id);
      //经纪人高于平均好评率比例
   ...
   * @return array
   */
  public function get_appraise_and_avg($broker_id)
  {
    $mem_key = $this->_mem_key . 'get_good_avg_rate_' . $broker_id;
    $cache = $this->mc->get($mem_key);
    if (isset($cache['is_ok']) && $cache['is_ok'] == 1) {
      ...
      //经纪人的分值
      $appraise = $this->sincere_appraise_base_model->get_by_broker_id($broker_id);
      //获取基本的统计信息
   ...
   * @return array
   */
  public function get_appraise_info($broker_id)
  {
    $mem_key = $this->_mem_key . 'get_appraise_info_' . $broker_id;
    $cache = $this->mc->get($mem_key);
    if (isset($cache['is_ok']) && $cache['is_ok'] == 1) {
      ...
      $infomation_action_id = $operator_infomation_action['broker_appriase']['id'];
      ...
      $attitude_action_id = $operator_attitude_action['broker_appriase']['id'];
      ...
      $business_action_id = $operator_business_action['broker_appriase']['id'];
  ...
  public function is_exist_transaction($broker_id, $transaction_id)
  {
    ...
    is_exist_transaction($broker_id, $transaction_id);
  }
   ...
   */
  public function update_infomation($broker_id, $alias, $score = 0)
  {
    //获取类型编号
    $this->load->model('sincere_weight_base_model');
    $type_weight_config = $this->sincere_weight_base_model->get_config();
    ...
    $appraise_record = array(
      'broker_id' => $broker_id, 'type_id' => $type_id,
   ...
   * @return int 插入编号
   */
  public function update_attitude($broker_id, $alias, $score = 0)
  {
    //获取类型编号
    $this->load->model('sincere_weight_base_model');
    $type_weight_config = $this->sincere_weight_base_model->get_config();
   ...
   * @return int 插入编号
   */
  public function update_businsess($broker_id, $alias, $score = 0)
  {
    //获取类型编号
    $this->load->model('sincere_weight_base_model');
    $type_weight_config = $this->sincere_weight_base_model->get_config();
  ...
  /**
   * 处罚记录
   * @param int $broker_id 举报人编号
   * @param int $brokered_id 受罚人编号
   * @param string $alias 类型名称
   * @param int $score 分值
   * @param string $number 房源编号
   * @param string $house_info 房源详细信息 序列化房源数组信息
   */
  public function punish($broker_id, $brokered_id, $alias, $score, $number, $house_info)
  {
    ...
    } else if ($alias == 'house_info_false' || $alias == 'customer_info_false') //扣信息真实度
    {
      ...
      || $alias == 'whether_accept_brokerage'
    ) //扣合作满意度
    {
   ...
   */
  public function appraise_appeal($broker_id, $appraise_id, $transaction_id,
                                  $photo_url, $photo_name, $reason)
  {
    $this->load->model('broker_info_base_model');
    $broker_info = $this->broker_info_base_model->get_by_broker_id($broker_id);
    $insert_data = array(
      'broker_id' => $broker_id, 'broker_name' => $broker_info['truename'],
  ...
  public function update_appraise_appeal_status($id, $status)
  {
    ...
    if ($effected_rows && $status == 2) //申诉成功
    {
      ...
      //二、恶意评价罚分
      $score = $this->update_trust($appraise_cooperate['broker_id'], 'mali_appraise');
      //三、进入罚分库
      $this->punish($appraise_cooperate['partner_id'], $appraise_cooperate['broker_id'], 'mali_appraise', $score,
...
/* End of file Api_broker_sincere_base_model.php */
/* Location: ./applications/models/Api_broker_sincere_base_model.php */
