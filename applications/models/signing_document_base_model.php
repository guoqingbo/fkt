 ...
 * @author          angel_in_us
 */
class Signing_document_base_model extends MY_Model
{

    public function __construct()
    {
        parent::__construct();
        $this->signing_document = 'signing_document'; //签约公告
        // $this->signing_document_broker = 'signing_document_broker'; //公司公告经纪人
    }
    ...
    public function get_count_by_cond($where = array(), $database = 'dbback_city')
    {
        ...
        $this->db->from($this->signing_document);
        //查询条件
        if ($where) {
            $this->db->where($where);
        }
     ...
     * @return  int   符合条件的信息条数
     */
    public function get_signing_document_by($where, $start = -1, $limit = 20,
                                            $order_key = 'id', $order_by = 'DESC')
    {
        ...
        $this->dbback_city->from($this->signing_document);
        //$this->dbback_city->join($this->_tbl1,$this->signing_document.'.detail_id = '.$this->_tbl1.'.id');

        //排序条件
        $this->dbback_city->order_by($this->signing_document . '.' . $order_key, $order_by);
        if ($start >= 0 && $limit > 0) {
     ...
     * @return  int   符合条件的信息条数
     */
    public function get_signing_document_broker_by($where)
    {
        //查询字段
        $this->dbback_city->select("*");
        ...
        $this->dbback_city->from($this->signing_document_broker);
        //返回结果
    ...
    {
        $this->dbselect($database);
        $insert_id = $this->db->insert($this->signing_document, $where);
        return $insert_id;
    }
    ...
    {
        $this->dbselect($database);
        return $this->db->delete($this->signing_document, $where);
    }
    ...
    {
        $this->dbselect($database);
        $insert_id = $this->db->insert($this->signing_document_broker, $where);
        return $insert_id;
    }

    /**
     * 删除signing_document_broker
     *
     * @access  protected
     * @param  string $id 参数
     * @return  array   结果
    ...
    function update_notice_broker($id, $update_arr, $database = 'db_city')
    {
        $this->dbselect($database);
        $this->db->where('id', $id);
        $insert_id = $this->db->update($this->signing_document, $update_arr);
        return $insert_id;
    }
