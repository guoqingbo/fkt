 ...
 * @author          lu
 */
class Company_employee_base_model extends MY_Model
{
  ...
  {
    $sql = "select count(*) as number from broker_info b
                left join agency a on a.id = b.agency_id" . $where;
  ...
  public function get_all_by($c_id, $where, $start = -1, $limit = 20)
  {
    ...
    if ($c_id > 0) {
      $sql = "select b.id,b.broker_id,b.truename,b.phone,b.qq,b.package_id,b.base_salary,r.id rid,r.remark,a.name as store_name,s.name as sname
                    from broker_info b
                    ...
                    left join permission_system_group s on p.system_group_id=s.id {$where} ";
      $result = $this->dbback_city->query($sql)->result_array();
    } else {
      $sql = "select b.id,b.broker_id,b.truename,b.phone,b.qq,b.package_id,b.base_salary,r.id rid,r.remark,a.storename as store_name,s.name as sname
                    from broker_info b
                    left join broker_info_remark r on r.remarker_id = b.broker_id
                    left join register_broker a on a.broker_info_id = b.id
                    left join permission_company_group p on b.role_id = p.id 
                    left join permission_system_group s on p.system_group_id=s.id {$where} ";
      $result = $this->dbback_city->query($sql)->result_array();
    }
