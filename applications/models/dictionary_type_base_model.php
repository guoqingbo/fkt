 ...
 * @author          sun
 */
class Dictionary_type_base_model extends MY_Model
{
...
//        $this->load->model('department_access_area_base_model');
  ...
  /**
   * 根据数据字典id获取类型数据
   * @param string $dictionary_type_id_str 字典id
   * @param int $order_key 排序字段
   * @param string $order_by 升序、降序，默认降序排序
   * @return array 返回多条公司记录组成的二维数组
   */
  public function get_all_by_dictionary_type_id($dictionary_type_id_str = '', $order_key = 'id', $order_by = 'ASC')
  {
    ...
    $status = 1;
    if ($dictionary_type_id_str) {
      if (!empty($dictionary_type_id_str)) {
        $where_cond = 'id in (' . $dictionary_type_id_str . ')';
        $where_cond .= ' and status = ' . $status;
        //查询条件
   ...
   * @return array 类型记录组成的一维数组
   */
  public function get_by_id($dictionary_type_id)
  {
    $dictionarys = array();
    //查询字段
    if ($this->_select_fields) {
    ...
    if (is_array($dictionary_type_id)) {
      $this->dbback_city->where_in('id', $dictionary_type_id);
      $dictionarys = $this->dbback_city->get($this->_tbl)->result_array();
    } else if (intval($dictionary_type_id) >= 0) {
      $this->dbback_city->where('id', $dictionary_type_id);
  ...
  /**
   * 设置类型的状态
   * @param int $department_id 类型编号
   * @param int $status 类型状态
   */
  public function set_esta($dictionary_type_id, $status)
  {
    $data = array();
    $data['status'] = $status;
    $this->update_by_dictionary_type_id($data, $dictionary_type_id);
  }
   ...
   * @return array
   */
  function get_dictionary_type_by()
  {
   ...
   * @return array
   */
  function get_dictionary_type_by_one($dictionary_type_id = 0)
  {
  ...
  /**
   * 查找某类型下的子类型列表
   * @param int $dictionary_type_id 类型编号
   * @return array
   */
  function get_children_by_dictionary_type_id($dictionary_type_id)
  {
    //查询字段
    if ($this->_select_fields) {
      $this->dbback_city->select($this->_select_fields);
   ...
   * @return array 小区信息数组
   */
  public function get_dictionary_type_info_by_kw2($keyword = '', $search_arr = 0, $limit = 10, $order_key = 'id', $order = 'ASC')
  {
    $cmt_info = array();
...
//      $this->dbback_city->join ("(select dictionary_type_id, count(*) from " . $this->_tbl . " where `status` = " . $status . " group by dictionary_type_id) as b", "a.id = b.dictionary_type_id", 'left');
      ...
      $cond_where = 'select a.id as id, a.name as `name`, b.total as sub from';
      $cond_where .= "(select id, `name` from " . $this->_tbl . " where `name` LIKE '%" . $keyword . "%' and `status` = " . $status . ") as a";
      $cond_where .= " left join ";
      $cond_where .= "(select dictionary_type_id, count(*) as total from " . $this->_tbl . " where `status` = " . $status . " group by dictionary_type_id) as b";
      $cond_where .= " on a.id = b.dictionary_type_id ";
   ...
   * @return array 小区信息数组
   */
  public function get_dictionary_type_info_by_kw($keyword = '', $search_arr = 0, $limit = 10, $order_key = 'id', $order = 'ASC')
  {
    $cmt_info = array();
    $keyword = trim(strip_tags($keyword));
    ...
    if ($keyword != '' && is_full_array($search_arr)) {
      ...
      if (in_array($search_arr['role_level'], array(1, 2, 3, 4))) {
        ...
        //当前类型范围
        $this->dbback_city->where('id', $search_arr['dictionary_type_id']);
      } else if (in_array($search_arr['role_level'], array(5))) {
        //一级类型范围
        $this->dbback_city->where('dictionary_type_id', $search_arr['dictionary_type_id']);
      } else {
        return $cmt_info;
      }
   ...
   * @return array
   */
  function get_dictionary_1_by_dictionary_type_id($dictionary_type_id)
  {
    //查询字段
    if ($this->_select_fields) {
   ...
   * @return int
   */
  function count_childrea_by_dictionary_type_id($dictionary_type_id)
  {
    $this->dbback_city->where('status', 1);
    $this->dbback_city->where('dictionary_type_id', $dictionary_type_id);
    return $this->dbback_city->count_all_results($this->_tbl);
  ...
  /**
   * 查找某类型名下的经纪人总数
   * @param int $dictionary_type_id 公司编号
   * @return int
   */
  function count_childbroker_by_department_id($department_id)
  {
    $this->dbback_city->where('status', 1);
   ...
   * 添加类型
   * @param $name 类型名
   * @param $name_abbr 类型缩写
   * @param $desc 类型描述
   * @param int $status 类型状态
   * @param int $father_dictionary_type_id 父类型
   * @return int 插入成功的编号
   */
  public function add_dictionary_type($name, $name_abbr, $desc, $status = 1, $father_dictionary_type_id = 0)
  {
    $data = array();
    $data['name'] = $name;
    $data['name_abbr'] = $name_abbr;
    $data['desc'] = $desc;
    $data['dictionary_type_id'] = $father_dictionary_type_id;
    $data['status'] = $status;
  ...
  //获取类型信息
  public function select_dictionary_type($dictionary_type_id)
  {
    $this->dbback_city->where('id', $dictionary_type_id);
    $this->dbback_city->where('status', 1);
    return $this->dbback_city->get($this->_tbl)->result_array();
   ...
   * @return
   */
  public function update_dictionary_type($dictionary_type_id, $name, $name_abbr, $desc, $status = 1, $father_dictionary_type_id = 0)
  {
    $data = array();
    $data['name'] = $name;
    $data['name_abbr'] = $name_abbr;
    ...
    $result = $this->update_by_dictionary_type_id($data, $dictionary_type_id);

    return $result;
  }

  public function update_by_dictionary_type_id($update_data, $dictionary_type_id)
  {
    if (is_array($dictionary_type_id)) {
      $ids = $dictionary_type_id;
    } else {
      $ids[0] = $dictionary_type_id;
    }
  ...
  /**
   * 删除类型数据
   * @param array or int $dictionary_type_id 类型编号 可接受整数或者数组编号
   * @return boolean true 成功 false 失败
   */
  public function delete_by_dictionary_type_id($dictionary_type_id)
  {
    //多条删除
    if (is_array($dictionary_type_id)) {
      $ids = $dictionary_type_id;
