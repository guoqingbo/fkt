 ...
 * @author          sun
 */
class Agency_permission_node_base_model extends MY_Model
{
  /**
   * 权限模块表
   * @var string
   */
  private $_tbl = 'agency_permission_father_node';
  private $_tbl_list = 'agency_permission_node';
  ...
  public function __construct()
  {
    ...
    $this->_mem_key = $city . '_agency_permission_node_base_model_';
  }
  ...
  public function get_all_by($where = '', $start = -1, $limit = 20)
  {
    ...
    $sql = "select pid,name,pname,mid,l.status as status from agency_permission_node l LEFT JOIN agency_permission_father_node m on " . $where;
  ...
  {
    $sql = "select * from agency_permission_father_node ";
    $result = $this->dbback_city->query($sql)->result_array();
    return $result;
  }
  ...
  {
    $sql = "select id,name from agency_permission_node where status=1 and id = '" . $id . "'";
...
/* End of file agency_permission_node_base_model.php */
/* Location: ./applications/models/agency_permission_node_base_model.php */
