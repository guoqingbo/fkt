...
/**
 * permission_secondtab_menu_base_model CLASS
 *
 * 权限功能添加、删除、修改管理功能
 *
 * @package         MLS
 ...
 * @author          sun
 */
class Purview_secondtab_menu_base_model extends MY_Model
{
   ...
   * @var string
   */
  private $_tbl = 'purview_second_tab';
  ...
  public function __construct()
  {
    ...
    $this->_mem_key = $city . '_signatory_secondtab_menu_';
  }
  ...
  public function get_all()
  {
    ...
    if (isset($cache['is_ok']) && $cache['is_ok'] == 1) {
      ...
      $func = $this->get_all_by();
      $this->mc->add($mem_key, array('is_ok' => 1, 'data' => $func), 86400);
    }
    return $func;
  }
   ...
   */
  public function get_by_class_method($class, $method)
  {
    $all_func = $this->get_all();
    $new_func = array();
    if (is_full_array($all_func)) {
      foreach ($all_func as $v) {
        $arr_method = explode(',', $v['method']);
        if ($v['class'] == $class && in_array($method, $arr_method)) {
          $new_func = $v;
          break;
        }
      }
  ...
  /**
   * 通过菜单编号查找所属的功能
   * @param int $tab_id 菜单编号
   */
  public function get_func_menu_by_tab_id($tab_id)
  {
    $all_func = $this->get_all();
    $new_func = array();
    if (is_full_array($all_func)) {
   ...
   */
  public function get_by_tab_id_init_auth($tab_id, $init_auth)
  {
    $all_func = $this->get_all();
    $new_func = array();
    if (is_full_array($all_func)) {
      foreach ($all_func as $v) {
        if ($v['tab_id'] == $tab_id && $v['init_auth'] == $init_auth) {
          $new_func[] = $v;
        }
      }
    }
   ...
   */
  public function delete_by_tab_id($tab_id)
  {
    //多条删除
    if (is_array($tab_id)) {
      $tab_ids = $tab_id;
    ...
    if ($tab_ids) {
      $this->db_city->where_in('tab_id', $tab_ids);
...
/* End of file signatory_secondtab_menu_model_menu_base_model.php */
/* Location: ./applications/models/signatory_secondtab_menu_model_menu_base_model.php */
