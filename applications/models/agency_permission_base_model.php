...
/**
 * Agency_permission_base_model CLASS
 *
 * 门店访问数据范围类
 *
 * @package         MLS
 ...
 * @author          Fisher
 */
class Agency_permission_base_model extends MY_Model
{
   ...
   * @var string
   */
  private $_tbl = 'agency_permission';
  private $_tb2 = 'agency';
  ...
  //设置当前经纪人门店ID、公司、角色
  public function set_agency_id($agency_id = 0, $company_id = 0, $role_level = 0)
  {
    if ($agency_id > 0) {
      $this->_agency_id = intval($agency_id);
    }
    if ($company_id > 0) {
   ...
   * @param int $main_agency_id 主门店ID
   * @param int $sub_agency_id 被关联门店ID
   * @return array 返回数据
   */
  public function get_func_area_by_main_sub_id($main_agency_id, $sub_agency_id)
  {
    //查询字段
    $this->dbback_city->select('func_auth');
    //查询条件
    $this->dbback_city->where(array('main_agency_id' => $main_agency_id, 'sub_agency_id' => $sub_agency_id));
    //返回结果
    $agencyarea = $this->dbback_city->get($this->_tbl)->row_array();
    if (is_full_array($agencyarea) && !empty($agencyarea['func_auth'])) {
      $func_area = $agencyarea['func_auth'];
    } else {
      $func_area = array();
    }
    return $func_area;
   ...
   * @return array 返回数据
   */
  public function get_agency_id_by_main_id_access($main_agency_id, $access_str = '')
  {
    //判断当前经纪人是否是总经理
    if (1 == $this->_role_level) {
      $this->dbback_city->select('id as sub_agency_id');
      $this->dbback_city->where('status', 1);
      $this->dbback_city->where('company_id', $this->_company_id);
      $agency_id_arr = $this->dbback_city->get($this->_tb2)->result_array();
    } else {
      //查询字段
      $this->dbback_city->select('sub_agency_id');
      //查询条件
      $where_cond = array('main_agency_id' => $main_agency_id, 'is_effective' => 1, $access_str => 1);
      $this->dbback_city->where($where_cond);
      //返回结果
      $agency_id_arr = $this->dbback_city->get($this->_tbl)->result_array();
    }
    return $agency_id_arr;
  }
   ...
   * @return array 返回数据
   */
  public function get_id_by_main_id($main_agency_id)
  {
    //查询字段
    $this->dbback_city->select('id');
    //查询条件
    $where_cond = array('main_agency_id' => $main_agency_id);
    $this->dbback_city->where($where_cond);
    //返回结果
    $agency_id_arr = $this->dbback_city->get($this->_tbl)->result_array();
    return $agency_id_arr;
   ...
   * @return array 返回数据
   */
  public function get_data_by_main_id($main_agency_id)
  {
    //查询字段
  ...
  /**
   * 根据被关联门店id，获得相关数据id
   * @param int $sub_agency_id 主门店ID
   * @param string $access_str 待查询数据范围字段名
   * @return array 返回数据
   */
  public function get_id_by_sub_id($sub_agency_id)
  {
    //查询字段
    $this->dbback_city->select('id');
    //查询条件
    $where_cond = array('sub_agency_id' => $sub_agency_id);
    $this->dbback_city->where($where_cond);
    //返回结果
    $agency_id_arr = $this->dbback_city->get($this->_tbl)->result_array();
    return $agency_id_arr;
   ...
   * @param string $main_agency_id 主门店ID
   * @param array $owner_arr 数据归属
   * @param string $node_id 权限节点id
   * @return array 返回数据
   */
  public function check($main_agency_id = '', $owner_arr = array(), $node_id = '')
  {
    $result = false;
    //判断登录经纪人是否是总经理角色
    if (1 == $this->_role_level) {
      ...
      //数据所属门店id
      if (isset($owner_arr['agency_id']) && !empty($owner_arr['agency_id'])) {
        $sub_agency_id = intval($owner_arr['agency_id']);
      }
      //判断是否是自己门店的房客源数据
      if (isset($sub_agency_id) && $sub_agency_id == $this->_agency_id) {
        $result = true;
      } else {
        if (is_int($main_agency_id) && is_int($sub_agency_id) && !empty($main_agency_id) && !empty($sub_agency_id) && !empty($node_id)) {
          //查询字段
          $this->dbback_city->select('func_auth');
          //查询条件
          $where_cond = array('main_agency_id' => $main_agency_id, 'is_effective' => 1, 'sub_agency_id' => $sub_agency_id);
          $this->dbback_city->where($where_cond);
          //返回结果
   ...
   * @return array 返回数据
   */
  public function get_data_by_main_sub_id($main_agency_id, $sub_agency_id)
  {
    //查询字段
    $this->dbback_city->select('*');
    //查询条件
   ...
   * @param int $main_agency_id 主门店ID
   * @param array $agency_id_arr 被关联门店id
   * @return array 返回多条公司记录组成的二维数组
   */
  public function init_agency_area($company_id = 0, $main_agency_id = 0, $agency_id_arr = array(), $func_auth = '')
  {
    if (is_full_array($agency_id_arr)) {
      foreach ($agency_id_arr as $k => $v) {
        $insert_data = array(
          'company_id' => $company_id,
          'main_agency_id' => $main_agency_id,
          'sub_agency_id' => intval($v),
          'func_auth' => $func_auth,
          'is_view_house' => 1,
          'is_house_match' => 1,
  ...
  {
    $main_agency_id = $param_arr['main_agency_id'];
    $sub_agency_id = $param_arr['sub_agency_id'];

    $query_sql = 'select id from ' . $this->_tbl . ' where main_agency_id = "' . $main_agency_id . '" and sub_agency_id = "' . $sub_agency_id . '"';
