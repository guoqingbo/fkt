 ...
 * @author          lu
 */
class Department_organization_base_Model extends MY_Model
{
  ...
  {
    $sql = "select id,name,level from purview_system_group order by level ASC";
    $result = $this->dbback_city->query($sql)->result_array();
    return $result;
  ...
  {
//    $sql = "select id from purview_department_group where department_id = " . $department_id . " AND system_group_id in (".$system_group_id.")";
//    $result = $this->dbback_city->query($sql)->result_array();

      $result = [];
      $res = $this->dbback_city->select('id')
          ->where("department_id = " . $department_id . " AND system_group_id in (" . $system_group_id . ")")
  ...
  {
    $sql = "select p.id,p.system_group_id,b.signatory_id from purview_department_group p left join signatory_info b on p.id =b.role_id where p.id = " . $role_id . " AND b.signatory_id = " . $signatory_id;
    $result = $this->dbback_city->query($sql)->row_array();
//         $result = [];
//      $res = $this->dbback_city->select('id,system_group_id')
//          ->where("id in (" . $role_id . ")")
//          ->get("purview_department_group")
//          ->result_array();
//      if (!empty($res)){
//          foreach ($res as $key => $val){
//              $result['id'][] = $val['id'];
//              $result['system_group_id'][] = $val['system_group_id'];
//          }
//          $result['signatory_id'] = $signatory_id;
//      }
    return $result;
  ...
  {
    $sql = "select system_group_id from purview_department_group where id = " . $role_id;
    $result = $this->dbback_city->query($sql)->row_array();
    return $result;
