...
/**
 * bargain_divide_base_model CLASS
 *
 ...
 * @author          sun
 */
class Bargain_divide_base_model extends MY_Model
{
   ...
   * @var string
   */
  private $_tbl = 'bargain_divide';
  private $_tbl_bargain = 'bargain';
   ...
   * @return array 合同业绩分成记录组成的一维数组
   */
  public function get_by_bargain_divide_number($bargain_divide_no)
  {
    //查询字段
    if ($this->_select_fields) {
      $this->dbback_city->select($this->_select_fields);
    ...
    //查询条件
    $this->dbback_city->where('number', $bargain_divide_no);
    return $this->dbback_city->get($this->_tbl)->row_array();
  }
  ...
  {
    $sql = "select DISTINCT(bargain_divide.c_id),bargain_divide.achieve_signatory_id_b as signatory_id,c.commission_total from bargain_divide  LEFT JOIN bargain c on bargain_divide.c_id = c.id where " . $where;
    //返回结果
    return $result = $this->dbback_city->query($sql)->result_array();
  }
}

/* End of file bargain_divide_base_model.php */
/* Location: ./applications/models/bargain_divide_base_model.php */
