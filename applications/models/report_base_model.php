 ...
 * @author          sun
 */
class Report_base_model extends MY_Model
{
  ...
  /**
   *
   * @param type $cond_where查找条件
   * @return number获取房源id
   */
  public function get_report_house_bid($cond_where)
  {
    $arr_ids = array();

    if ($cond_where) {
      $this->dbback_city->select('number');
      $this->dbback_city->where($cond_where);
      $arr_ids = $this->dbback_city->get($this->_tbl)->result_array();
    }
