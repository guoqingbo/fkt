 ...
 * @author      Lion
 */
class sell_guest extends MY_Controller
{
  ...
  public function lists($page = 1)
  {
    ...
    //加载发布页面模板
    $this->view('guest/sell_guest', $data);
  ...
  public function details($house_id)
  {
    ...
    if ($house_id <= 0) {
      //$this->jump(MLS_URL.'/sell/lists/', '没有发现您查询的记录');
      //return;
    }
    ...
    $picinfo = $this->pic_model->find_house_pic_by('sell_house', $house_id);
...
//            $house_info['huxing'] = str_replace('/thumb/','/',$huxingpic[0]['url']);
    ...
    //加载页面
    $this->view('guest/sell_guest_info', $data);
  ...
  public function del($del_id = 0)
  {
    ...
    if ($str) {
      ...
      $this->sell_house_model->update_info_by_cond($arr, $cond_where);
    }
    if ($isajax) {
      echo json_encode(array('result' => 'ok'));
    } else {
      $this->jump(MLS_URL . '/sell/lists/', '删除成功');
    }
  }
  ...
  public function match($house_id)
  {
    ...
    //加载发布页面模板
    $this->view('guest/sell_guest_match', $data);
  }
