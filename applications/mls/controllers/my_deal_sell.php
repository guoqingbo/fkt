 ...
 * @author      sun
 */
class My_deal_sell extends MY_Controller
{
  /**
   * 录入经纪人id
   *
  ...
  public function index()
  {
    ...
    //查询房源条件
    $cond_where = "type = 1 AND broker_id = {$broker_id} ";
  ...
  public function house_list()
  {
    ...
    $house_deal_list = $this->house_deal_model->get_all_by("type = 1 AND broker_id = {$broker_id} ", -1);
  ...
  public function add($house_id = 0)
  {
    ...
    if ('add' == $submit_flag) {
      ...
      if ($id) {
        $this->jump(MLS_URL . '/my_deal_sell/details/' . $id . '/1', '添加成功');
        exit;
      } else {
        $this->jump(MLS_URL . '/my_deal_sell/add/' . $house_id, '添加失败');
        exit;
      }
    }
