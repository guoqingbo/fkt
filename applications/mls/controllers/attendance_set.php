 ...
 * @author      Lion
 */
class Attendance_set extends MY_Controller
{
  ...
  public function index()
  {
    ...
    if ($submit_flag == 1) {
      ...
      if ($id) {
        //修改
        $rs = $this->attendance_set_model->update_by_id($datainfo, $id);
      } else {
        $datainfo['company_id'] = $company_id;
        //添加
        $rs = $this->attendance_set_model->add_info($datainfo);
      }
      if ($rs) {
        $this->jump(MLS_URL . '/attendance_set', '保存设置成功');
        exit;
      } else {
        $this->jump(MLS_URL . '/attendance_set', '保存设置失败');
        exit;
      }
    }
    $data = array();
    ...
    $info = $this->attendance_set_model->get_one_by("company_id = {$company_id}");
    $data["info"] = $info;
    ...
    $data['css'] = load_css('mls/css/v1.0/base.css,mls/third/iconfont/iconfont.css,'
      . 'mls/css/v1.0/attendance_set_manage.css,'
    ...
    $this->view("attendance/set", $data);
  }


}

/* End of file attendance_set.php */
/* Location: ./application/mls/controllers/attendance_set.php */
