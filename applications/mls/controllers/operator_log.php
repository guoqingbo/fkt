...
class Operator_log extends MY_Controller
{
  ...
  public function index()
  {
    ...
    if ($func_area == 1) {
      ...
      //权限条件
      $cond_where .= "`operator_log`.broker_id = {$broker_id}";
    } else if ($func_area == 2) {
      $agency_id = $this->user_arr['agency_id'];
      //权限条件
      $cond_where .= "`operator_log`.agency_id = {$agency_id}";
      ...
      if (is_full_array($agency_ids)) {
        ...
        //权限条件
        $cond_where .= "`operator_log`.agency_id in({$agency_ids_str})";
      }
    }
    ...
    if ($agency_id) {
      $cond_where .= " AND `operator_log`.agency_id = {$agency_id}";
    ...
    //条件-员工
    $broker_id = isset($post_param['broker_id']) ? intval($post_param['broker_id']) : 0;
    if ($broker_id) {
      $cond_where .= " AND `operator_log`.broker_id = {$broker_id}";
    }

    //符合条件的总行数
    $this->_total_count =
      $this->operator_log_model->count_by($cond_where);
    ...
    //获取列表内容
    $list = $this->operator_log_model->get_list_by($cond_where, $this->_offset, $this->_limit);
    ...
    $this->view('agency/office/operator_log', $data);
  }
  ...
  public function details($id)
  {
    ...
    //详情信息
    $data_info = $this->operator_log_model->get_by_id($id);
    if ($isajax) {
  ...
  public function del($del_id = 0)
  {
    ...
    if ($str) {
      $ids = explode(',', $str);
      $this->operator_log_model->del_by_id($ids);
    }
    if ($isajax) {
      echo json_encode(array('result' => 'ok'));
    } else {
      $this->jump(MLS_URL . '/operator_log/', '删除成功');
    }
  }
