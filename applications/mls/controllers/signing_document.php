 ...
 * @author
 */
class Signing_document extends MY_Controller
{
    /**
     * 城市参数
     *
...
//    $this->load->model('company_employee_model');
    ...
    public function index($page = 1)
    {
        ...
        //关键字类型
        if (!empty($post_param['document_keyword_type'])) {
            $data['where_cond']['document_keyword_type'] = $post_param['document_keyword_type'];
            //关键字
...
//        }
        ...
        if (isset($data['where_cond']['document_keyword_type']) && !empty($data['where_cond']['document_keyword_type']) && isset($data['where_cond']['keyword']) && !empty($data['where_cond']['keyword'])) {

            if ($data['where_cond']['document_keyword_type'] == 1) {//文件名
                $cond_where .= " AND attachment_name like '%" . $data['where_cond']['keyword'] . "%'";
            }
            if ($data['where_cond']['document_keyword_type'] == 2) {//发布人
                $cond_where .= " AND signatory_name like '%" . $data['where_cond']['keyword'] . "%'";
            }
        }
        //符合条件的总行数
        $this->_total_count = $this->signing_document_model->get_count_by_cond($cond_where);
        $pages = $this->_total_count > 0 ? ceil($this->_total_count / $this->_limit) : 0;
        //获取列表内容
        $list = $this->signing_document_model->get_signing_document_by($cond_where, $this->_offset, $this->_limit);
        $data['list'] = $list;
        //print_r($list);exit;
        ...
        $this->view('signing_document/signing_document.php', $data);
    }
    ...
    public function notice_modify($id)
    {
        ...
        //$id>0,表示修改
        if ($id > 0) {
            $data['notice_detail'] = $this->signing_document_model->get_detail_by_id($id);
            $data['id'] = $id;
//            $this->upload_attachment($id);
        ...
        $data['user_menu'] = $this->user_menu = $this->purview_tab_model->get_tab('signing_document', 'index');
        $data['user_func_menu'] = $this->user_func_menu;
        $data['config'] = $this->bargain_config_model->get_config();
        //获取所有区域
        $district = $this->district_model->get_district("is_show = 1");
...
//      $district_agency=$this->agency_model->get_all_by_district("status = 1 and company_id > 0",$district_id_str);
        ...
        $this->view('signing_document/signing_document_modify.php', $data);
    }
    ...
    public function detail()
    {
        ...
        $detail = $this->signing_document_model->get_detail_by_id($id);
...
//    $company_id = intval($broker_info['company_id']);
        ...
        if ($get_param['id'] > 0) {
            $result = $this->signing_document_model->update_notice_broker($get_param['id'], $data);
        } else {
            $data['createtime'] = time();
            $result = $this->signing_document_model->add_notice($data);
        }
    ...
    public function del()
    {
        ...
        $detail = $this->signing_document_model->get_detail_by_id($ids);
        $path_info = pathinfo($detail["attachment"]);
        $del_res = unlink($config['upload_path'] . DIRECTORY_SEPARATOR . $path_info["basename"]); //删除文件
        if ($del_res) {


            $result = 0;
            if (is_numeric($ids)) {
                $insert_id = $this->signing_document_model->company_notice_del('id = ' . $ids . '');
                if ($insert_id) {
                    $result++;
                }
            } else {
                foreach ($ids as $vo) {
                    $insert_id = $this->signing_document_model->company_notice_del('id = ' . $vo . '');
                    if ($insert_id) {
                        $result++;
                    }
                }
...
//        $path_info=pathinfo($detail["attachment"]);
        $config['upload_path'] = str_replace("\\", "/", UPLOADS . DIRECTORY_SEPARATOR . 'file');
        $file = $config['upload_path'] . DIRECTORY_SEPARATOR . basename($detail["attachment"]);
        ...
        if (file_exists($file)) {
            ...
            Header("Content-Disposition: attachment; filename=" . $detail["attachment_name"]);
    ...
    //上传附件
    public function upload_document($id = 0)
    {
        //$detail = $this->signing_document_model->get_detail_by_id($id);
        ...
        if ($this->upload->do_upload('upload_document')) {
            ...
            if ($id <= 0) {
                $data = array(
                    ...
                    'department_name' => $this->user_arr['department_name'],
            ...
            //unlink($config['upload_path'] . DIRECTORY_SEPARATOR . $detail["attachment"]); //删除文件
            // $result = $this->signing_document_model->update_notice_broker($id, $data);
            //     unlink($data['upload_data']['full_path']); //删除文件
        } else {
            $res['result'] = '';
        }
        $res = json_encode($res);
        echo "<script>window.parent.upload_res_pop('" . $res . "')</script>";
    }
