...
class Contract extends MY_Controller
{
  ...
  private function _get_cond_str($form_param)
  {
    ...
    if ($keyword) {
      if ($keyword_type == 1) {
        //业主姓名
        $cond_where .= " AND `contract`.owner = '" . $keyword . "'";
      } elseif ($keyword_type == 2) {
        //客户姓名
        $cond_where .= " AND `contract`.customer = '" . $keyword . "'";
      }
    }
    //合同id
    ...
    if (isset($form_param['starttime']) && $form_param['starttime']) {
      $cond_where .= " AND `contract`.createtime >= '" . $form_param['starttime'] . "'";
    }

    if (isset($form_param['endtime']) && !empty($form_param['endtime'])) {
      $cond_where .= " AND `contract`.createtime <= '" . $form_param['endtime'] . "'";
    }

    //楼盘
    if (isset($form_param['block_id']) && !empty($form_param['block_id'])) {
      $cond_where .= " AND `contract`.block_id = '" . $form_param['block_id'] . "'";
    }

    //转正状态
    if (isset($form_param['status']) && !empty($form_param['status'])) {
      $cond_where .= " AND `contract`.status = '" . $form_param['status'] . "'";
    }
    ...
    if ($is_check) {
      if (in_array($is_check, array(1, 2, 3, 4))) {
        $cond_where .= " AND `contract`.is_check = '" . $is_check . "'";
      } elseif ($is_check == 6) {
        $cond_where .= " AND `contract`.is_completed = '1'";
      } elseif ($is_check == 5) {
        $cond_where .= " AND `contract`.is_commission = '1'";
      }
    }
    ...
    if (isset($form_param['owner_type']) && $form_param['owner_type']) {
      if ($form_param['owner_type'] == 1) {
        if (isset($form_param['owner_name']) && $form_param['owner_name']) {
          $cond_where .= " AND `contract`.owner like '%" . $form_param['owner_name'] . "%'";
        }
      } else {
        if (isset($form_param['owner_name']) && $form_param['owner_name']) {
          $cond_where .= " AND `contract`.customer like '%" . $form_param['owner_name'] . "%'";
        }
      }
    }
    ...
    if (isset($form_param['agency_id_a']) && $form_param['agency_id_a']) {
      $cond_where .= " AND `contract`.agency_id_a = '" . $form_param['agency_id_a'] . "'";
    }

    //签约人
    if (isset($form_param['broker_id_a']) && $form_param['broker_id_a']) {
      $cond_where .= " AND `contract`.broker_id_a = '" . $form_param['broker_id_a'] . "'";
    }
    ...
    if ($date_type == 3) {
      //结盘日期
      if ($start_time) {
        $cond_where .= " AND `contract`.completed_time >= '" . $start_time . "'";
      }
      if ($end_time) {
        $cond_where .= " AND `contract`.completed_time <= '" . $end_time . "'";
      }
    } elseif ($date_type == 2) {
      //结佣日期
      if ($start_time) {
        $cond_where .= " AND `contract`.commission_time >= '" . $start_time . "'";
      }
      if ($end_time) {
        $cond_where .= " AND `contract`.commission_time <= '" . $end_time . "'";
      }
    } else {
      //签约日期
      if ($start_time) {
        $cond_where .= " AND `contract`.signing_time >= '" . $start_time . "'";
      }
      if ($end_time) {
        $cond_where .= " AND `contract`.signing_time <= '" . $end_time . "'";
      }
    }
    //录入门店
    if (isset($form_param['enter_company_id']) && $form_param['enter_company_id']) {
      $cond_where .= " AND `contract`.enter_company_id = '" . $form_param['enter_company_id'] . "'";
    }

    //录入门店
    if (isset($form_param['enter_agency_id']) && $form_param['enter_agency_id']) {
      $cond_where .= " AND `contract`.enter_agency_id = '" . $form_param['enter_agency_id'] . "'";
    }

    //录入人
    if (isset($form_param['enter_broker_id']) && $form_param['enter_broker_id']) {
      $cond_where .= " AND `contract`.enter_broker_id = '" . $form_param['enter_broker_id'] . "'";
    }
  ...
  public function report($type = 1)
  {
    ...
    $report_delete_per = $this->broker_permission_model->check('112');
    $report_report_per = $this->broker_permission_model->check('113');
    $data['auth'] = array(
  ...
  public function contract_list($type = 1)
  {
    ...
    foreach ($list as $key => $val) {
      ...
      if (mb_strlen($val['house_addr'], 'UTF8') > 15) {
        $list[$key]['house_addr'] = mb_substr($val['house_addr'], 0, 14, 'utf-8') . '...';
      }
      ...
      if ($post_param['id']) {
        ...
        if ($data['result']) {
          ...
          $add_log_param = array(
            ...
            'type' => 35,
            'text' => '修改合同编号为' . $data_info['number'] . '的合同报备',
            'from_system' => 1,
            'from_ip' => get_ip(),
            'mac_ip' => '127.0.0.1',
        ...
        if ($data['result']) {
          ...
          $add_log_param = array(
            ...
            'broker_name' => $this->user_arr['truename'],
            'type' => 35,
            'text' => '新增合同编号为' . $data_info['number'] . '的合同报备',
            'from_system' => 1,
            'from_ip' => get_ip(),
            'mac_ip' => '127.0.0.1',
            'from_host_name' => '127.0.0.1',
  ...
  {
    $id = $this->input->get('id');
    $rs = $this->contract_model->update_by_id(array('status' => 2, 'is_check' => 1), $id);
    if ($rs) {
      //合同跟进——删除合同
      $data = array(
        'c_id' => $id,
        'type_name' => "合同录入",
        'content' => "本日对该合同信息进行转正。",
        'broker_id' => $this->user_arr['broker_id'],
        'broker_name' => $this->user_arr['truename'],
        'updatetime' => time()
      );
  ...
  public function exportReport($type)
  {
    ...
    $objPHPExcel->getActiveSheet()->setTitle('合同报备列表');
