...
class Cooperate_friends extends MY_Controller
{
  ...
  private function _friend_lists()
  {
    //处理个人信息
    ...
    $broker_info['company_name'] = $company['name'];
    $data['broker_info'] = $broker_info;
    //获取表单提交数据
    $post_param = $this->input->post(NULL, TRUE);
    $data['post_param'] = $post_param;
    //获取朋友列表
    $select_fields = array('DISTINCT(cooperate_friends.broker_id_friend)', 'cooperate_friends.id', 'cooperate_friends.createtime', 'broker_info.truename', 'broker_info.agency_id', 'broker_info.company_id', 'broker_info.photo');
    ...
    $friend_num = $this->cooperate_friends_base_model->get_friends_num_by_cond($cond_where);
  ...
  public function get_friend_num()
  {
    ...
    $friend_num = $this->cooperate_friends_base_model->get_friends_num_by_cond($cond_where);
    $result = array('status' => 1, 'msg' => '获取成功！', 'friend_num' => $friend_num, 'search_name' => $post_param['search_name']);
    echo json_encode($result);
  }

  private function _add_friend_lists()
  {
    //处理个人信息
    $broker_info = $this->user_arr;
    $broker_id = $broker_info['broker_id'];
    ...
    if ($post_param['search_name'] && $post_param['search_name'] != '查找经纪人') {
      ...
      foreach ($broker_all_info as $key => $vo) {
        ...
        if ($vo['broker_id'] != $broker_id) {
          $broker_all_info_new[$key] = $broker_all_info[$key];
        }
      }
    } else {
      $broker_all_info_new = array();
    ...
    $data['broker_all_info'] = $broker_all_info_new;

    $data['page_title'] = '添加朋友';
    //需要加载的css
    $data['css'] = load_css('mls/css/v1.0/base.css,mls/third/iconfont/iconfont.css');

    $this->view('my_friends/add_friends', $data);
  }
  ...
  public function add_apply()
  {
    $broker_info = $this->user_arr;
    $broker_id = $broker_info['broker_id'];
    $broker_id_friend = $this->input->post('broker_id_friend', TRUE);
    $add_arr = array(
    ...
    if (is_full_array($apply_info)) {
      $result = array('status' => 0, 'msg' => '重复申请！');
    } else {
      ...
      if ($insert_id > 0) {
        ...
        //$this->push_func_model->send(1, 15, 1, $broker_id, $broker_id_friend, array('msg_id' => $msg_id),array('broker_name' => $params['name']));//暂时屏蔽
      } else {
  ...
  public function sure_apply($apply_id)
  {
    ...
    if ($apply_info['status'] == 1) {
      ...
      if ($update_result) {
        ...
        //$this->push_func_model->send(1, 15, 2, $apply_info['broker_id_receive'], $apply_info['broker_id_send'], array('msg_id' => $msg_id),array('broker_name' => $params['name']));//暂时屏蔽
      }
  ...
  //好友拒绝申请
  public function refuse_apply($apply_id)
  {
    //申请详情
    $apply_info = $this->cooperate_friends_base_model->get_apply_by_id($apply_id);
    if ($apply_info['status'] == 1) {
      ...
      if ($update_result) {
        ...
        //$this->push_func_model->send(1, 15, 3, $apply_info['broker_id_receive'], $apply_info['broker_id_send'], array('msg_id' => $msg_id),array('broker_name' => $params['name']));//暂时屏蔽
      } else {
  ...
  //删除好友
  public function del_friend($broker_id_friend)
  {
    $broker_info = $this->user_arr;
    $broker_id = $broker_info['broker_id'];
    //好友详情双方
