...
function chang(type){
 ...
 $.ajax({
    ...
    <div class="fg_box">
            ...
            <div class="fg"> <a href="/cooperation/lists/" class="reset">重置</a> </div>
        </div>
</div>
        ...
        <table class="table">
            <tr>
                ...
                <td class="c5"><div class="info"><a href="javascript:void(0)" onclick="selllist_order(5);return false;" id="order_area" class="i_text <?php if($post_param['orderby_id'] == 6 ){ echo 'i_down'; }elseif($post_param['orderby_id'] == 5){ echo 'i_up'; } ?>">面积<br>
                        (㎡)</a></div></td>
                <td class="c5"><div class="info"><a href="javascript:void(0)" onclick="selllist_order(7);return false;" id="order_price" class="i_text ">报价<br>(W)</a></div></td>
                <td class="c7"><div class="info">委托门店</div></td>
                <td class="c7"><div class="info">委托经纪人</div></td>
    ...
    <div class="inner shop_inner" id="js_inner">
        <table class="table table_q">
			...
			<tr id="<?php echo $val['id'];?>">
				...
				<td class="c7"><div class="info"><?php echo $val['telno'];?></div></td>
				<td >
				...
				<?php }else{?>
					<div class="info"><div class="info">
                            <?php
                                if('0'==$val['reward_type']){
                            ?>
                            <a href="javascript:void(0);" onclick="is_share('<?=$val['id']?>',1)" class="fun_link">通过</a>
                            <?php
                                }else if('1'==$val['reward_type']){
                            ?>
                                    <a href="javascript:void(0);" onclick="is_share('<?= $val['id'] ?>',1)"
                                       class="fun_link">通过</a>
                            <?php
                                }else if('2'==$val['reward_type']){
                            ?>
                                    <a href="javascript:void(0);" onclick="is_share('<?= $val['id'] ?>',1)"
                                         class="fun_link">通过</a>
                                <?php
...
<div id="js_fun_btn" class="fun_btn clearfix">
	...
	<input type="checkbox" id="js_checkbox" style="float:left; margin:3px 10px 0 0;" onclick="getIds()">
...
<div class="pop_box_g pop_see_inform pop_no_q_up" id="js_pop_msg1">
    <div class="hd">
        <div class="title">提示</div>
        <div class="close_pop">
            <a class="JS_Close iconfont msg_iconfont_close" title="关闭" href="<?php echo MLS_URL;?>/cooperation/index"></a>
        </div>
    </div>
    <div class="mod">
        <div class="inform_inner">
...
function all_share(type){
    ...
    $("#dialog_share").click(function(){
        ...
        $.ajax({
            url: "<?php echo MLS_URL;?>/cooperation/change_all_share/",
            type: "GET",
            dataType: "json",
...
function is_share(house_id,type){
    ...
    $("#dialog_share").click(function(){
        $.ajax({
            url: "<?php echo MLS_URL;?>/cooperation/change_is_share/",
            type: "GET",
            dataType: "json",
            data: {
                house_id:house_id,
