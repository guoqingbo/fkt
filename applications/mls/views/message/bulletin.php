...
<div class="table_all">
    ...
    <div class="inner shop_inner" id="js_inner">
        <table class="table">
			...
			//print_r($list);
			foreach($list as $vo){?>
				<tr <?=($vo['is_read']==1)?"class=''":"class='fw'"?> onClick="detail_pop(<?php echo $vo['msg_id'];?>)" id="tr<?php echo $vo['msg_id'];?>">
					<td class="c3"><div class="info"><input type="checkbox" class="checkbox" name="items" value="<?=$vo['id']?>"></div></td>
					<td class="c3"><div class="info"><img id="img<?php echo $vo['msg_id'];?>" src="<?php echo MLS_SOURCE_URL;?>/mls/images/v1.0/<?=($vo['is_read']==0)?"notice1.png":"notice2.png"?>"></div></td>
					<td class="c5"><div class="info">
					<?php echo $type[$vo['type']];?>
					</div></td>
					<td class="c20"><div class="info"><?=$vo['title']?></div></td>
					<td class="c40"><div class="info"><?=$vo['message']?></div></td>
					<td><div class="info" style="color:#999; font-weight:normal;"><?=($vo['from']==0)?"科地地产":"其他平台"?>&nbsp; <?=date("Y-m-d H:i:s",$vo['createtime'])?></div></td>
				</tr>
			<?php }}else{ ?>
...
function detail_pop(id){
	$.ajax({
		...
		success: function(data){
			...
			$("#d_ctime").html("");
			$('#d_message').html("");
			$("#js_see_msg_info .m_bd").html("<button class='btn-lv1 btn-mid JS_Close' type='button'>确定</button>");
			...
			if(data['apply_id']){
				if(data['status'] == 0){
					var info = "<button class='btn-lv1 btn-left JS_Close' type='button' onclick='apply(1,"+data['apply_id']+")'>接受</button><button type='button' class='btn-hui1 JS_Close' onclick='apply(2,"+data['apply_id']+")'>拒绝</button>"
					$("#js_see_msg_info .m_bd").html(info);
				}
			}
...
function apply(type,apply_id){
	var url;
	if(type == 1){
		url = "/cooperate_friends/sure_apply/"+apply_id;
	}else{
		url = "/cooperate_friends/refuse_apply/"+apply_id
	}
	$.ajax({
		type: "post",
		url: url,
		...
		success: function(data){
			if(data['status'] == 1){
				...
				if(type==1){
					window.parent.window.refreshOnTime();
				}
