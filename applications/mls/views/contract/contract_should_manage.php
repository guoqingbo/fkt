<body>
	<!--应收应付开始-->
		...
		<tbody>
			...
			<td width="10%">
			    <?php if($val['status'] ==0){?>
			    <a href="javascript:void(0);" <?php if($auth['should_edit']['auth']){?>onclick="flow_should_detail(<?=$val['id'];?>,<?=$contract['id'];?>);"<?php }else{?>onclick="window.parent.window.permission_none();"<?php }?>>修改</a>&nbsp;&nbsp;|&nbsp;&nbsp;
			    <a href="javascript:void(0);" <?php if($auth['should_delete']['auth']){?>onclick="open_should_delete(<?=$val['id'];?>)"<?php }else{?>onclick="window.parent.window.permission_none();"<?php }?>>删除</a>
			    <?php }else{?>
...
<script>
    ...
    function view_detail(id){
        window.parent.document.getElementById('should').src = '/contract/contract_should_detail/'+id;
        window.parent.window.openWin('js_should_pop');
    }
    
    function flow_should_detail(id,c_id){
        window.parent.document.getElementById('should').src = '/contract/contract_should_modify/'+c_id+"/"+id;
        window.parent.window.openWin('js_should_pop');
    }
    //打开收付删除弹窗
