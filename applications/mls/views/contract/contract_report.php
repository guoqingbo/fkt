    ...
    <div class="forms_scroll2">
	    <div class="shop_tab_title scr_clear" id="js_search_box">
            <a href="javascript:void(0);" class="btn-lv fr"
               <?php if ($auth['add']['auth']){ ?>onclick="$('#js_modify_box .iframePop').attr('src','/contract/modify_report_index/<?= $type ?>');openWin('js_modify_box');"
               <?php }else{ ?>onclick="permission_none();"<?php } ?>><span>+ 新增合同报备</span></a>
            <a href="/contract/report/1" class="link <?= $type == 1 ? 'link_on' : '' ?>"><span
                        class="iconfont hide"></span>出售</a>
            <a href="/contract/report/2" class="link <?= $type == 2 ? 'link_on' : '' ?>"><span
                        class="iconfont hide"></span>出租</a>
	    </div>
	    <!-- 上部菜单选项，按钮-->
      <div class="fl">
		...
		<form name="search_form" id="subform" method="post" action="">
		    ...
		    <div class="fg_box">
                ...
                <div class="fg">
                    <input type="text" class="fg-time" name="start_time" value="<?= $post_param['start_time']; ?>"
                           autocomplete="off" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'})" onchange="check_num();">
                </div>
                <div class="fg fg_tex03">—</div>
                <div class="fg fg_tex03">
                    <input type="text" class="fg-time" name="end_time" value="<?= $post_param['end_time']; ?>"
                           autocomplete="off" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'})" onchange="check_num();">
			...
			<div class="fg"> <a href="javascript:void(0);" onclick="$('#subform :input[name=page]').val('1');$('#subform').attr('action', '/contract/report/<?=$type?>/');form_submit();return false;" class="btn"><span class="btn_inner">搜索</span></a> </div>
                <div class="fg"><a href="javascript:void(0);"
                                   onclick="$('#subform').attr('action', '/contract/exportReport/<?= $type ?>/');form_submit();$('#subform').attr('action', '');return false;"
...
$(function(){
	document.onkeydown = function(e){ //enter
		...
		<div class="title shop_title" id="js_title">
		    <table class="table">
			<tr>
                <td class="c9" style="width:10%;">合同编号</td>
                <td class="c9" style="width:10%;">房源编号</td>
			    <td class="c20">房源地址</td>
                <td class="c8" style="width:10%;">签约日</td>
			...
			<tr onclick="edit_report(<?=$val['id']?>);">
                ...
                <td class="c9" style="width:10%;">
                    <div class="info c227ac6" onclick=""><a href="javascript:void(0);"
                                                            onclick="$('#js_pop_box .iframePop').attr('src','/<?php echo $type == 1 ? 'sell' : 'rent'; ?>/details_house/<?= substr($val['house_id'], 2); ?>/4');openWin('js_pop_box');"><?= $val['house_id']; ?></a>
                    </div>
                </td>
			    <td class="c20"><div class="info"><?=$val['house_addr'];?></div></td>
                <td class="c8" style="width:10%;">
                    <div class="info"><?= date('Y-m-d', $val['signing_time']); ?></div>
                </td>
			    <td class="c15"><div class="info"><?=$val['agency_name_a'];?></div></td>
			    <td class="c8"><div class="info"><?=$val['broker_name_a'];?></div></td>
                <td class="c8">
                    <div class="info <?= $val['status'] == 2 ? "c680" : 'f00' ?>"><?= $config['report_status'][$val['status']]; ?></div>
                </td>
			    <td>
                    <?php if ($val['is_check'] == '0') { ?>
                        <a href="javascript:void(0)"
                           <?php if ($auth['report']['auth']){ ?>onclick="$('#contract_id').val(<?= $val['id']; ?>);openWin('js_pop_report');"
                           <?php }else{ ?>onclick="permission_none();"<?php } ?>>转正</a>
				<?php }else{?>
                        <span style="color:#b2b2b2;">转正</span>
				<?php }?>
				<span style="margin:0 5px;color:#b2b2b2;">|</span>
                    <?php if ($val['is_check'] == '0') { ?>
                        <a href="javascript:void(0)"
                           <?php if ($auth['edit']['auth']){ ?>onclick="$('#js_modify_box .iframePop').attr('src','/contract/modify_report_index/<?= $type ?>/<?= $val['id']; ?>');openWin('js_modify_box');"
                           <?php }else{ ?>onclick="permission_none();"<?php } ?>>编辑</a>
                    <?php } else { ?>
                        <span style="color:#b2b2b2;">编辑</span>
                    <?php } ?>
                    <span style="margin:0 5px;color:#b2b2b2;">|</span>
                    <a href="javascript:void(0)"
                       <?php if ($auth['delete']['auth']){ ?>onclick="$('#contract_id').val(<?= $val['id']; ?>);openWin('js_pop_del');"
                       <?php }else{ ?>onclick="permission_none();"<?php } ?>>删除</a>
...
<!--合同报备详情弹窗-->
<div class="pop_box_g" id="js_pop_detail" style="width:580px; height:352px; display: none;overflow:hidden;">
    <div class="hd header">
        <div class="title">合同报备详情</div>
	<div class="close_pop"><a href="javascript:void(0);" title="关闭" class="JS_Close iconfont">&#xe60c;</a></div>
    </div>
    <div class="reclaim-mod" style="height:316px;float:left;display:inline; overflow-y:auto; overflow-x:hidden;">
		...
		<tr>
            ...
            <td width="70" class="label"><b class="resut_table_state_1 zws_em ">*</b>合同编号：</td>
		...
		<script type="text/javascript">
            ...
            <table style="width:100%;">
                <tr>
                    <td width="70" class="label" style="width:70px;"><b class="resut_table_state_1 zws_em ">*</b>签约日期：
                    </td>
                    <td width="100">
                        <input type="text" style="width:80px;" size="14" class="input_text time_bg"
                               name="contract_time_edit" autocomplete="off"
                               onclick="WdatePicker({dateFmt:'yyyy-MM-dd'})" disabled>
	                    <div class="errorBox"></div>
                    </td>
                    <td width="70" class="label" style="width:70px;">
                        <b class="resut_table_state_1 zws_em ">*</b>签约人：
                    </td>
                    <td width="150">
	                    <span class="input_add_F" style="padding:0; width:110px;display:inline;">
							<select class="select mr10" name="contract_agency_edit" id="contract_agency_edit" disabled>
				                <?php foreach ($agencys as $key => $val) { ?>
                        ...
                        <span class="input_add_F" style="width:40px;display:inline;">
		...
		<script>
		    ...
		    <td colspan="4" class="center">
                <button type="button" class="btn-lv1 btn-left JS_Close zws_btn_bg_w93"
                        onclick="openWin('js_pop_report');">转为正式合同
                </button>
    ...
    <div class="hd">
     ...
     <div class="mod">
	    ...
	    <div class="up_inner">
			    ...
			    <img alt="" src="<?php echo MLS_SOURCE_URL;?>/mls/images/v1.0/dakacg.gif"></td>
                        <td>
                            <p class="left" style="font-size:14px;color:#666;" id="js_prompt2">合同报备添加成功！</p>
                        </td>
                    </tr>
                </table>
	    ...
	    <div class="up_inner">
                <table class="del_table_pop">
                    <tr>
                        ...
                        <td>
                            <p class="left" style="color:#666;">合同报备删除后不可修改,是否确认删除？</p>
                        </td>
                    </tr>
                </table>
...
<div class="pop_box_g pop_see_inform pop_no_q_up" style="display:none;" id="js_pop_report_sussess">
    ...
    <div class="mod">
	    ...
	    <div class="up_inner">
                ...
                <button class="btn-lv1 btn-left JS_Close" type="button" onclick="location.href='/contract/modify_contract/<?=$type;?>/'+$('#contract_id').val();return false;">去添加</button>
...
<!--添加和修改弹框-->
<div id="js_modify_box" class="iframePopBox" style="width: 580px;height:400px;">
    <a class="JS_Close close_pop iconfont" href="javascript:void(0)" date-iframe="1">&#xe60c;</a>
    <iframe frameborder="0" scrolling="no" width="580" height="400px" class='iframePop' src=""
...
<script>
    ...
    function delete_this() {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data['result'] == 1) {
                    $('#js_prompt1').text('合同报备已删除！');
                    openWin('js_pop_success');
                } else {
                    $('#js_prompt2').text('合同报备删除失败！');
    ...
    function update_status() {
        var contract_id = $('#contract_id').val();
        $.ajax({
            url: "/contract/update_report_status",
            type: "GET",
            dataType: "json",
            data: {
                id: contract_id
