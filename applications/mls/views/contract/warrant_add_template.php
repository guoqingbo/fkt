...
<div class="delate_btn1 moudle_finsh" style="width:842px;height:502px; border:none;">
    ...
    <div class="qz_moudle_add_step_stage"  style="height:460px;overflow-y:auto;position:relative;">
        ...
        <div class="warrant_process L25" style="padding-bottom:20px;">
                <ul>
                    <?php if($template['step']){foreach($template['step'] as $key => $val){?>
                    <li class="warrant_process_bg3"  style="padding-bottom:0;" title="<?=$val['stage_name1'];?>">
                    <p><?=$val['stage_name2'];?></p>
                    </li>
                    <li class="warrant_process_bg2"  style="padding-bottom:0;">
                        <img src="<?php echo MLS_SOURCE_URL;?>/mls/images/v1.0/qz_process5_10.gif" />
                    </li>
                    <?php }}?>
                    <li class="warrant_process_bg5"  style="padding-bottom:0;" onclick="$('#title').html('新建权证流程模板步骤');openWin('js_temp_box2');">
            ...
            <table align="center" style="float:left" class="qz_finse_step_table">
                ...
                <tbody class="ghbscq" align="center" id="template_table">
                    ...
                    <tr class="resut_table_border qz_porcee"  id="tr<?=$val['id'];?>">
                        ...
                        <td width="20%"><?=$val['agency_name'];?></td>
                        <td width="7%"><?=$val['broker_name'];?></td>
                        <td width="12%"><?=$val['createtime'];?></td>
                        <td width="14%">
                            <a href="javascript:void(0);" onclick="open_warrant_edit(<?=$val['id'];?>,<?=$key+1?>)">修改</a>|
                            <a href="javascript:void(0);" onclick="open_warrant_delete(<?=$val['id'];?>);">删除</a>
                        </td>
                    </tr>
                    <?php }}?>
                </tbody>
...
<div style="display:none" class="delate_btn1 qz_stepW qz_stepH" id="js_temp_box2" style="position:relative;">
   ...
   <div class="qz_moudle_add_step" style="height: 303px;overflow-y: auto;">
        ...
        <input type="hidden" id="template_id" value="<?=$id;?>">
         <div class="qz_moudle_con1 t"><a href="javascript:void(0)" onclick="save_warrant();">确定</a></div>
  </div>
</div>
...
<div id="js_del_warrant" class="pop_box_g pop_see_inform pop_no_q_up" >
	...
	<div class="inform_inner">
	    <div class="up_inner">
		<p class="text"><img alt="" src="<?php echo MLS_SOURCE_URL;?>/mls/images/v1.0/s_ico.png">&nbsp;&nbsp;模板步骤删除之后不影响使用该模板的合同权证流程步骤。是否确认删除？</p>
		<button type="button" class="btn-lv1 JS_Close" onclick="delete_warrant();">确定</button>
...
<!--删除-->
<div id="js_edit_warrant_pop" class="pop_box_g pop_see_inform pop_no_q_up" >
    <div class="hd">
	<div class="title">提示</div>
	<div class="close_pop"><a href="javascript:void(0);" title="关闭" class="JS_Close iconfont"></a></div>
    </div>
	...
	<div class="inform_inner">
	    <div class="up_inner">
		<p class="text"><img alt="" src="<?php echo MLS_SOURCE_URL;?>/mls/images/v1.0/s_ico.png">&nbsp;&nbsp;修改模板步骤之后不影响使用该模板的合同权证流程步骤。是否确认修改？</p>
		<button type="button" class="btn-lv1 JS_Close" onclick="warrant_detail()">确定</button>
		<button type="button" class="btn-hui1 JS_Close"  style="margin:0 0 0 10px;background:#fafafa;color:#000;border:1px soild #dcdcdc;border-radius:2px;">取消</button>
	    </div>
	</div>
    </div>
...
<div class="pop_box_g pop_see_inform pop_no_q_up" style="display:none;" id="js_pop_success">
    <div class="hd">
        <div class="title">提示</div>
        <div class="close_pop"><a href="javascript:void(0);" title="关闭" class="JS_Close iconfont" onclick="window.location.href='/contract/warrant_template_add/<?=$id?>/<?=$key1?>/<?=$c_id?>';return false;"></a></div>
    </div>
     <div class="mod">
    	<div class="inform_inner">
	    <div class="up_inner">
                ...
                <button class="btn-lv1" type="button" onclick="window.location.href='/contract/warrant_template_add/<?=$id?>/<?=$key1?>/<?=$c_id?>';return false;">确定</button>
            </div>
         </div>
    </div>
</div>
...
<script>
    ...
    function open_warrant_delete(id,key){
        $('#stage_id').val(id);
        $('#step_id').val(key);
        openWin('js_del_warrant');
    }

    function open_warrant_edit(id,key){
        $('#stage_id').val(id);
        $('#step_id').val(key);
        openWin('js_edit_warrant_pop');
    }

    function warrant_detail(){
        $('#title').html('修改权证流程模板步骤');
        $.ajax({
			type: 'post',
			url: '/contract/template_detail',
			data: {
				id:$('#stage_id').val(),
                key:$('#step_id').val()
			},
			...
			success: function(data){
                if(data['result'] == 1){
                    ...
                    for(var i in  data['warrant_list']['stage_id']){
                        $("input[value='"+data['warrant_list']['stage_id'][i]+"']").attr('checked',true);
                    }
	...
	function save_warrant(){
		...
		if(arr.length>0){
			...
			$.ajax({
				type: 'post',
				url: '/contract/save_template_step',
				data: {
    ...
    function delete_warrant(){
        $.ajax({
            url:"/contract/delete_template_step",
            type:"POST",
            dataType:"json",
            data:{
                stage_id:$('#stage_id').val(),
    ...
    function save(id,key,c_id){
        if(key ==1){
            $.ajax({
                url:"/contract/save_warrant_step",
                type:"POST",
                dataType:"json",
                data:{
                    template_id:id,
            ...
            $.ajax({
                url:"/contract/save_template_name",
                type:"POST",
                dataType:"json",
                data:{
                    template_id:$('#template_id').val(),
                ...
                success:function(data){
                    if(data['result'] == 'ok'){
                        window.parent.frames["template_pop"].location='/contract/warrant_template/'+id;
