...
<!--权证添加和修改弹窗-->
    <div class="achievement_money_pop qz_add_step_w610" id="js_temp_pop">
        ...
        <form action="" id="warrant_form" method="post">
        <div style="height: 400px;overflow-y: auto;overflow-X: hidden;width: 100%;">
             <div class="qz_moudle_add_step" style="width:96%;padding:0 2%;">
                  <dl class="qz_moudle_add_step_type">
                      <dd>权证流程模板：<b><?=$warrant_info['template_name'];?></b></dd>
                  ...
                  <dl class="qz_moudle_add_step_stage"  style="width:96%;padding:0 2%;">
                        ...
                        <?php if($stage){foreach($stage as $key=>$val){?>
                            ...
                            <?php }elseif($key%4 == 3){?>
                            <p class="step_checkboxW4"><input type="checkbox" name="step" value='<?=$val['id'];?>' <?php if($step['stage_id']){if(in_array($val['id'],$step['stage_id'])){echo 'checked';}}?>><?=$val['stage_name'];?></p>
                            <?php }?>
                        <?php }}?>
                  ...
                  <dl class="qz_moudle_add_step_stage"  style="width:96%;padding:0 2%;">
                        <dd>备注：</dd>
                        <dt>
                        <textarea class="qz_add_step_textarea" name="warrant_remark"><?=$step['remark'];?></textarea>
                ...
                <span style="width:86%;float:left;display:inline;">
                  ...
                  <dl class="qz_moudle_add_step_stage content_W100">
                        ...
                        <dt style="width:320px;float:left;display:inline;">
                            ...
                            <select class="step_select bottomP14" style="width:120px;float:left;display:inline;padding:0 0 0 5px;" name="remind_agency_id" <?=0==$step['is_remind']?'disabled':'';?>>
                                <?php foreach($agencys as $key =>$val){?>
                                <option value="<?=$val['id']?>" <?=$val['id']==$step['remind_agency_id']?'selected':'';?>><?=$val['name']?></option>
                                <?php }?>
                            </select>
                            <span class="errorBox zws_block" style="float:none;"></span>
                            </div>
                            <div  style="float: left;display: inline;">
                            <select class="step_select bottomP14" style="float:left;display:inline;" name="remind_broker_id" <?=0==$step['is_remind']?'disabled':'';?>>
                                <?php foreach($brokers as $key =>$val){?>
                                <option value="<?=$val['broker_id']?>" <?=$val['broker_id']==$step['remind_broker_id']?'selected':'';?>><?=$val['truename']?></option>
                                <?php }?>
                            </select>
                            </div>
                        </dt>
                    ...
                    $(function(){
                        ...
                        $("input[name='is_remind']").click(function(){
                            if($("input[name='is_remind']:checked").val() ==1){
                                $("select[name='remind_agency_id']").removeAttr('disabled');
                                $("select[name='remind_broker_id']").removeAttr('disabled');
                                ...
                                $("select[name='remind_agency_id']").attr('disabled',true);
                                $("select[name='remind_broker_id']").attr('disabled',true);
                                $("input[name='remind_time']").attr('disabled',true);
                                $("textarea[name='remind_remark']").attr('disabled',true);
                            }
                        })

                        $("select[name='remind_agency_id']").change(function(){
                        var agency_id = $("select[name='remind_agency_id']").val();
                        if(agency_id){
                            $.ajax({
                                ...
                                success:function(data){
                                    ...
                                    $("select[name='remind_broker_id']").html(html);
                                    }else{
                                    $("select[name='remind_broker_id']").html("<option value=''>请选择</option>");
                                    }
                                }
                            })
                        }else{
