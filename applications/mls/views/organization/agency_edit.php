...
function modify_agency(){
	...
	if(is_submit_t ==1 && is_submit_s == 1){
		...
		$.ajax({
			type : 'post',
			url  : '/organization/modify_agency_edit/',
			data : {telno:telno,service_area:service_area,agency_id:"<?php echo $agency_info['id']?>"},
			dataType :'json',
    ...
    <div class="mod bg-weight">
		<div class="zws_person_add">
				...
				<dt>
					<div class="my_info" style="margin-bottom: 0;width: 130px;">
						<div class="my_tx" style="margin-bottom:0;">
							<div class="tx_normal" style="background:url(<?php echo MLS_SOURCE_URL;?>/mls/images/v1.0/add_img_03.jpg) no-repeat center;">
								<form name='fileform_photo' id='fileform_photo' action='/organization/upload_photo_agency' enctype='multipart/form-data' target='filepost_iframe' method='post'>
								<input type="hidden" name="agency_id" id="agency_id" value="<?=$agency_info['id']?>">
									<?php if($agency_info['photo']) {?>
									<img class="myself_photo_a" id="photo_replace" src="<?=$agency_info['photo']?>" width="130" height="100"/>
			...
			<dl>
				<dd>联系电话</dd>
				<dt>
					<input type="text" value="<?php echo $agency_info['telno']?>" class="zws_person_add_input zws_p_select_w300" id='telno' onkeyup="check_num()">
			...
			<dl>
				<dd>服务范围</dd>
				<dt>
					<input type="text" value="<?php echo $agency_info['service_area']?>" class="zws_person_add_input zws_p_select_w300" id='service_area' onkeyup="check_num()">
		...
		<div class="center mt10">
			<button class="btn-lv1 btn-left" type="button" onclick="modify_agency();">确定</button>
...
function changePhoto(fileurl){
	var photo_addHtml = "<img class='myself_photo_a' id='photo_replace' src='"+fileurl+"' width='130' height='100'/>";
