<div class="contract-wrap clearfix">
	...
	<div class="forms_scroll2">
		<div class="shop_tab_title scr_clear" id="js_search_box">
			<a href="/collocation_contract/pay_owner_audit/1" class="link <?php if($tab == 1){?> link_on <?php }?>"><span class="iconfont hide"></span>应付业主</a>
			<a href="/collocation_contract/pay_owner_audit/2" class="link <?php if($tab == 2){?> link_on <?php }?>"><span class="iconfont hide"></span>实付业主</a>
		</div>
		<!-- 上部菜单选项，按钮-->
		<form name="search_form" id="search_form" method="post" action="">
			...
			$("#agency_id").change(function(){
				...
				<div class="fg_box">
					...
					<div class="fg"> <a href="/contract_audit/pay_owner_audit/<?=$tab?>" class="reset">重置</a> </div>
				</div>
			</div>
<script>
$(function(){
	document.onkeydown = function(e){ //enter
		...
		if(ev.keyCode==13) {
			...
			<div class="contract-crumbs clearfix" id="js_fun_btn2">
				<p>共有<strong class="ff7800"><?php if($tab == 1){echo $wait_audit_1;}else{echo $wait_audit_2;}?></strong>条待审核<?php if($tab == 1){?>应付业主<?php }else{?>实付业主<?php }?></p>
			</div>
			<div class="table_all">
				<div class="title shop_title" id="js_title">
					<table class="table">
						<tr>
							<td class="c6"><?php if($tab == 1){?>应付时间<?php }else{?>实付时间<?php }?></td>
							<td class="c6">托管合同编号</td>
							<td class="c5">租金</td>
							<td class="c5">水费</td>
							<td class="c5">电费</td>
							<td class="c5">燃气费</td>
				...
				<div class="inner shop_inner" id="js_inner">
					...
					?>
						<tr>
							<td class="c6"><div class="info"><?php if($tab == 1){echo date('Y-m-d',$val['need_pay_time']);}else{echo date('Y-m-d',$val['actual_pay_time']);}?></div></td>
							<td class="c6" onclick = "window.location.href='/collocation_contract/contract_detail/<?=$val['c_id']?>/<?=$tab?>';return false;"><div class="info c227ac6"><?=$val['collocation_id']?></div></td>
							<td class="c5"><div class="info"><?=strip_end_0($val['rental'])?></div></td>
							...
							<td class="c5"><div class="info"><?=$val['broker_name']?></div></td>
...
<script>
	...
	function pay_owner_audit(id,type,c_id){
		var tab = <?=$tab?>;
		if(type == 3){//反审核
			$("#dialogSaveDiv").html('<img src="<?php echo MLS_SOURCE_URL;?>/mls/images/v1.0/s_ico.png" id="imgg" alt=""> 反审核后当前信息将可修改，<br/>是否确认操作？');
		}else if(type == 1){//通过
			if(tab == 1){//应付
				$("#dialogSaveDiv").html('<img src="<?php echo MLS_SOURCE_URL;?>/mls/images/v1.0/s_ico.png" id="imgg" alt=""> 审核完成后应付业主信息将不可修改，<br/>是否确定当前操作？');
			}else{//实付
				$("#dialogSaveDiv").html('<img src="<?php echo MLS_SOURCE_URL;?>/mls/images/v1.0/s_ico.png" id="imgg" alt=""> 审核完成后实付业主信息将不可修改，<br/>是否确定当前操作？');
			}
		}else if(type == 2){//拒绝
