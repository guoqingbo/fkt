	...
	<form name="search_form" id="search_form" method="post" action="">
		...
		$("#agency_id").change(function(){
			...
			<div class="fg_box">
				...
				<div class="fg"> <a href="/contract_audit/collocation_audit/" class="reset">重置</a> </div>
			</div>
		</div>
	...
	document.onkeydown = function(e){ //enter
		...
		<div class="table_all">
			<div class="title shop_title" id="js_title" style="padding-right:0;">
				<table class="table">
					<tr>
						...
						<td class="c9">业主</td>
						<td class="c12">租金（元/月）</td>
						<td class="c9">出租时长（月）</td>
						<td class="c9">签约门店</td>
						<td class="c9">签约人</td>
						<td class="c9">状态</td>
						<td>操作</td>
			...
			<div class="inner shop_inner" id="js_inner">
				...
				?>
					<tr>
						<td class="c9" onclick = "window.location.href='/collocation_contract/contract_detail/<?=$val['id']?>/1';return false;"><div class="info c227ac6"><?=$val['collocation_id']?></div></td>
						<td class="c20"><div class="info"><?=$val['houses_address']?></div></td>
						<td class="c9"><div class="info"><?=$val['owner']?></div></td>
						<td class="c12"><div class="info"><?=strip_end_0($val['rental'])?></div></td>
						<td class="c9"><div class="info"><?=$val['total_month']?></div></td>
						...
						<?php if($val['status'] == 1){?>
							<?php if (isset($auth['audit']['auth']) && $auth['audit']['auth']) { ?>
								<a href="javascript:void(0)" onclick="collocation_audit('<?=$val['id']?>',1)">审核</a>
							<?php }else{?>
								<a href="javascript:void(0)" onclick="permission_none();">审核</a>
							<?php }?>
						<?php }elseif($val['status'] == 2){?>
							<?php if (isset($auth['turn_audit']['auth']) && $auth['turn_audit']['auth']) { ?>
								<a href="javascript:void(0)" onclick="collocation_audit('<?=$val['id']?>',2)">反审核</a>
							<?php }else{?>
...
<script>
	//审核
	function collocation_audit(id,type){
		if(type == 1){//审核
			...
			$('#dialog_share').click(function(){
				...
				if(audit_end){
					...
					$('#dialog_share2').click(function(){
						$.ajax({
							url: "<?php echo MLS_URL;?>/collocation_contract/audit_agreement/",
							type: "GET",
							dataType: "json",
							data: {
								id:id,
