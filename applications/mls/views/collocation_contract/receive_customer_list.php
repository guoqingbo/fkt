<div class="contract-wrap clearfix">
	...
	<div class="forms_scroll2">
		<div id="js_search_box" class="shop_tab_title  scr_clear m0">
			<a href="/collocation_contract/receive_customer_list/1" class="link <?php if($tag == 1){?> link_on <?php }?>">应收客户列表<span class="iconfont hide"></span></a>
			<a href="/collocation_contract/receive_customer_list/2" class="link <?php if($tag == 2){?> link_on <?php }?>">实收客户列表<span class="iconfont hide"></span></a>
		</div>
		<form name="search_form" id="search_form" method="post" action="">
			...
			<div class="search_box clearfix" style="margin-top:0;overflow: hidden" id="js_search_box_02">
				...
				<div class="fg_box">
					...
					<?php if($tag == 1){?>
						<select class="select w80" name='search_where'>
							<option value='need_receive_time' <?php if((!empty($post_param['search_where']) && $post_param['search_where'] == 'need_receive_time')){echo 'selected="selected"';}?>>应收时间</option>
						</select>
					<?php }else{?>
						<select class="select w80" name='search_where'>
							<option value='actual_receive_time' <?php if((!empty($post_param['search_where']) && $post_param['search_where'] == 'actual_receive_time')){echo 'selected="selected"';}?>>实收时间</option>
						</select>
					<?php }?>
					</p>
					<div class="fg">
						<input style="width:110px" type="text" name="time_s" class="fg-time" value="<?php if(isset($_POST['time_s'])) {echo $_POST['time_s'];}?>" onfocus="WdatePicker({lang:'zh-cn',dateFmt:'yyyy-MM-dd'})">
					</div>
					<div class="fg fg_tex03">—</div>
					<div class="fg fg_tex03">
						<input style="width:110px" type="text" name="time_e" class="fg-time" value="<?php if(isset($_POST['time_e'])) {echo $_POST['time_e'];}?>" onfocus="WdatePicker({lang:'zh-cn',dateFmt:'yyyy-MM-dd'})">
					</div>
				</div>
				...
				<div class="fg_box">
					...
					<div class="fg"><a href="/collocation_contract/customer_export/<?=$tag?>" class="btn"><span class="btn_inner">导出</span></a></div>
					<div class="fg"> <a href="/collocation_contract/receive_customer_list/<?=$tag?>" class="reset">重置</a> </div>
				</div>
			</div>
<script>
$(function(){
		...
		if(ev.keyCode==13) {
			...
			<div class="table_all">
				<div class="title shop_title" id="js_title">
					<table class="table">
						<tr>
							<td class="c6" style="width:8%;"><?php if($tag == 1){?>应收时间<?php }else{?>实收时间<?php }?></td>
							<td class="c6" style="width:8%;">托管合同编号</td>
							<td class="c6" style="width:8%;">出租合同编号</td>
						...
						<tr class="">
							<td class="c6" style="width:7%;">
								<div class="info">
									<?php
										if($val['type'] == '1'){
											echo date('Y-m-d',$val['need_receive_time']);
										}else{
											echo date('Y-m-d',$val['actual_receive_time']);
										}
									?>
								</div>
							</td>
							<td class="c6" style="width:7%;" ><div class="info "><?=$val['collocation_id']?></div></td>
							<td class="c6" style="width:7%;cursor:pointer;" onclick = "window.location.href='/collocation_contract/rent_contract_detail/<?=$val['r_id']?>/<?=$tag?>';return false;"><div class="info c227ac6"><?=$val['collo_rent_id']?></div></td>
							<td class="c6" style="width:7%;cursor:pointer;" onclick="$('#js_pop_box .iframePop').attr('src','/rent/details_house/<?=substr($val['house_id'],2);?>/4');openWin('js_pop_box');return false;"><div class="info c227ac6"><?=$val['house_id']?></div></td>
							...
							<td class="c6"><div class="info"><?=strip_end_0($val['other_fee'])?></div></td>
