<script src="<?php echo MLS_SOURCE_URL; ?>/min/?f=common/third/My97DatePicker/WdatePicker.js"
...
<div class="search_box clearfix" id="js_search_box_02">
    <form name="search_form" id="subform" method="post" action="">
        <div class="fg_box">
            ...
            <div class="fg">
                <input type="text" class="fg-time" id="start_date_begin" name="start_date_begin" onclick="WdatePicker()"
                       value="<?= $post_param['start_date_begin'] ?>">
            </div>
            <div class="fg fg_tex03">—</div>
            <div class="fg fg_tex03">
                <input type="text" class="fg-time" id="start_date_end" name="start_date_end" onclick="WdatePicker()"
                       value="<?= $post_param['start_date_end'] ?>">
                        ...
                        <?php if ($agency_list && $role_level != 6) { ?>
                            ...
                            foreach ($agency_list as $key => $val) {
                                ...
                                <option <?php if ($val['agency_id'] == $post_param['post_agency_id'] || ($val['agency_id'] == $agency_id && $post_param['post_broker_id'] == ''))
                                    echo "selected"; ?>
                                        value="<?php echo $val['agency_id']; ?>"><?php echo $val['agency_name']; ?></option>
                            <?php }
                        } else { ?>
        ...
        <div class="fg_box">
            ...
            <div class="fg">
                <a href="javascript:void(0)"
                   onclick="$('#subform :input[name=page]').val('1');form_submit();return false;" class="btn"><span
                            class="btn_inner">搜索</span></a></div>
            <div class="fg"><a href="/hidden_call/call_record/" class="reset">重置</a></div>
        </div>
    </form>
</div>
            ...
            if ($list) {
                foreach ($list as $key => $value) {
                    ...
                    <tr <?php if ($key % 2 != 0) {
                        ...
                        <td class="c10"><?= $value['agency_name'] ?></td>
                        <td class="c10"><?= $value['broker_name'] ?></td>
                        <td class="c10"><?= $value['owner_phone'] ?></td>
                        <td class="c10"><?= date('Y-m-d H:i:s', $value['create_time']) ?></td>
                        <td class="c10"><a href="<?= $value['record_url'] ?>" target="_blank">下载</a></td>
                    </tr>
                <?php }
            } else {
                ?>
...
<div id="js_find_daily_pop" class="iframePopBox" style="width:580px; height:510px;border-left:none;">
    ...
    <iframe frameborder="0" scrolling="auto" width="580px" height="510px" class='iframePop' src=""
