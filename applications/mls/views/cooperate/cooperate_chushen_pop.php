<body>
<form id="chushen_form" name = 'chushen_form' method="post" action =''>
	<div class="pop_see_inform pop_no_q_up pop_box_g" id="infomat" style="width:800px; height:500px;">
		<div class="hd">
			<div class="title">提交审核资料</div>
		</div>
        <!--		<div class="mod infomat" style="height:18px;">-->
        <!--		    <p style="color:blue;">恭喜你房源成交，提交审核资料且审核通过后就能获得<span style="color:red">500积分</span>，积分可以在积分商城兑换实物礼品哦</p>-->
        <!--		</div>-->
        <div class="mod infomat" style="height:18px;">
		...
		<div class="mod infomat">
			<div class="pane-out clearfix">
				<div class="pane pane2">合同编号：
					<select class="select" name="c_id" id="c_id">
					    <option value="<?=$cooperate_info['id'];?>"><?=$cooperate_info['order_sn'];?></option>
					</select>
					<div class="errorBox"></div>
				</div>
				<span>*若无合同编号，请确认是否已在合作中心确认成交，否则不能提交审核资料</span>
			</div>
			<div class="pane-out clearfix">
			...
			<div class="pane-out clearfix">
				<div class="pane"><?php echo $cooperate_info['tbl']=="sell"?"买方姓名：":"租客姓名："; ?><input type="text" value="" class="txt" name="buyer_owner" autocomplete="off"><div class="errorBox"></div></div>
			...
			<h3>请上传审核凭证（<font color='red'>1.房管局<?php echo $cooperate_info['tbl']=="sell"?"买卖":"租房"; ?>合同，2.受理单，3.税票清晰照片</font>）（缺一不可）：</h3>
			<div class="upload clearfix" onclick="$('#photo').css('display','none');">
