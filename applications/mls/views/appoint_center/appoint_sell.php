...
<div class="search_box clearfix"  id="js_search_box_02">
    ...
    <div class="fg_box">
        ...
        <div class="fg">
            <input type="text" name="phone" id="phone" value="" class="input w110">
        </div>
    </div>
    <div class="fg_box">
        <p class="fg fg_tex"> 房源编号：</p>
        <div class="fg">
            <input type="text" name="hid" id="hid" value="<?=isset($post_param['hid'])?$post_param['hid']:""?>" class="input w60">
        </div>
    </div>
    <div class="fg_box">
	...
	<div class="fg_box hide">
        ...
        <div class="fg">
			<input type="text" class="hr_framework_text hr_framework_timebg fl" onfocus="WdatePicker({lang:'zh-cn',dateFmt:'yyyy-MM-dd 00:00:00'})" id="stimemin" name="stimemin" value="<?=isset($post_param['stimemin'])?$post_param['stimemin']:""?>">

        </div>
        <p class="fg fg_tex fg_tex02">—</p>
        <div class="fg">
			<input type="text" class="hr_framework_text hr_framework_timebg fl" onfocus="WdatePicker({lang:'zh-cn',dateFmt:'yyyy-MM-dd 00:00:00'})" id="stimemax" name="stimemax" value="<?=isset($post_param['stimemax'])?$post_param['stimemax']:""?>">&nbsp;&nbsp;<span style="font-weight:bold;color:red;" id="stimemin_reminder"></span>
        </div>
    </div>
...
<div class="table_all" >
    <div class="title shop_title" id="js_title">
        <table class="table">
            <tr>
              	<td class="c7"><div class="info">房源编号</div></td>
                <td class="c7"><div class="info">区属</div></td>
                <td class="c7"><div class="info">板块</div></td>
                <td class="c12"><div class="info">楼盘</div></td>
                <td class="c7"><div class="info">户型</div></td>
                <td class="c7"><div class="info">面积(㎡)</div></td>
  				<td class="c7"><div class="info">总价(万)</div></td>
                <td class="c7"><div class="info">楼层</div></td>
                <td class="c8"><div class="info">会员昵称</div></td>
                <td class="c8"><div class="info">联系方式</div></td>
                <td class="c15"><div class="info">预约时间</div></td>
            </tr>
        </table>
    </div>
    <div class="inner shop_inner" id="js_inner">
        <table class="table">
			...
			if($list){
				foreach($list as $vo){?>
					<tr id="tr<?php echo $vo['id'];?>" date-url="/appoint_center/details_house/<?=$vo['app_id']?>/<?php echo $vo['id'];?>/1" controller="appoint_center" _id="<?php echo $vo['id'];?>">
						<td class="c3" style="display:none">
							<div class="info">
								<input type="checkbox" name="items" value="<?php echo $vo['id'];?>" class="checkbox" style="display:none;">
							</div>
						</td>
						<td class="c7"><div class="info c227ac6" id=""><?=$vo['hid']?></div></td>
						<td class="c7"><div class="info"><?=$district[$vo['district_id']]['district']?></div></td>
						<td class="c7"><div class="info"><?=$street[$vo['street_id']]['streetname']; ?></div></td>
						<td class="c12"><div class="info"><?=$vo['block_name']?></div></td>
						<td class="c7"><div class="info"><?=$vo['room'];?>-<?=$vo['hall'];?>-<?=$vo['toilet'];?></div></td>
						<td class="c7"><div class="info"><?=$vo['buildarea'];?></div></td>
						<td class="c7"><div class="info cf60"><strong class="f60"><?=$vo['price'];?></strong></div></td>
						<td class="c7"><div class="info"><?php echo $vo['floor']; ?><?php if($vo['floor_type']==2){ echo "-".$vo['subfloor'];}?>/<?php echo $vo['totalfloor']; ?></div></td>
						<td class="c8"><div class="info"><?=$vo['uname'];?></div></td>
						<td class="c8"><div class="info"><?=$vo['phone'];?></div></td>
						<td class="c15"><div class="info c1ab273"><?=date("Y-m-d H:i:s",$vo['stime'])?></div></td>
					</tr>
					<?php }}else{?>
...
<!--匹配页弹框-->
<div id="js_pop_box_g_match" class="iframePopBox" style=" width:1200px; height:504px; ">
    <a class="JS_Close close_pop iconfont" href="javascript:void(0)" date-iframe="1">&#xe60c;</a>
    <iframe frameborder="0" scrolling="no" width="1200" height="504" class='iframePop' src=""></iframe>
</div>
<!--页面处理中弹层-->
