...
$(function () {
    ...
    $('#container').highcharts({
        ...
        tooltip: {
            ...
            pointFormat: '<tr style="width:70px;"><td style="color:{series.color};padding:0;width:50%;float:left;display:inline;font-size:10px;text-align:center;white-space:nowrap; word-break:keep-all;">{series.name}: </td>' +
                '<td style="width:50%;float:left;display:inline;padding:0;font-size:10px;text-align:center;white-space:nowrap; word-break:keep-all;"><b>{point.y} 套</b></td></tr>',
            footerFormat: '</table>',
            shared: true,
            useHTML: true
        },
...
<div class="data_aly_content clearfix" style="padding-top:13px;background:#fff">
	<div class="main_menu fl" style="width:150px;border:1px solid rgb(230,230,230)">
		<div class="tab_box wh_hover " id="js_tab_box" style="background:#fff;border:none;height:94px;overflow:hidden">
			...
			<a href="/count_info/index/1" class="link link_on">房源统计</a>
	...
	<form name="search_form" id="search_form" method="post" action="" >
		<div class="top_bar clearfix">
			...
			<select class="sel_shop fl" name="config">
				<option value="1" <?php if($post_param['config'] == '1'){ echo 'selected="selected"';}?>>房源状态</option>
				<option value="2" <?php if($post_param['config'] == '2'){ echo 'selected="selected"';}?>>房源来源</option>
				<option value="3" <?php if($post_param['config'] == '3'){ echo 'selected="selected"';}?>>房源区属</option>
				<option value="4" <?php if($post_param['config'] == '4'){ echo 'selected="selected"';}?>>物业类型</option>
				<option value="5" <?php if($post_param['config'] == '5'){ echo 'selected="selected"';}?>>委托类型</option>
			</select>
            <?php
            if($agency_info) {
            ?>
			...
			<div class="top_bar_r fr">
				...
				<input type="button" class="re" value="重置" onclick="location.href='/count_info/index/1'">
			</div>
		</div>
		...
		<div class="middle_bar clearfix">
			<span class="title fl">房源状态统计</span>
			<a class="daochu fr" style="text-align:center; line-height:24px;" href="/count_info/house_export/<?=$post_param['type']?>/<?=$post_param['config']?>/<?=$post_param['agency_id']?>/<?=$post_param['broker_id']?>">导出</a>
		</div>
				...
				<tr>
					...
					switch($post_param['config']){
						...
						case 2:
							...
							break;
						case 3:
							echo $v['district_'.$key];
							break;
						case 4:
							echo $v['sell_type_'.$key];
							break;
						case 5:
							echo $v['entrust_'.$key];
							break;
