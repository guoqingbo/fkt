...
<div class="pop_box_g" id="js_pop_box_g"  style="display:block; border:none;width: 950px;">
    ...
    <div class="wrapper ywgj" style="margin: 0 auto;margin-top: 40px;position:relative;">
        <h1 style="margin-bottom:40px;"><?php echo $dong_name; ?>栋座的门牌单元</h1>
        <?php if(is_full_array($all_dong_unit_door)){ ?>
            <ul class="ld_ul">
                <li class="li_ld clearfix li_ld_new">
                    <div class="add_ld">
                        <input id="dong_name" type="text" value="<?php echo $dong_name; ?>" msgs="<?php echo $dong_name; ?>" onfocus="bindFocus(this,this.value,true)"
                               onblur="bindBlur(this,$(this).attr('msgs'))"/>
                    </div>
                    <div class="add_dy">
                        <?php foreach($all_dong_unit_door as $key2=>$value2){ ?>
                        <div class="dy_main">
                            <div class="bind_dy">
                                <input type="text" name="unit_name[]" value="<?php echo $key2; ?>" msgs="<?php echo $key2; ?>" onfocus="bindFocus(this,this.value)" onblur="bindBlur(this,$(this).attr('msgs'))"/>
                            </div>
                            <div class="bind_mp clearfix">
                                <div class="mp_li clearfix">
                                    <?php foreach($value2 as $key3=>$value3){ ?>
                                    <div class="more_mp">
                                        <input class="door" type="text" value="<?php echo $value3['name']; ?>" msgs="<?php echo $value3['name']; ?>" onfocus="bindFocus(this,this.value)" onblur="bindBlur(this,$(this).attr('msgs'))"/>
                                        <?php if($key3 > 0){ ?>
        ...
        <div class="all_btns" style="padding-bottom:80px;">
            <div class="bt_xg clearfix" style="width:440px;<?php if(is_full_array($all_dong_unit_door)){echo 'display: block;';} ?>">
                <?php if('1'==$is_lock){ ?>
                <button class="button_bc bt_readd jll" id="modify_button" style="display:none;">修改资料</button>
                <button class="button_bc  bt_readd_no jll"  id="modify_button3">修改资料</button>
                <?php }else{ ?>
                <button class="button_bc bt_readd jll" id="modify_button">修改资料</button>
                <button class="button_bc  bt_readd_no jll"  id="modify_button3" style="display:none;">修改资料</button>
                <?php } ?>
                <button class="button_bc bt_readd jll" id="modify_button_2" style="display:none;">保存修改</button>
                ...
                <button class="button_bc" onclick="window.history.go(-1);" style="float:left">返回</button>
            </div>
...
<script>
  ...
  <?php } ?>
    ...
    $('#modify_button_2').live('click',function(){
        ...
        $.ajax({
            url: "/community/modify_dong/",
            type: "GET",
            data: {
                dong: dong,
                dong_id:<?php echo $dong_id; ?>,
                cmt_id:<?php echo $cmt_id; ?>,
                dong_name:dong_name
            },
            success: function(data) {
    ...
    $('#close_refresh').live('click',function(){
