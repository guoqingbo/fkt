...
class Buy_customer_model extends Customer_base_model
{
   ...
   */
  public function add_buy_customer_info($data_info)
  {
   ...
   */
  public function get_buylist_by_cond($cond_where, $offset = 0, $limit = 10, $order_key = 'updatetime', $order_by = 'DESC')
  {
    $arr_data = array();
  ...
  public function get_all_customer_by_ids($customer_id)
  {
    ...
    if (!empty($customer_id)) {
      $sql = " SELECT * FROM  `buy_customer` WHERE id IN ($customer_id) ";
  ...
  public function get_all_isshare_by_ids($id)
  {
    ...
    if (!empty($id)) {
      $sql = " SELECT is_share FROM  `buy_customer` WHERE id IN ($id) ";
      $query = $this->db->query($sql);
      $result_arr = $query->result_array();
   ...
   */
  public function get_buynum_by_cond($cond_where = '')
  {
    $buynum = 0;
  ...
  public function get_id_by_brokerid($broker_id)
  {
    ...
    $this->dbback_city->from($this->_buy_customer_tbl);
  ...
  //    出售客源验证数组
  public function checkarr($arr, $broker_info, $view_import_customer)
  {
    $data = array();
    $data_fail = array();
    //加载求购、求租基本配置MODEL
    ...
    if (!empty($arr[12])) { //电话不能为空
      $broker = $this->broker_info_model->get_one_by(array('phone' => $arr[12]));
  ...
  public function dist_info($where = array(), $like = array(), $offset = 0, $pagesize = 0, $database = 'dbback_city')
  {
    $comm = $this->get_data(array('form_name' => $this->district, 'where' => $where, 'like' => $like, 'limit' => $offset, 'offset' => $pagesize), $database);
    return $comm;
  }

  public function street_info($where = array(), $like = array(), $offset = 0, $pagesize = 0, $database = 'dbback_city')
  {
  ...
  public function get_customer_cooperate_end_by_company_id($company_id = 0)
  {
    ...
    $result_arr = array();
    if (!empty($company_id)) {
      $where_sql = 'SELECT distinct buy_customer.id FROM `buy_customer` left join `cooperate` on buy_customer.id = cooperate.customer_id ';
      $where_sql .= ' where cooperate.tbl = "sell" and cooperate.esta in (5,6,8,9,10,11) ';
      $where_sql .= ' and buy_customer.company_id = "' . intval($company_id) . '"';
  ...
  public function get_customer_id_esta_4_7()
  {
    ...
    $where_sql .= ' where tbl = "sell" and (esta = 4 or esta = 7) and apply_type=2';
    $query = $this->db->query($where_sql);
    $result_arr = $query->result_array();
    return $result_arr;
  }
  ...
  public function change_is_share_not_customer_id($customer_id_arr, $agency_id = 0)
  {
    $customer_id_str = '';
    $result = false;
    if (is_full_array($customer_id_arr) && !empty($agency_id)) {
      foreach ($customer_id_arr as $k => $v) {
      ...
      $data['is_share'] = 0;
      $old_share = 1;
      $cond_where = "agency_id = '" . "$agency_id" . "' and id not in ($customer_id_str) and is_share = '" . $old_share . "'";
      $result = parent::update_info_by_cond($data, $cond_where);
    }
    return $result;
  }
  ...
  {
    $result = $this->modify_data($where, $data, $database = 'db_city', $form_name = $this->_buy_customer_tbl);
    return $result;
  }
  ...
  public function get_isshare_2_customer_id_by_agency_id($agency_id)
  {
    $this->dbselect('dbback_city');

    if (!empty($agency_id)) {
      $sql = " SELECT id FROM  `buy_customer` WHERE is_share = 2 AND agency_id = '" . $agency_id . "'";
      $query = $this->db->query($sql);
      $result_arr = $query->result_array();
