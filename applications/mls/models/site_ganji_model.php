...
class Site_ganji_model extends Site_ganji_base_model
{
  ...
  public function __construct()
  {
    ...
    if ($this->citysite->ganji_city($tpcity)) {
      $this->broker_info['city_spell'] = $this->citysite->ganji_city($tpcity);
    }
  }
  ...
  //赶集VIP 下架
  //public function esta_delete_vip($house_id, $act, $nolog=0, $queue_id=0, $broker_id=0 )
    ...
    try {
      ...
      if ($login['cookie']) {
        ...
        if (strpos($tmpInfo, '\u7acb\u5373\u5237\u65b0\u6210\u529f')) {
          ...
          $this->group_refresh_model->info_count($this->broker_info, 4);
          //王欣统计
  ...
  public function save_bind_vip22()
  {
    ...
    if ($username && $password) {
      ...
      if ($userid[1]) {
        ...
        $param = array();
        $param['broker_id'] = $broker_info['broker_id'];
        ...
        if (count($find) >= 1) {
          ...
          $result = $this->add_data($param, 'db_city', 'mass_site_broker');
        }
  ...
  protected function publish_param_normal($queue)
  {
    ...
    $mass_broker = $this->site_mass_model->isbind_site('ganji', $broker_id);  //目标站点 帐号信息
    ...
    if ($house_info['pic_ids'] && $house_info['pic_tbl']) {
      ...
      if ($pic_info) {
        foreach ($pic_info as $key => $val) {
          ...
          $shinei = $this->upload_image($picurl, $broker_id, 'ganji');
          $shineipic[$i] = array(
            'image' => $shinei,
            'thumb_image' => str_replace('.jpg', '_120-100c_6-0.jpg', $shinei),
            'width' => '600',
            'height' => '398',
            'id' => 'SWFUpload_1_' . $i,
            'is_new' => 'true'
  ...
  protected function publish_param_vip($queue)
  {
    ...
    $broker_info = $this->broker_info_model->get_by_broker_id($broker_id);  //经纪人信息
    $mass_broker = $this->site_mass_model->isbind_site('ganjivip', $broker_id);  //目标站点 帐号信息
    $userlist = explode('|', $mass_broker['otherpwd']);
    $userid = $mass_broker['user_id'];
    $username = $userlist[1];
    $userphone = $userlist[0];
    ...
    if ($house_info['pic_ids'] && $house_info['pic_tbl']) {
      ...
      if ($pic_info) {
        foreach ($pic_info as $key => $val) {
          $picurl = changepic_send($val['url']);
          if ($val['type'] == 1) {
            $shinei = $this->upload_image($picurl, $broker_id, 'ganjivip');
            $shineipic[$i] = array(
              'image' => $shinei,
              'thumb_image' => str_replace('.jpg', '_120-100c_6-0.jpg', $shinei),
              'width' => '600',
            ...
            $huxing = $this->upload_image($picurl, $broker_id, 'ganjivip');
            $huxingpic[$j] = array(
              'image' => $huxing,
              'thumb_image' => str_replace('.jpg', '_120-100c_6-0.jpg', $huxing),
              'width' => '600',
              'height' => '398',
              'id' => 'SWFUpload_1_' . $i,
              'is_new' => 'true'
...
/* Location: ./application/mls/models/site_ganji_model.php */
