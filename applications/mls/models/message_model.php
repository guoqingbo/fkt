...
class Message_model extends MY_Model
{
                                     ...
                                     $database = 'dbback_city')
  {
    ...
    $this->db->join($this->message_broker, $this->message_broker . '.msg_id = ' . $this->message . '.id');
    $this->db->where($where);
    $this->db->order_by("{$this->message}.is_top DESC,{$this->message_broker}.createtime DESC");
    //echo "{$this->message_broker}.is_top DESC,{$this->message_broker}.createtime DESC";die();
    $this->db->limit($limit, $offset); //开始位置，数量
    $result = $this->db->get();
    return $result->result_array();
  }
