...
class Customer_collect_model extends Customer_base_model
{
   ...
   * @return  int 取消条数
   */
  public function cancel_collection_by_id($c_id, $broker_id, $type = 'buy_customer')
  {
    $up_num = 0;
   ...
   * @return  array 收藏记录
   */
  public function get_collect_ids_by_bid($broker_id, $type = 'buy_customer', $status = 1)
  {
    $broker_id = intval($broker_id);
    $type = strip_tags($type);
   ...
   * @return  int 收藏个数
   */
  public function get_collectionnum_by_cid($customer_id, $broker_id, $type = 'buy_customer', $status = '')
  {
    ...
    $cond_where = "customer_id = '" . $customer_id . "' AND broker_id = '" . $broker_id . "' AND tbl = '" . $type . "'";
    $cond_where .= $status != 0 ? " AND status = '" . $status . "'" : "";
   ...
   * @return  int 收藏个数
   */
  public function get_collectionnum_by_bid($broker_id, $type = 'buy_customer', $status = 1)
  {
    $broker_id = intval($broker_id);
    $type = strip_tags($type);
    $status = intval($status);
