...
class Attendance_model extends Attendance_base_model
{
  ...
  {
    //查询字段
    $this->dbback_city->select("a.*,b.truename AS broker_name,c.name AS agency_name");

    //查询条件
    $this->dbback_city->where("a.id = {$id}");

    $this->dbback_city->from($this->_tbl1 . " AS a");
    $this->dbback_city->join($this->_tbl2 . " AS b", "a.broker_id = b.broker_id");
    $this->dbback_city->join($this->_tbl3 . " AS c", "a.agency_id = c.id");
