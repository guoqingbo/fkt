...
class Site_anjuke_model extends Site_anjuke_base_model
{
  ...
  public function isbind_anjuke()
  {
    ...
    return $this->isbind($broker_id, $site_id);
  }
  ...
  public function collect_list($act)
  {
    ...
    $site_id = $this->anjuke['id'];
    $broker_id = $this->broker_info['broker_id'];
    $login = $this->isbind($broker_id, $site_id);
    $cookie = empty($login['cookies']) ? '' : $login['cookies'];

    if ($cookie) {
      if ($act == 'sell') {
        $list_one = 'http://my.anjuke.com/ajkbroker/combo/broker/manage/ajk/';
        $list_two = 'http://my.anjuke.com/ajkbroker/combo/broker/manage/ajk/v2/';
        $bug = '';
        $type = array('楼售', '铺售');
      } else {
        $list_one = 'http://my.anjuke.com/ajkbroker/combo/broker/manage/hz/';
        $list_two = 'http://my.anjuke.com/ajkbroker/combo/broker/manage/hz/v2/';
        $bug = 'http://my.anjuke.com';
        $type = array('楼租', '铺租');
      }
      $tmpInfo = $this->curl->vget($list_one, $cookie);
      $moreInfo = $this->curl->vget('http://my.anjuke.com/ajkbroker/combo/broker/manage/jp', $cookie); //商业地产:商铺 and 写字楼
      if (empty($tmpInfo)) {
        $tmpInfo = $this->curl->vget($list_two, $cookie);
        $moreInfo = $this->curl->vget('http://my.anjuke.com/ajkbroker/combo/broker/manage/jp/v2/', $cookie);
      }
  ...
  public function collect_sell_info()
  {
    ...
    $infourl = str_replace('view/', 'view/A', $infourl);
    ...
    if (empty($login['cookies'])) {
      ...
      if (strstr($url, 'shop')) {
        ...
        $url = 'http://my.anjuke.com/ajkbroker/house/publish/jpss/' . $hid[1];  //编辑页跳转,重新拼装
      } elseif (strstr($url, 'office')) {
        preg_match('/sale\/(.*)\?from=no/siU', $url, $hid);
        $url = 'http://my.anjuke.com/ajkbroker/house/publish/jpos/' . $hid[1];
      }
  ...
  public function collect_rent_info()
  {
    ...
    if (empty($login['cookies'])) {
      ...
      if (strstr($url, 'shop')) {
        ...
        $url = 'http://my.anjuke.com/ajkbroker/house/publish/jpsr/' . $hid[1];  //编辑页跳转,重新拼装
      } elseif (strstr($url, 'office')) {
        preg_match('/rent\/(.*)\?from=no/siU', $url, $hid);
        $url = 'http://my.anjuke.com/ajkbroker/house/publish/jpor/' . $hid[1];
      }
      $picInfo = $this->curl->vget($infourl);
      $tmpInfo = $this->curl->vget($url, $login['cookies']);
      preg_match('/class="com-title page-title">(.*)<\/h4>/siU', $tmpInfo, $mytype);
  ...
  public function get_house($house_id, $block_info, $act)
  {
      ...
      if( $act=='sell' ){ //出售 sell
          if( $the_type==1 || $the_type==2 ){ //住宅 别墅
              $data_house['post_url'] = 'http://my.anjuke.com/ajkbroker/house/publish/ajk';
              ...
              $data_house['post_fielde'] .= '&act=publish&fixPlanId=&broker_id='.$userid.'&areaTplId=0&commTplId=0&commpropTplId=0&CommId='.$block_id
                      ...
                      .'&tags='.$shinei_pic.$huxing_pic.'&broker_action=save';
          }elseif ( $the_type==3 ){ //商铺
              $data_house['post_url'] = 'http://my.anjuke.com/ajkbroker/house/publish/jpss';
              ...
              $data_house['post_url'] = 'http://my.anjuke.com/ajkbroker/house/publish/jpos';
          ...
          if( $the_type==1 || $the_type==2 ){ //住宅 别墅
              ...
              $data_house['post_url'] = 'http://my.anjuke.com/ajkbroker/house/publish/hz/';
              ...
              $data_house['post_url'] = 'http://my.anjuke.com/ajkbroker/house/publish/jpsr';
              ...
              $data_house['post_url'] = 'http://my.anjuke.com/ajkbroker/house/publish/jpor';
  ...
  public function send_house($data_house, $house_id)
  {
      ...
      if( $login['cookies'] ){
          ...
          if( $publish_id ){
              ...
              $this->publish_count_num_model->info_count($broker_info, 5, array('tbl'=>$tbl, 'house_id'=>$house_id));
              //记录 安居客楼盘
  ...
  public function esta_delete($house_id, $act, $param = array())
  {
    ...
    if (!empty($login['cookies']) && $publish_id) {
      ...
      if ($the_type == 3 || $the_type == 4) {
        $post_url = 'http://my.anjuke.com/ajkbroker/ajax/house/jp_house?act=deleteHouses'; //商铺 写字楼
      } elseif ($act == 'sell') {
        $post_url = 'http://my.anjuke.com/ajkbroker/ajax/combo/ajk/delete_house/'; //二手房
      } else {
        $post_url = "http://my.anjuke.com/ajkbroker/ajax/house/hz_house?act=deleteHouses"; //租房
      }
  ...
  public function get_keyword($alias = '', $act = '')
  {
    ...
    if (!empty($login['cookies'])) {
      ...
      $url = 'http://my.anjuke.com/ajkbroker/ajax/community/search/?q=' . urlencode($keyword) . $type;
  ...
  public function upload_image($url, $broker_id)
  {
    ...
    $login = $this->site_mass_model->isbind_site('anjuke', $broker_id);
  ...
  public function check_login()
  {
      ...
      if( empty($username) ){
          $where = array('broker_id'=>$this->broker_info['broker_id'], 'site_id'=>$this->anjuke['id'], 'status'=>1);
      ...
      if( $pro[1]=='登录成功  - 安居客通行证' )
      {
          ...
          $resInfo = $this->curl->vget('http://my.anjuke.com/user/combo/brokerhome', $cookie); //获取用户信息
          preg_match('/<input type="hidden" name="ql_broker" value="(.*)" id="ql_broker_id"\/>/siU', $resInfo, $uid);
      }
      if( !empty($uid[1]) ){
  ...
  */
  ...
  public function publish_param($queue)
  {
    ...
    $mass_broker = $this->site_mass_model->isbind_site('anjuke', $broker_id);  //目标站点 帐号信息
...
/* Location: ./application/mls/models/site_anjuke_model.php */
