...
class Pic_model extends MY_Model
{
  ...
  {
    parent::__construct();
    if (!empty($_SESSION[USER_SESSION_KEY]["city_spell"])) {
      $this->_save_dir = '/' . $_SESSION[USER_SESSION_KEY]["city_spell"] . 'mls/';
    }
  }
  ...
  {
    //$temp_pic="D:/img/image.jpg";
    $temp_pic = str_replace("\\", "/", $temp_pic);
   ...
   * 上传图片，并返回上传后的地址
   */
  public function common_upload($need_watermark = 0, $watermark_str = '')
  {
    ...
    //为1表示启用文字水印
    if ($need_watermark == 1 && $watermark_str != '') {
      $this->uf->setWatermark(TRUE);
      $this->uf->setWatermarkType(1);
      $this->uf->setWatermarkFont();
      $this->uf->setWatermarkString($watermark_str);
      $this->uf->setWatermarkSize(24);
      $this->uf->setWatermarkPosition('cc');
    }
   ...
   * 房源打印
   */
  public function house_print($cmt_name = '', $price = '', $area = '', $apartment = '', $fitment = '', $forward = '', $remark = '', $broker = '', $shinei = '', $huxing = '', $house_id, $type = 0, $agency_name = '', $qrcode = '')
  {
    ...
    $insert_pic_arr = array();
    if (!empty($shinei)) {
      $insert_pic_arr['shinei'] = $shinei;
    }
    if (!empty($huxing)) {
      $insert_pic_arr['huxing'] = $huxing;
    }
    //经纪人二维码
    if (!empty($qrcode)) {
      $insert_pic_arr['qrcode'] = $qrcode;
    }
    header('Content-Type:text/html;charset=utf-8');
    ...
    $result = $this->uf->house_pic_deal($insert_str_arr, $insert_pic_arr, $house_id, $type);
    return $result;
  }
   ...
   * 个人中心，门店二维码打印
   */
  public function agency_print($agency_scode_img = '', $agency_id)
  {
    header('Content-Type:text/html;charset=utf-8');
    ...
    $result = $this->uf->agency_pic_deal($agency_scode_img, $agency_id);
    return $result;
  }
  ...
  public function find_house_pic_by($tbl, $house_id, $sort = '')
  {
     ...
     * @return array
     */
    public function get_house_pic_by($tbl, $house_id, $type = '')
    {
        $uploadtable = 'upload';
        $this->db_city->select('id,type,url,rowid,is_top');
        $this->db_city->where('tbl', $tbl);
        $this->db_city->where('rowid', $house_id);
        $this->db_city->order_by('is_top', 'DESC');
        if ($type !== '') {
