...
//    出售房源验证数组
  ...
  public function checkarr($arr, $broker_info, $view_import_house)
  {
    ...
    public function guli_checkarr($arr, $broker_info, $view_import_house)
    {
        $data = array();
        $data_fail = array();
        //加载出售基本配置MODEL
        ...
        $data['config'] = $this->house_config_model->get_config();
        //if(!empty($arr[0]) && !eregi("[^\x80-\xff]","$arr[0]")){ //楼盘名称不为空并且为中文
        if (!empty($arr[3]) && !eregi("^[\u4e00-\u9fa5a-zA-Z]+$", "$arr[3]")) { //楼盘名称不为空并且可以是中文、英文
            ...
            if ($community_info[0]['id']) {
                ...
                //判断是否楼盘需要加入临时小区
                $this->load->model('district_base_model');
                if (!empty($arr[1])) {  //区属不能空
                    $dist_arr = $this->district_base_model->get_district_id($arr[1]);
                    if (!empty($dist_arr)) {
                        $res[1] = true;
                    } else {
                        $res[1] = false;
                ...
                if (!empty($arr[2])) {  //板块不能空
                    if (!empty($dist_arr)) {
                        $streetname_arr = $this->district_base_model->get_streetname_bydist($dist_arr['id']);
                        //print_r($streetname_arr);exit;
                        if (in_array($arr[2], $streetname_arr)) {
                            $res[2] = true;
                        } else {
                            $res[2] = false;
                            $data_fail[] = 2;
                        ...
                        $street_arr = $this->district_base_model->get_street_id($arr[2]);
                        //print_r( $street_arr);exit;
                        if (!empty($street_arr)) {
                            $res[2] = true;
                        } else {
                ...
                if (($res[1] == true) || ($res[2] == true) || ($res[5] == true)) {
                    $res[0] = true;
                } else {
                    $res[0] = false;
                    $data_fail[] = 0;
...
//            $data_fail[] = 1;
//        }
        if (!empty($arr[5])) { //栋座不为空并且为数字
            $res[5] = true;
        } else {
            $res[5] = false;
            $data_fail[] = 5;
        }
        if (!empty($arr[5]) && eregi("^[0-9]+$", $arr[5])) {  //单元不为空并且为数字
            $res[5] = true;
        } else {
            $res[5] = false;
            $data_fail[] = 5;
...
//            $data_fail[] = 4;
//        }
        if (!empty($arr[11]) && !eregi("[^\x80-\xff]", "$arr[11]")) { //业主姓名不为空并且为中文
            $res[11] = true;
        } else {
            $res[11] = false;
            $data_fail[] = 11;
        }
        if (!empty($arr[12])) { //业主电话不为空
            $tel = explode("/", $arr[12]);
            if (count($tel) < 4) {
                $isMob = "/^1[3-5,8]{1}[0-9]{9}$/";
                $isTel = "/^([0-9]{3,4})?[0-9]{7,8}$/";
...
//            $data_fail[] = 13;
//        }
//        if (!empty($arr[8]) && is_numeric($arr[8])) { //面积不能为空
//            $res[8] = true;
//        } else {
//            $res[8] = false;
//            $data_fail[] = 8;
