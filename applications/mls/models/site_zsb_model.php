...
class Site_zsb_model extends MY_Model
{
  ...
  public function __construct()
  {
    ...
    $where = array('broker_id' => $this->broker_info['broker_id'], 'site_id' => $this->zsb['id'], 'status' => 1);
    $site_broker = $this->site_model->get_brokerinfo_byids($where);
    $this->zsb['username'] = empty($site_broker[0]) ? '' : $site_broker[0]['username'];
    $this->zsb['password'] = empty($site_broker[0]) ? '' : $site_broker[0]['password'];
  }
  ...
  public function get_house_rent($house_id, $act, $publishid = 0)
  {
    ...
    if (is_int($role_level) && $role_level > 6) {
      if ($broker_id != $house_info['broker_id'] && $house_info['nature'] == '1') {
        //获取当前经纪人的临时详情
        $result = $this->rent_house_model->get_temporaryinfo($house_info['id'], $house_info['broker_id'], $database = 'db_city');
        if (!empty($result)) {
    ...
    //$zsb_house['Content'] = $this->autocollect_model->con_replace($zsb_house['Content']);//描述富文本处理
    $zsb_house['Content'] = $this->autocollect_model->special_replace($zsb_house['Content']);  //描述富文本处理
  ...
  public function get_house_sell($house_id, $act, $publishid = 0)
  {
    ...
    if (is_int($role_level) && $role_level > 6) {
      if ($broker_id != $house_info['broker_id'] && $house_info['nature'] == '1') {
        //获取当前经纪人的临时详情
        $result = $this->sell_house_model->get_temporaryinfo($house_info['id'], $house_info['broker_id'], $database = 'db_city');
        if (!empty($result)) {
          $zsb_house['Title'] = $result[0]['title'];
    ...
    if ($house_id) {
      ...
      if (empty($publishinfo)) {
        ...
        if ($data['flag'] == 'success') {
          ...
          $tbl = ($act == 'sell') ? 1 : 2;
          $this->publish_count_num_model->info_count($broker_info, 7, array('tbl' => $tbl, 'house_id' => $house_id));
        } else {
  ...
  public function queue_refresh($alias = '')
  {
    ...
    $log_param = array('broker_id' => $broker_id,
      'house_id' => $house_id,
      'site_id' => $site_id,
    ...
    if (isset($result['state']) && $result['state'] == 'success') {
      ...
      $this->group_refresh_model->info_count($this->broker_info, 7);
      //王欣统计 成功
      $log_param['status'] = 1;
      $log_param['msg'] = '';
...
/* Location: ./application/mls/models/site_zsb_model.php */
