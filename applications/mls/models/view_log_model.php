...
class View_log_model extends MY_Model
{
   ...
   * @param string $agency_id 客源所属门店编号
   * @param string $broker_id 客源所属经纪人编号
   * @param string $agency_id_v 查看经纪人门店编号
   * @param string $agency_name_v 查看经纪人门店名称
   * @param string $broker_id_v 查看经纪人编号
   * @param string $broker_name_v 查看经纪人姓名
   * @param string $broker_telno_v 查看经纪人手机号码
   * @return  boolean 是否添加成功，TRUE-成功，FAlSE失败。
   */
  public function add_customer_view_log($type, $cid, $agency_id, $broker_id,
                                        $agency_id_v, $agency_name_v, $broker_id_v, $broker_name_v, $broker_telno_v)
  {
    ...
    $agency_id_v = intval($agency_id_v);
    $broker_id_v = intval($broker_id_v);
    $cond_where = "c_id = '" . $cid . "' AND  agency_id = '" . $agency_id . "' AND "
      . "broker_id = '" . $broker_id . "' AND agency_id_v = '" . $agency_id_v . "' "
      . "AND broker_id_v = '" . $broker_id_v . "'";
    $log_num = $this->get_view_log_num_by_cond($type, $cond_where);

    if (intval($log_num) == 0) {
      $log_arr['c_id'] = intval($cid);
      $log_arr['agency_id'] = intval($agency_id);
      $log_arr['broker_id'] = intval($broker_id);
      $log_arr['agency_id_v'] = intval($agency_id_v);
      $log_arr['agency_name_v'] = strip_tags($agency_name_v);
      $log_arr['broker_id_v'] = intval($broker_id_v);
      $log_arr['broker_name_v'] = strip_tags($broker_name_v);
      $log_arr['broker_telno_v'] = strip_tags($broker_telno_v);
   ...
   * @param string $agency_id 房源所属门店编号
   * @param string $broker_id 房源所属经纪人编号
   * @param string $agency_id_v 查看经纪人门店编号
   * @param string $agency_name_v 查看经纪人门店名称
   * @param string $broker_id_v 查看经纪人编号
   * @param string $broker_name_v 查看经纪人姓名
   ...
   * @return  boolean 是否添加成功，TRUE-成功，FAlSE失败。
   */
  public function add_house_view_log($type, $hid, $agency_id, $broker_id,
                                     $agency_id_v, $agency_name_v, $broker_id_v, $broker_name_v, $broker_telno_v)
  {
    $log_arr = array();
    $hid = intval($hid);
    ...
    $agency_id_v = intval($agency_id_v);
    $broker_id_v = intval($broker_id_v);
    $cond_where = "h_id = '" . $hid . "' AND  agency_id = '" . $agency_id . "' AND "
      . "broker_id = '" . $broker_id . "' AND agency_id_v = '" . $agency_id_v . "' "
      . "AND broker_id_v = '" . $broker_id_v . "'";
    $log_num = $this->get_view_log_num_by_cond($type, $cond_where);
   ...
   * cc
   */
  public function add_collect_click_log($broker_id, $house_id, $type)
  {
    $data = array(
      'broker_id' => $broker_id,
      'house_id' => $house_id,
