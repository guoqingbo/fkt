...
class Site_model extends MY_Model
{
  ...
  public function __construct()
  {
    ...
    $this->mass_site_signatory = 'mass_site_signatory';
  ...
  function upload_img($url)
  {
    ...
    $picname = $signatory_id . date('YmdHis') . $form;
    $localfile = "./applications/mls_guli/photo/" . $picname;
   ...
   * @author       angel_in_us
   */
  function get_site_signatory_num($where = array(), $like = array(), $database = 'dbback_city')
  {
    $sell_sum = $this->get_data(array('form_name' => $this->mass_site_signatory, 'where' => $where, 'select' => array('count(*) as num')), $database);
    return $sell_sum[0]['num'];
  }
   ...
   * @author       angel_in_us
   */
  function get_signatory_site($where = array(), $where_in = array(), $like = array(), $offset = 0, $limit = 50, $database = 'dbback_city')
  {
    $result = $this->get_data(array('form_name' => $this->mass_site_signatory, 'where' => $where, 'where_in' => $where_in, 'like' => $like, 'offset' => $offset, 'limit' => $limit), $database);
    return $result;
  }
  ...
  {
    $result = $this->del($where, 'db_city', $this->mass_site_signatory);
    return $result;
  }
   ...
   * @author       angel_in_us
   */
  function add_signatory_interface($data = array())
  {
    $result = $this->add_data($data, 'db_city', $this->mass_site_signatory);
    return $result;
  }
   ...
   * @author       angel_in_us
   */
  function get_signatoryinfo_byids($where = array(), $database = 'dbback_city')
  {
    $result = $this->get_data(array('form_name' => $this->mass_site_signatory, 'where' => $where), $database);
    return $result;
  }
   ...
   * @author       angel_in_us
   */
  function check_signatory_site($where = array(), $database = 'dbback_city')
  {
    $result = $this->get_data(array('form_name' => $this->mass_site_signatory, 'where' => $where), $database);
    return $result;
  }
  ...
  /**
   * 根据 username 来更新 mass_site_signatory 表中的密码
   */
  function update_signatory_pwd($where = array(), $arr = array())
  {
    $result = $this->modify_data($where, $arr, 'db_city', $this->mass_site_signatory);
    return $result;
  }

  /**
   * 根据 signatory_id 获取绑定 群发站点
   */
  function get_mess_site($signatory_id, $site = '')
  {
    $data = array();
    if ($signatory_id) {
      $this->dbback_city->select($this->mass_site . ".*," . $this->mass_site_signatory . ".status as bstatus");
      $where = $this->mass_site_signatory . ".signatory_id = '$signatory_id'";
      if ($site) {
        $where .= " and " . $this->mass_site . ".id in ($site,0)";
      }
      $this->dbback_city->where($where);
      $this->dbback_city->from($this->mass_site_signatory);
      $this->dbback_city->join($this->mass_site, "$this->mass_site_signatory.site_id =  $this->mass_site.id");
   ...
   * 根据 signatory_id 获取绑定 群发站点
   */
  function publish_site($signatory_id, $type = '')
  {
    $data = array();
    if ($signatory_id) {
      $this->dbback_city->select($this->mass_site . ".*");
      $where = $this->mass_site_signatory . ".signatory_id = '$signatory_id' and " . $this->mass_site_signatory . ".status = 1 ";
...
/* Location: ./application/mls_guli/models/site_model.php */
