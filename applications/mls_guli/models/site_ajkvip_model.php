...
class Site_ajkvip_model extends Site_ajkvip_base_model
{
  ...
  public function collect_list($act)
  {
    ...
    $site_id = $this->website['id'];
    $signatory_id = $this->signatory_info['signatory_id'];
    $login = $this->isbind($signatory_id, $otherpwd);
    $cookie = empty($login['cookies']) ? '' : $login['cookies'];

    if ($cookie) {
      if ($act == 'sell') {
        $list_one = 'http://vip.anjuke.com/combo/signatory/manage/ajk/';
        $list_two = 'http://vip.anjuke.com/combo/signatory/manage/ajk/?from=V2';
        $type = array('楼售', '铺售');
      } else {
        $list_one = 'http://vip.anjuke.com/combo/signatory/manage/hz/';
        $list_two = 'http://vip.anjuke.com/combo/signatory/manage/hz/?from=V2';
        $type = array('楼租', '铺租');
      }
      $tmpInfo = $this->curl->vget($list_one, $cookie);
      $moreInfo = $this->curl->vget('http://vip.anjuke.com/combo/signatory/manage/jp/', $cookie); //商业地产:商铺 and 写字楼
      if (empty($tmpInfo)) {
  ...
  public function collect_sell_info()
  {
    ...
    if (empty($cookie)) {
      ...
      if (strstr($infourl, 'esffyxxd')) {
        $infourl = str_flow(array('view/'), array('view/A'), $infourl);
      }
  ...
  public function esta_delete($extra)
  {
    ...
    $signatory_id = $extra['signatory_id'];
  ...
  public function upload_image($url, $signatory_id)
  {
    ...
    $login = $this->site_mass_model->isbind_site('ajkvip', $signatory_id);
  ...
  public function queue_publish($alias)
  {
    ...
    if (empty($block_info)) {
      ...
      if ($publishinfo) {
        $extra = array('house_id' => $house_id, 'signatory_id' => $signatory_id, 'tbl' => $tbl, 'nolog' => 1);
  ...
  public function queue_esta($queue)
  {
    ...
    if ($group[0]) {
      ...
      $extra = array(
        ...
        'queue_id' => $group['id'],
        'signatory_id' => $group['signatory_id'],
        'tbl' => $group['tbl'],
        'nolog' => 0
      );
  ...
  public function publish_param($queue)
  {
    ...
    $mass_signatory = $this->site_mass_model->isbind_site('ajkvip', $signatory_id);  //目标站点 帐号信息
    ...
    $data_house['act'] = $act;
    $data_house['post_field'] .= '&signatory_id=' . $userid . '&cityId=' . $city_id . '&sites=1&roomImagesSaved=' . $shinei_pic . '&file=&defaultImgID='
...
/* Location: ./application/mls_guli/models/site_ajkvip_model.php */
