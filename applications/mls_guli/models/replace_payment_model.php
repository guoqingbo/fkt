 ...
 * @author          No.one
 */
class Replace_payment_model extends MY_Model
{

  private $_tbl = 'replace_payment';
  ...
  public function count_by($where = '')
  {
    ...
    return $this->dbback_city->count_all_results($this->_tbl . ' f');
  }
  ...
  public function get_list_by_cond1($where = '')
  {
    ...
    //排序条件
    $this->dbback_city->order_by('flow_time', 'SC');
  ...
  {
    $result = $this->get_data(array('form_name' => 'replace_payment', 'where' => $where, 'where_in' => $where_in, 'like' => $like, 'offset' => $offset, 'limit' => $limit), $database);
    return $result;
  }
...
//总计数据
    ...
    {
        //查询字段
        $this->dbback_city->select("SUM(money_number) AS money_total");
  ...
  public function get_count_by_cond($cond_where = '')
  {
    ...
    if ($cond_where != '') {
      ...
      $count_num = $this->dbback_city->count_all_results('replace_payment');
    }
