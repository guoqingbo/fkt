...
//    读取文件1
  public function read($model, $signatory_info, $upload, $i, $type, $view_import_data)
  {
    $this->load->model($model);
    $filename = $upload['full_path'];
    $signatory_id = intval($signatory_info['signatory_id']);
    ...
    if ($valid_num <= 1000) {
      ...
      foreach ($excelData as $array => $arr) {
        if ($this->$model->checkarr($arr, $signatory_info, $view_import_data) == 'pass') {
          $datas[] = $arr;
        } else {
          $datas_fail[$array] = $this->$model->checkarr($arr, $signatory_info, $view_import_data);
        }
      }
      ...
      if (!empty($datas) && empty($datas_fail)) {
        ...
        $data = array('signatory_id' => $signatory_id,
          'content' => serialize($datas),
          'createtime' => time()
        );
        $id = $this->$model->add_data($data, 'db_city', 'tmp_uploads');
        /*return '<link type="text/css" rel="stylesheet" href="'.MLS_SOURCE_URL.'/min/?f=mls_guli/css/v1.0/base.css,mls_guli/third/iconfont/iconfont.css,mls_guli/css/v1.0/house_manage.css"><p class="up_m_b_date_up">'.$upload['client_name'].'<span class="up_s">上传成功</span>，共上传'.count($datas).'条房源</p>'
        .'<input type="hidden" id=tmp_id value='.$id.'>';*/
        /*return '<link type="text/css" rel="stylesheet" href="'.MLS_SOURCE_URL.'/min/?f=mls_guli/css/v1.0/base.css,mls_guli/third/iconfont/iconfont.css,mls_guli/css/v1.0/house_manage.css"><p class="up_m_b_date_up" style="text-align: center">'.$upload['client_name'].'<span class="up_s">上传成功</span>，共上传'.count($datas).'条房源。</p>'
        .'<input type="hidden" id=tmp_id value='.$id.'>';*/

        return '<!DOCTYPE html><html><head lang="en"><meta charset="UTF-8"><title>空白页面</title><link type="text/css" rel="stylesheet" href="' . MLS_SOURCE_URL . '/min/?f=mls_guli/css/v1.0/base.css,mls_guli/third/iconfont/iconfont.css,mls_guli/css/v1.0/house_manage.css">
<style>*{ background: transparent !important;}</style></head><body style="background: transparent"><p class="up_m_b_date_up" style="text-align: center">' . $upload['client_name'] . '<span class="up_s">上传成功</span>，共上传' . count($datas) . '条信息。</p>'
        ...
        /*return '<link type="text/css" rel="stylesheet" href="'.MLS_SOURCE_URL.'/min/?f=mls_guli/css/v1.0/base.css,mls_guli/third/iconfont/iconfont.css,mls_guli/css/v1.0/house_manage.css"><p class="up_m_b_date_up">'.$upload['client_name'].'<span class="up_e">上传失败</span>，请按照标准模板重新上传</p>';*/
        /*return '<link type="text/css" rel="stylesheet" href="'.MLS_SOURCE_URL.'/min/?f=mls_guli/css/v1.0/base.css,mls_guli/third/iconfont/iconfont.css,mls_guli/css/v1.0/house_manage.css"><p class="up_m_b_date_up" style="text-align: center">'.$upload['client_name'].'<span class="up_e">上传失败</span>，请按照标准模板重新上传</p>';*/
        $fail_html = '<!DOCTYPE html><html><head lang="en"><meta charset="UTF-8"><title>空白页面</title><link type="text/css" rel="stylesheet" href="' . MLS_SOURCE_URL . '/min/?f=mls_guli/css/v1.0/base.css,mls_guli/third/iconfont/iconfont.css,mls_guli/css/v1.0/house_manage.css">
...
<style>*{ background: transparent !important;}</style></head><body style="background: transparent"><p class="up_m_b_date_up" style="text-align: center">' . $upload['client_name'] . '<span class="up_e">上传失败</span>，一次最多只能导入1000条记录哦！</p></body></html>';
  ...
  public function read_you($model, $signatory_info, $upload, $i, $type)
  {
    $this->load->model($model);
    ...
    if ($valid_num <= 1000) {
      ...
      foreach ($excelData as $array => $arr) {
        ...
        $arr[9] = str_flow('-', '/', $arr[9]);
        if ($arr[12] == '清水') {
          $arr[12] = '毛坯';
        }
        if ($model == 'sell_model') {
          $arr[15] = str_flow('万元', ' ', $arr[15]);
          if (!$arr[21]) {
            $arr[21] = '其他';
          }
          if ($arr[21] == '汽博') {
