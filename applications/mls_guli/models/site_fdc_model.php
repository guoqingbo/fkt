      ...
      if ($pic_info) {
        if ($the_type == 3 || $the_type == 4) {  //3商铺 4写字楼
          foreach ($pic_info as $key => $val) {
            ...
            $ext = array('url' => $picurl, 'signatory_id' => $signatory_id, 'the_type' => $the_type, 'sessid' => $sessid[1]);
          ...
          foreach ($pic_info as $key => $val) {
            $picurl = changepic_send($val['url']);
            $ext = array('url' => $picurl, 'signatory_id' => $signatory_id, 'the_type' => $the_type, 'gid' => $gid[1], 'adminid' => $adminid[1]);
...
/* Location: ./application/mls_guli/models/site_fdc_model.php */
