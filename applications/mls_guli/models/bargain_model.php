...
class Bargain_model extends Bargain_base_model
{
  ...
  public function get_range_menu_by_role_level($user_arr, $post_agency_id = 0)
  {
    ...
    {
      //获取该公司下的所有门店
      //$departments = $this->agency_model->get_children_by_company_id($user_arr['company_id']);
      $agencys = $this->agency_model->get_children_by_company_id(3);
      array_unshift($agencyss, array('id' => '', 'name' => '请选择'));
      if ($post_agency_id > 0) {
    ...
    } else if ($user_arr['role_level'] < 8) //门店
    {
      ...
      $this->broker_info_model->set_select_fields(array('broker_id', 'truename'));
      $brokers = $this->broker_info_model->get_by_department_id($user_arr['department_id']);
      array_unshift($brokers, array('brokers_id' => '', 'truename' => '请选择'));
  ...
  //根据角色层次获取访问范围权限
  public function get_range_agency_by_role_level($user_arr, $post_department_id = 0)
  {
//        $this->load->model('department_model');
//        $this->load->model('signatory_info_model');
    $this->load->model('agency_model');
    $this->load->model('broker_info_model');
    $departments = array(); //门店
    $signatorys = array(); //经纪人
    if ($user_arr['role_level'] < 6) //公司
    {
      //获取该公司下的所有门店
      //$departments = $this->agency_model->get_children_by_company_id($user_arr['company_id']);
      $departments = $this->agency_model->get_children_by_company_id(3);
      array_unshift($departments, array('id' => '', 'name' => '请选择'));
      if ($post_department_id > 0) {
        $this->signatory_info_model->set_select_fields(array('signatory_id', 'truename'));
        $signatorys = $this->signatory_info_model->get_by_department_id($post_department_id);
        array_unshift($signatorys, array('signatory_id' => '', 'truename' => '请选择'));
      } else {
        $signatorys = array(array('signatory_id' => '', 'truename' => '请选择'));
      }
    } else if ($user_arr['role_level'] < 8) //门店
    {
      $departments = array(
        array('id' => $user_arr['department_id'],
          'name' => $user_arr['department_name'])
      );
      $this->signatory_info_model->set_select_fields(array('signatory_id', 'truename'));
      $signatorys = $this->signatory_info_model->get_by_department_id($user_arr['department_id']);
      array_unshift($signatorys, array('signatory_id' => '', 'truename' => '请选择'));
    } else //查看自己的。目前没有此权限设置
    {
      $departments = array(
      ...
      $signatorys = array(
        array('signatory_id' => $user_arr['signatory_id'],
          'truename' => $user_arr['truename'])
      );
    }
    return array('departments' => $departments, 'signatorys' => $signatorys);
  }
}
