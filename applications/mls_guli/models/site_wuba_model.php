...
class Site_wuba_model extends Site_wuba_base_model
{
  ...
  public function isbind_wuba($vip)
  {
    ...
    if (!empty($site_signatory[0])) {
      $data['otherpwd'] = $site_signatory[0]['otherpwd'];
  ...
  public function bind_again()
  {
    ...
    if ($login['userid']) {
      $where = array('signatory_id' => $this->signatory_info['signatory_id'], 'site_id' => $this->wubaw['id']);
  ...
  public function refresh_vip($house_id, $act)
  {
    ...
    if ($login['cookie']) {
      ...
      if (strpos($tmpInfo, '立即刷新成功')) {
        ...
        $this->group_refresh_model->info_count($this->signatory_info, 2);
        //王欣统计
  ...
  public function get_keyword($vip, $act)
  {
    ...
    if ($find['cookie'] && in_array($sell_type, array(1, 2))) {
      ...
      $tmpInfo = str_flow('callback1111(', '', $tmpInfo);
      $tmpInfo = str_flow(' ] })', ' ] }', $tmpInfo);
  ...
  public function queue_esta($queue)
  {
    ...
    //帐号信息
    $login = $this->site_mass_model->isbind_site_by_id($site_id, $signatory_id);
    //发布信息
  ...
  protected function publish_param_normal($queue)
  {
    ...
    $mass_signatory = $this->site_mass_model->isbind_site('58P', $signatory_id);  //目标站点 帐号信息
    ...
    if ($house_info['pic_ids'] && $house_info['pic_tbl']) {
      ...
      if ($pic_info) {
        foreach ($pic_info as $key => $val) {
          ...
          $shinei = $this->upload_image($picurl, $signatory_id, '58P');
          $pic .= $shinei . "|";
        }
      }
    }
  ...
  protected function publish_param_vip($queue)
  {
    ...
    $cityid = $wuba_city['id'];  //58专属城市id
    $signatory_info = $this->signatory_info_model->get_by_signatory_id($signatory_id);  //经纪人信息
    $mass_signatory = $this->site_mass_model->isbind_site('58W', $signatory_id);  //目标站点 帐号信息
    $userlist = explode('|', $mass_signatory['otherpwd']);
    $userid = $mass_signatory['user_id'];
    $username = $userlist[1];
    $userphone = $userlist[0];
    ...
    if ($house_info['pic_ids'] && $house_info['pic_tbl']) {
      ...
      if ($pic_info) {
        foreach ($pic_info as $key => $val) {
          $picurl = changepic_send($val['url']);
          $shinei = $this->upload_image($picurl, $signatory_id, '58W');
          $pic .= $shinei . "|";
        }
      }
      $this->load->database();
...
/* Location: ./application/mls_guli/models/site_wuba_model.php */
