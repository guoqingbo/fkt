...
class Site_baixing_model extends Site_baixing_base_model
{
  ...
  public function collect_list($act)
  {
    ...
    if ($cookie) {
      ...
      foreach ($prj[2] as $val) {
        ...
        if ($htype[2] == $intype) {
          if ($time[1]) {
            $day = str_flow(array('年', '月', '日'), array('-', '-', ''), $time[1]);
  ...
  public function collect_sell_info()
  {
    ...
    if (!empty($prj['images'])) {
      foreach ($prj['images'] as $val) {
        $tpval = str_flow('_180x180', '_bi', $val);
  ...
  //上传图片
  public function upload_image($url, $signatory_id, $tbl, $publish_id)
  {
...
/* Location: ./application/mls_guli/models/site_baixing_model.php */
