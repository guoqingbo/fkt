 ...
 * @author          Lion
 */
class Customer_brower_model extends MY_Model
{
  /**
   * Constructor
   */
  ...
  {
    parent::__construct();
    $this->brower_customer_log = 'browse_cunstomer_mess_log';
  }
  ...
  {
    $result = $this->add_data($paramlist, 'db_city', $this->brower_customer_log);
    return $result;
  }
  ...
  {
    $comm = $this->get_data(array('form_name' => $this->brower_customer_log, 'group_by' => $group_by, 'where' => $where, 'limit' => $offset, 'offset' => $pagesize, 'order_by_array' => $order_by_array), $database);
    return $comm;
  }
  ...
  {
    $comm = $this->get_data(array('form_name' => $this->brower_customer_log, 'where' => $where, 'select' => array('count(*) as num')), 'dbback_city');
    return $comm[0]['num'];
  }
  ...
  public function get_brower_log_group_num($customer_id)
  {
    ...
    if (!empty($customer_id)) {
      ...
      $sql .= " (SELECT * FROM (`browse_cunstomer_mess_log`) ";
      $sql .= " WHERE `customer_id` = $customer_id GROUP BY `signatory_id`) AS NUM";
   ...
   * @return string 浏览记录数
   */
  public function get_today_brower_log_num($customer_id, $signatory_id, $today_browertime)
  {
    $this->dbselect('dbback_city');
    $where_sql = '';
    if (!empty($customer_id) && !empty($today_browertime)) {
      $where_sql = "SELECT count( * ) AS num FROM (";
      $where_sql .= "`browse_cunstomer_mess_log`";
      $where_sql .= ")";
      $where_sql .= " WHERE 1=1";
