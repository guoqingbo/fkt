...
class Bargain_transfer_model extends Transfer_base_model
{
  ...
  public function del_by_cid($cid)
  {
     ...
     * @return 0 or 1
     */
    public function get_bargain_ids($params)
    {
        $where = 'company_id = ' . $params['enter_company_id'] . ' and stage_id = ' . $params['stage_id'];
        if ($params['transfer_start_time'] > 0) {
            $where .= ' and complete_time > ' . strtotime($params['transfer_start_time']);
        }
        if ($params['transfer_end_time'] > 0) {
            $where .= ' and complete_time < ' . strtotime($params['transfer_end_time']);
        }
        $this->dbback_city->where($where);
