...
class Rent_house_model extends House_base_model
{
   ...
   * cc 2016.1.27
   */
  public function get_temporaryinfo($house_id, $signatory_id, $database = 'db_city')
  {
    $where = array('house_id' => $house_id, 'signatory_id' => $signatory_id);
  ...
  function change_department_id_by_borker_id($signatory_id, $department_id)
  {
  ...
  {
    $select_fields = array('id', 'signatory_id', 'block_name', 'block_id', 'district_id', 'subfloor', 'totalfloor',
  ...
  public function checkarr($arr, $signatory_info, $view_import_house)
  {
    $data = array();
    $data_fail = array();
    //加载出售基本配置MODEL
    ...
    if (!empty($arr[22])) { //电话不能为空
      $signatory = $this->signatory_info_model->get_one_by(array('phone' => $arr[22]));
  ...
  public function get_isshare_2_house_id_by_department_id($department_id)
  {
    ...
    if (!empty($department_id)) {
      $sql = " SELECT id FROM  `rent_house` WHERE isshare = 2 AND department_id = '" . $department_id . "'";
      $query = $this->db->query($sql);
      $result_arr = $query->result_array();
...
/* Location: ./applications/mls_guli/models/rent_house_model.php */
