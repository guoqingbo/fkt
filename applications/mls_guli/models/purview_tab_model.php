 ...
 * @author          sun
 */
load_m("Purview_tab_base_model");

class Purview_tab_model extends Purview_tab_base_model
{
   ...
   * @var string
   */
  private $_tbl = 'purview_base_tab';

  /**
   * 二级菜单表
   * @var string
   */
  private $_tbl_2 = 'purview_second_tab';
  ...
  public function __construct()
  {
    ...
    $this->_mem_key = $city . '_purview_tab_base_model_';
    $this->_init_tabs();
  ...
  public function get_tree_menu($class, $method)
  {
    ...
    $menu_html = '<ul>';
    if (is_full_array($this->_tab_arr) && $this->_now_module_id == 3) //成交管理
    {
      foreach ($this->_tab_arr as $tab) {
        ...
        $tabclass = '';
        //只有一级菜单
        if (!$is_exist_secondtab && $class == $tabcm[0]
          && $method == $tabcm[1]
        ) {
  ...
  private function _init_module()
  {
    ...
    if (isset($cache['is_ok']) && $cache['is_ok'] == 1) {
      ...
      $this->load->model('purview_module_model');
      $where = array('is_display' => 1);
      $module = $this->purview_module_model->get_all_by($where);
  ...
  public function _init_tabs()
  {
    ...
    if (is_full_array($module_arr)) {
      ...
      if (is_full_array($secondtab_arr)) {
        ...
        $signatory_data = $this->signatory_model->get_user_session();

        if (is_full_array($signatory_data)) {
          //读取经纪人认证信息
          $group_id = $signatory_data['group_id'];

          if (!empty($signatory_data['signatory_id'])) {
            $this->signatory_purview_model->set_signatory_id($signatory_data['signatory_id'], $signatory_data['company_id']);
          }
          //有权限的二级菜单
          foreach ($secondtab_arr as $secondtab) {
            $permision = $this->signatory_purview_model->check($secondtab['pid']);
            //检查该菜单是否有权限
          ...
          foreach ($tab_arr as $tab) {
            $permision = $this->signatory_purview_model->check($tab['pid']);
  ...
  public function get_secondtab($class, $method)
  {
    //菜单功能列表
    $secondtab_str = '';
    if ($this->_now_tab_id > 0 && is_full_array($this->_secondtabs[$this->_now_tab_id])) {
...
/* End of file purview_tab_model.php */
/* Location: ./app/models/purview_tab_model.php */
