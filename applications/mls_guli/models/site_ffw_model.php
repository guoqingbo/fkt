...
class Site_ffw_model extends Site_ffw_base_model
{
  ...
  public function isbind_site()
  {
    ...
    if (!empty($site_signatory[0])) {
      ...
      $data['user_id'] = $site_signatory[0]['user_id'];
      ...
      if ('0' == $json['ret_code']) {
        if (isset($json['data']['list']) && is_array($json['data']['list'])) {
          foreach ($json['data']['list'] as $value) {
            ...
            if ('0' == $_json['ret_code']) {
              $data['des'] = $this->autocollect_model->con_flow($_json['data']['intro']);
  ...
  public function collect_sell_info()
  {
    ...
    if ($find['cookie']) {
      ...
      if ('0' == $_json['ret_code']) {
        ...
        $house_info['content'] = $this->autocollect_model->con_flow(strip_tags($_json['data']['intro']));    //房源描述
  ...
  public function get_house($house_id, $block_info, $act)
  {
    ...
    $cityname = $this->signatory_info['cityname'];
    $signatory_id = $this->signatory_info['signatory_id'];
  ...
  public function send_house($data_house, $house_id)
  {
    ...
    if ($login['cookie']) {
      ...
      if (!empty($prj)) {
        ...
        $this->publish_count_num_model->info_count($this->signatory_info, 6, array('tbl' => $tbl, 'house_id' => $house_id));
        //记录 搜房楼盘
    ...
    try {
      ...
      if ($login['cookie']) {
        ...
        if ($msg) {
          ...
          $this->group_refresh_model->info_count($this->signatory_info, 6);
          //王欣统计
  ...
  public function check_login()
  {
    ...
    if (empty($username)) {
      $where = array('signatory_id' => $this->signatory_info['signatory_id'], 'site_id' => $this->site['id'], 'status' => 1);
      $site_signatory = $this->site_model->get_signatoryinfo_byids($where);
      if (empty($site_signatory[0])) return false;
      $username = $site_signatory[0]['username'];
      $otherpwd = $site_signatory[0]['otherpwd'];
  ...
  public function queue_refresh($queue)
  {
    ...
    if ($group[0]) {
      ...
      return parent::refresh($signatory_id, $house_id, $act, '', $queue_id);
    }
  }
  ...
  public function publish_param($queue)
  {
    ...
    $mass_signatory = $this->site_mass_model->isbind_site('ffw', $signatory_id);  //目标站点 帐号信息
    ...
    $post_fields = array('id' => $mass_signatory['user_id'], 'token' => $mass_signatory['cookies']);
