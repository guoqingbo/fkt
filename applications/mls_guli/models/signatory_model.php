 ...
 * @author          sun
 */
load_m("Signatory_base_model");

class Signatory_model extends Signatory_base_model
{
   ...
   */
  public function get_signatory_sms($type)
  {
    //引入用户SMS操作类，并初始化验证码的有效时长
    $this->load->model('signatory_sms_model');
    //验证码过期时间
    $this->signatory_sms_model->expiretime = $this->validcode_expiretime;
    //验证码类型
    $this->signatory_sms_model->type = $type;
    return $this->signatory_sms_model;
  }
  ...
  public function check_online()
  {
    ...
    //在线用户SESSION数据有值且id大于0
    if (isset($u_session['signatory_id']) && intval($u_session['signatory_id']) > 0) {
