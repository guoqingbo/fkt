...
class Cooperate_model extends Cooperate_base_model
{
  ...
  public function add_cooperate($data_arr)
  {
    ...
    $data = array(
      ...
      'signatoryid_a' => $data_arr['signatoryid_a'],
      'block_name' => $data_arr['block_name'],
      'signatory_name_a' => $data_arr['signatory_name_a'],
      'phone_a' => $data_arr['phone_a'],
      'agentid_b' => $data_arr['agentid_b'],
      'signatoryid_b' => $data_arr['signatoryid_b'],
      'signatory_name_b' => $data_arr['signatory_name_b'],
    ...
    if ($cid > 0) {
      ...
      $data = array(
        ...
        'signatory_a' => $data_arr['signatory_a'],
        'signatory_b' => $data_arr['signatory_b'],
        'step_time' => $time
      );
      ...
      $log_arr = array();
      $log_arr['cid'] = $cid;
      $log_arr['signatory_id'] = $data_arr['signatoryid_b'];
      $log_arr['step'] = $step;
      $log_arr['esta'] = $esta;
  ...
  /*
   * 读取申请合作的经纪人信息
   * @parames int $signatory_id
   * @return array
   */
  public function get_cooperate_signatory($signatory_id)
  {
    ...
    if ($signatory_id > 0) {
      ...
      //$signatory = $this->api_signatory_model->get_baseinfo_by_signatory_id($signatory_id);
      //echo '<pre>';print_r($signatory);die;
      if (!empty($signatory) && $signatory['signatory_id'] > 0) {
        $signatory_info['signatory_id'] = $signatory['signatory_id'];//编号
        $signatory_info['phone'] = $signatory['phone'];//电话号码
        $signatory_info['photo'] = $signatory['photo'];//头像
        $signatory_info['trust'] = $signatory['trust'];//信用值
        $signatory_info['credit'] = $signatory['credit'];//积分
        $signatory_info['cop_suc_ratio'] = $signatory['cop_suc_ratio'];//合作成功率
        $signatory_info['truename'] = $signatory['truename'];//真实姓名
        $signatory_info['department_id'] = $signatory['department_id'];//门店编号
        $signatory_info['department_name'] = $signatory['department_name'];//公司名称
        $signatory_info['company_id'] = $signatory['company_id'];//公司ID
      }
      unset($signatory);
    }
  ...
  public function get_cooperate_house($tbl, $rowid)
  {
    ...
    if ($rowid > 0) {
      ...
      if (isset($house) && !empty($house['id']) && $house['id'] > 0) {
        $house_info['tbl'] = $tbl;
        $house_info['signatory_id'] = $house['signatory_id'];
   ...
   *
   * @access    public
   * @param    int $signatory_id 发起合作的经纪人ID
   * @param    string $cond_where 查询条件
   * @return    int   符合条件的信息条数
   */
  public function get_cooperate_num_apply($signatory_id, $cond_where)
  {
    $cooperate_num = 0;
    $signatory_id = intval($signatory_id);
    $cond_where = "signatoryid_b = " . $signatory_id . " AND " . $cond_where;
   ...
   * @return    array   合作列表
   */
  public function get_cooperate_lists_apply($signatory_id, $cond_where, $offset = 0, $limit = 10,
   ...
   * @return    int   符合条件的信息条数
   */
  public function get_cooperate_num_accept($signatory_id, $cond_where)
  {
    $cooperate_num = 0;
    $signatory_id = intval($signatory_id);
    $cond_where = "signatoryid_a = " . $signatory_id . " AND " . $cond_where;
    $cooperate_num = parent::get_cooperate_num_by_cond($cond_where);
   ...
   * @return    array   合作列表
   */
  public function get_cooperate_lists_accept($signatory_id, $cond_where, $offset = 0, $limit = 10,
                                             $order_key = 'dateline', $order_by = 'DESC')
  {
    $cooperate_list = array();
   ...
   */
  public function check_is_cooped_by_houseid($houseid, $tbl, $signatory_id, $cond_where_cp = '')
  {
    $check_result = array();
    $signatory_id = intval($signatory_id); //经纪人编号
    $tbl = strip_tags($tbl); //房源类型
    $cond_where = '';

    //查询经纪人是否已经申请过该客源
    if (is_array($houseid)) {
      if (!empty($houseid) && $signatory_id > 0 && !empty($tbl)) {
        $houseid_str = implode(",", $houseid);
        $cond_where = "rowid IN(" . $houseid_str . ") AND tbl = '" . $tbl . "' "
          . "AND signatoryid_b = '" . $signatory_id . "' AND esta IN (1,2,3,4,7)";
      }
   ...
   */
  public function check_is_cooped_by_cid($cid, $tbl, $signatory_id, $cond_where_cp = '')
  {
    $check_result = array();
    $signatory_id = intval($signatory_id); //经纪人编号
    $tbl = strip_tags($tbl); //房源类型
    ...
    //查询经纪人是否已经申请过该客源
    if (is_array($cid)) {
      if (!empty($cid) && $signatory_id > 0 && !empty($tbl)) {
        $cid_str = implode(",", $cid);
   ...
   * @access    public
   * @param    int $cid 合作编号
   * @param    int $signatory_id 被申请经纪人编号
   * @param    boolean $escape 是否转义更新字段的值
   * @return    int 更新影响行数
   */
  public function accept_cooperation($cid, $signatory_id, $escape = TRUE)
  {
    $esta = 4;//状态
    $who_do = 3;//下步甲方操作
    $step = 3;//日志步骤
    ...
    $up_num = 0;
    $cond_where = "id = '" . $cid . "' AND signatoryid_a = '" . $signatory_id . "'";
    ...
    if ($up_num > 0) {
      ...
      $log_arr['step'] = $step;
      $log_arr['esta'] = $esta;
      $log_arr['signatory_id'] = $signatory_id;
      $log_arr['dateline'] = $time;
      $this->add_cooperate_log($log_arr);
    }
   ...
   * @return    int 更新影响行数
   */
  public function refuse_to_cooperation($cid, $signatory_id, $step, $refuse_reason, $escape = TRUE)
  {
    $step = intval($step);
    $esta = 5;
    $now_time = time();
    ...
    $cond_where = "id = '" . $cid . "' AND ( signatoryid_a = '" . $signatory_id . "' OR signatoryid_b = '" . $signatory_id . "') ";
   ...
   * @return    int 更新影响行数
   */
  public function cancle_cooperation($cid, $signatory_id, $cancle_arr, $escape = TRUE)
  {
    $now_time = time();
    $up_arr = array();
   ...
   * @return    int 更新影响行数
   */
  public function sub_allocation_scheme($cid, $signatory_id, $commission_arr, $escape = TRUE)
  {
    $step = 2;
    $now_time = time();
   ...
   * @return    int 更新影响行数
   */
  public function confirm_allocation_scheme($cid, $signatory_id, $escape = TRUE)
  {
    $up_num = 0;
    $up_arr = array();
    $up_arr['step'] = 3;
    ...
    $cond_where = "id = '" . $cid . "' AND signatoryid_b = '" . $signatory_id . "'";
    $up_num = parent::update_info_by_cond($up_arr, $cond_where, $escape);
   ...
   * @return    int 更新影响行数
   */
  public function sub_total_price($cid, $signatory_id, $esta, $total_price, $escape = TRUE)
  {
    $up_num = 0;
    $time = time();
   ...
   * @return    int 更新影响行数
   */
  public function sub_cooperate_result($cid, $signatory_id, $esta, $escape = TRUE)
  {
    $up_num = 0;
    $up_arr = array();
    $esta = intval($esta);
   ...
   * @param    int $signatoryid_a 被申请合作的经纪人ID
   * @param    int $signatoryid_b 发起合作的经纪人ID
   * @param    int $row_id 查询条件
   * @return    int  符合条件的合作信息条数
   */
  public function get_valid_cooperate_num($tbl, $signatoryid_a, $signatoryid_b, $row_id)
  {
    $cooperate_num = 0;
    $tbl = trim(strip_tags($tbl));
    $signatoryid_b = intval($signatoryid_b);
    $signatoryid_a = intval($signatoryid_a);
    $row_id = intval($row_id);

    $cond_where = "tbl = '" . $tbl . "' AND rowid = " . $row_id . " AND signatoryid_b = " . $signatoryid_b . " AND "
      . "signatoryid_b = " . $signatoryid_b . " AND esta IN (1,2,3,4,7)";
    $cooperate_num = parent::get_cooperate_num_by_cond($cond_where);
  ...
  public function stop_cooperate($houseid, $tbl, $stop_reason)
  {
    if ($houseid > 0 && !empty($tbl) && in_array($tbl, array('sell', 'rent'))) {
      ...
      if ($cooperate_num > 0) {
        ...
        $select_fields = array('id', 'order_sn', 'esta', 'tbl', 'rowid', 'signatoryid_a', 'signatory_name_a', 'signatoryid_b', 'signatory_name_b');
              ...
              if ($up_num > 0) {
                switch ($stop_reason) {
                  ...
                  case 'cancel_house':   //取消合作
                    ...
                    break;
                  case 'cop_deal_house':   //成交内房源成交
                    $message_type1 = '1-10-9';
                    break;
                }

                if ($message_type1 != '') {
                  $signatory_id = $value['signatoryid_b'];
                  $signatory_name = $value['signatory_name_b'];
                  ...
                  if ($esta == 4) {
                    $this->message_base_model->add_message($message_type4, $signatory_id, $signatory_name, $url, $params);
                  } else {
                    $this->message_base_model->add_message($message_type2, $signatory_id, $signatory_name, $url, $params);
                  }
                  if ($message_type2 != '') {
                    $signatory_id = $value['signatoryid_a'];
                    $signatory_name = $value['signatory_name_a'];
                    $url = '/cooperate/send_order_list/?cid=' . $value['id'];
                    if ($esta == 4) {
                      $this->message_base_model->add_message($message_type3, $signatory_id, $signatory_name, $url, $params);
                    } else {
                      $this->message_base_model->add_message($message_type1, $signatory_id, $signatory_name, $url, $params);
                    }
                  }
                }
              }
  ...
  */
  public function get_cooperation_by($rowid, $signatoryid_a, $agentid_a)
  {
    $sql = "select * from cooperate where rowid= '" . $rowid . "' and signatoryid_a = '" . $signatoryid_a . "' and agentid_a = '" . $agentid_a . "' and esta <= 4";
   ...
   * @param int $department_id 门店id
   * @return $result_arr 房源id
   */
  public function get_sell_house_cooperate_id_esta_1_by_department_id($department_id = 0)
  {
    ...
    if (!empty($department_id)) {
      ...
      $where_sql .= 'and sell_house.department_id = "' . intval($department_id) . '"';
   ...
   * @return $result_arr 房源id
   */
  public function get_rent_house_cooperate_id_esta_1_by_department_id($department_id = 0)
  {
    $this->dbselect('dbback_city');
    $where_sql = '';
    $result_arr = array();
    if (!empty($department_id)) {
      ...
      $where_sql .= 'and rent_house.department_id = "' . intval($department_id) . '"';
      $query = $this->db->query($where_sql);
      $result_arr = $query->result_array();
    }
    return $result_arr;
   ...
   */
  public function get_buy_customer_cooperate_id_esta_1_by_department_id($department_id = 0)
  {
    $this->dbselect('dbback_city');
    $where_sql = '';
    $result_arr = array();
    if (!empty($department_id)) {
      ...
      $where_sql .= ' where cooperate.tbl = "sell" and cooperate.step = 1 and cooperate.esta = 1 ';
      $where_sql .= 'and buy_customer.department_id = "' . intval($department_id) . '"';
      $query = $this->db->query($where_sql);
      $result_arr = $query->result_array();
    }
    return $result_arr;
   ...
   * @return $result_arr 客源id
   */
  public function get_rent_customer_cooperate_id_esta_1_by_department_id($department_id = 0)
  {
    $this->dbselect('dbback_city');
    $where_sql = '';
    $result_arr = array();
    if (!empty($department_id)) {
      ...
      $where_sql .= ' where cooperate.tbl = "rent" and cooperate.step = 1 and cooperate.esta = 1 ';
      $where_sql .= 'and rent_customer.department_id = "' . intval($department_id) . '"';
      $query = $this->db->query($where_sql);
      $result_arr = $query->result_array();
    }
    return $result_arr;
