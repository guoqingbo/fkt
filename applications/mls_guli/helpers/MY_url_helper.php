...
if ( ! function_exists('customer_modify_url'))
{
	function customer_modify_url($customer_id , $type = 'buy')
	{
        ...
        {
            $page_url = MLS_SIGN_URL.'/customer/modify/'.$customer_id;
        }
        else if($customer_id > 0 && $type == 'rent')
        {
            $page_url = MLS_SIGN_URL.'/rent_customer/modify/'.$customer_id;
        }
...
if ( ! function_exists('customer_publish_url'))
{
	function customer_publish_url($type = 'buy')
	{
        ...
        if( $type == 'buy')
        {
            $page_url = MLS_SIGN_URL.'/customer/publish';
        }
        else if($type == 'rent')
        {
            $page_url = MLS_SIGN_URL.'/rent_customer/publish';
        }
...
if ( ! function_exists('customer_details_url'))
{
	function customer_details_url($customer_id , $type = 'buy')
	{
        ...
        if($customer_id > 0 && $type == 'buy')
        {
            $page_url = MLS_SIGN_URL.'/customer/customer_detail/'.$customer_id;
        }
        else if($customer_id > 0 && $type == 'rent')
        {
            $page_url = MLS_SIGN_URL.'/rent_customer/customer_detail/'.$customer_id;
        }
