   ...
   <!--left 菜单部分-->
    ...
    <div class="forms_scroll2">
	    <div class="shop_tab_title scr_clear" id="js_search_box">
            <a href="javascript:void(0);" class="btn-lv fr" <?php if($auth['add']['auth']){?>onclick="$('#js_modify_box .iframePop').attr('src','/contract/modify_report_index/<?=$type?>');openWin('js_modify_box');"<?php }else{?>onclick="permission_none();"<?php }?>><span>+ 新增预约</span></a>
            <a href="/contract/report/1" class="link link_on"><span class="iconfont hide"></span>买卖</a>
<!--		<a href="/contract/report/2" class="link <?/*=$type==2?'link_on':''*/?>"><span class="iconfont hide"></span>出租</a>-->
	    </div>
	    <!-- 上部菜单选项，按钮-->
      ...
      <div class="fg_box">
        ...
        <div class="fg">
          <input type="text" name="agency_name_a" value="<?=$post_param['agency_name_a'];?>" class="input w120 ui-autocomplete-input" autocomplete="off"><span role="status" aria-live="polite" class="ui-helper-hidden-accessible"></span>
      ...
      <div class="fg_box">
        ...
        <div class="fg">
          <input type="text" name="broker_name_a" value="<?=$post_param['broker_name_a'];?>" class="input w120 ui-autocomplete-input" autocomplete="off"><span role="status" aria-live="polite" class="ui-helper-hidden-accessible"></span>
          <input name="broker_id_a" value="<?=$post_param['broker_id_a'];?>" type="hidden">
        </div>
      </div>
      <script type="text/javascript">
        $(function(){
          ...
          $("input[name='broker_name_a']").autocomplete({
		   ...
		   <!-- <div class="fg_box">
			...
			<div class="fg"> <a href="javascript:void(0);" onclick="$('#subform :input[name=page]').val('1');$('#subform').attr('action', '/contract/report/<?=$type?>/');form_submit();return false;" class="btn"><span class="btn_inner">搜索</span></a> </div>
<!--			<div class="fg"> <a href="javascript:void(0);" onclick="$('#subform').attr('action', '/contract/exportReport/<?/*=$type*/?>/');form_submit();$('#subform').attr('action', '');return false;" class="btn"><span class="btn_inner">导出</span></a> </div>-->
			<div class="fg"> <a href="/contract/report/<?=$type?>" class="reset">重置</a> </div>
		    </div>
		</form>
	    </div>
<script>
...
<script>
  ...
  function open_house(){
    ...
    var agency_id = childiframe.window.document.getElementById('contract_agency_add').value;
    var broker_id = childiframe.window.document.getElementById('contract_broker_add').value;
      if (agency_id > 0) {
          $('#js_house_box .iframePop').attr('src', '/contract/get_house/1/' + house_id + '?agency_id=' + agency_id + '&broker_id=' + broker_id);
          openWin('js_house_box');
      } else {
          openWin('js_pop_select_agency');
      }
  }
  ...
  function delete_this() {
    ...
    $.ajax({
      ...
      success: function (data) {
        if (data['result'] == 1) {
          $('#js_prompt1').text('预约已删除！');
          openWin('js_pop_success');
        } else {
          $('#js_prompt2').text('删除预约失败！');
  ...
  function agree_this() {
    ...
    $.ajax({
      ...
      success: function (data) {
        if (data['result'] == 1) {
          $('#js_prompt1').text('处理成功！');
          openWin('js_pop_success');
        } else {
          $('#js_prompt2').text('处理失败！');
          openWin('js_pop_false');
        }
      }
    })
  ...
  function disagree_this() {
    var contract_id = $('#contract_id').val();
    $.ajax({
      url: "/contract/disagree_report",
      type: "GET",
      dataType: "json",
      data: {
        id: contract_id
    ...
    $.ajax({
      ...
      success:function(data){
        if(data['result'] == 1){
          ...
          $('#bargain_id').val(data['bargain_id']);
          openWin('js_pop_report_success');
        }else{
    ...
    if(id){
      ...
      childiframe.window.document.getElementById('contract_houseid_add').disabled = false;
		...
		<div class="inner shop_inner" id="js_inner">
		    <table class="table" style="overflow-y: scroll; overflow-x:auto;">
          <tbody>
            ...
            <tr onclick="edit_report(<?=$val['id']?>);">
                ...
                <td class="c9" style="width:10%;">
                  <div class="info c227ac6" onclick="">
                    <?php if($val['house_id'] != '—') {?>
                      <a href="javascript:void(0);" onclick="$('#js_pop_box .iframePop').attr('src','/<?php echo $type == 1 ? 'sell' : 'rent';?>/details_house/<?=substr($val['house_id'],2);?>/4');openWin('js_pop_box');"><?=$val['house_id'];?></a>
                    <?php }else{?>
                      <?=$val['house_id'];?>
                    <?php }?>
                  </div>
                ...
                <td class="c8"><div class="info"><?=$val['broker_tel_a'];?></div></td>
                <td class="c8">
                <?php if($val['status'] == 1){?>
                ...
                <td>
              <?php if($val['status'] == 1){?>
                <a class="c1ab273" href="javascript:void(0)" <?php if($auth['edit']['auth']){?>onclick="$('#contract_id').val(<?=$val['id'];?>);openWin('js_pop_agree');"<?php }else{?>onclick="permission_none();"<?php }?>>同意</a>
              <?php }else{?>
                <span style="color:#b2b2b2;">同意</span>
              <?php }?>
              <?php if($val['status'] == 1){?>
                <a class="f00" href="javascript:void(0)" <?php if($auth['edit']['auth']){?>onclick="$('#contract_id').val(<?=$val['id'];?>);openWin('js_pop_disagree');"<?php }else{?>onclick="permission_none();"<?php }?>>不同意</a>
              <?php }else{?>
                <span style="color:#b2b2b2;">不同意</span>
              <?php }?>
              <!--<span style="margin:0 5px;color:#b2b2b2;"></span>--><br>
              <?php if(($val['status'] == 1 || $val['status'] == 2 ) && $val['is_check'] =='0'){?>
              <a href="javascript:void(0)" <?php if($auth['edit']['auth']){?>onclick="$('#js_modify_box .iframePop').attr('src','/contract/modify_report_index/<?=$type?>/<?=$val['id'];?>');openWin('js_modify_box');"<?php }else{?>onclick="permission_none();"<?php }?>>修改预约</a>
              <?php }else{?>
              <span style="color:#b2b2b2;">修改预约</span>
              <?php }?>
              <!--<span style="margin:0 5px;color:#b2b2b2;"></span>--><br>
              <?php if(($val['status'] == 1 || $val['status'] == 2 )){?>
                <a href="javascript:void(0)" <?php if($auth['report']['auth']){?>onclick="$('#contract_id').val(<?=$val['id'];?>);openWin('js_pop_cancel');"<?php }else{?>onclick="permission_none();"<?php }?>>取消预约</a>
              <?php }else{?>
                <span style="color:#b2b2b2;">取消预约</span>
              <?php }?>
              <!--<span style="margin:0 5px;color:#b2b2b2;"></span>--><br>
              <?php if(($val['status'] == 1 || $val['status'] == 2 )){?>
                <a href="javascript:void(0)" <?php if($auth['report']['auth']){?>onclick="$('#contract_id').val(<?=$val['id'];?>);$('#number').text('<?=$val["number"];?>');openWin('js_pop_report');"<?php }else{?>onclick="permission_none();"<?php }?>>转正处理</a>
              <?php }else{?>
                <span style="color:#b2b2b2;">转正处理</span>
              <?php }?>
              <br>
              <?php /*if($auth['delete']['auth'] && $val['status'] != 6){*/?><!--
                <a href="javascript:void(0)" <?php /*if($auth['delete']['auth']){*/?>onclick="$('#contract_id').val(<?/*=$val['id'];*/?>);openWin('js_pop_del');"<?php /*}else{*/?>onclick="permission_none();"<?php /*}*/?>>删除</a>
              <?php /*}else{*/?>
                <span style="color:#b2b2b2;">删除</span>
              --><?php /*}*/?>
                  <input type="hidden" id="contract_id" value="">
                    <input type="hidden" id="bargain_id" value="">
              </td>
            </tr>
            <?php }}else{?>
...
<!--			<button  type="button" class="btn-lv1 btn-left JS_Close zws_btn_bg_w93" onclick="openWin('js_pop_cancel');">转为正式合同</button>-->
	    ...
	    <div class="up_inner">
                <table class="del_table_pop">
                    <tr>
                        ...
                        <td>
			    <p class="left" style="color:#666;">预约删除后不可恢复,是否确认删除？</p>
                        </td>
                    </tr>
                </table>
...
<!--未选择门店提示框-->
<div class="pop_box_g pop_see_inform pop_no_q_up" style="display:none;" id="js_pop_select_agency">
    <div class="hd">
        <div class="title">提示</div>
        <div class="close_pop"><a href="javascript:void(0);" title="关闭" class="JS_Close iconfont"></a></div>
    </div>
    <div class="mod">
        <div class="inform_inner">
            <div class="up_inner">
                <table class="del_table_pop">
                    <tr>
                        <td class="c14" valign="top"><img alt=""
                                                          src="<?php echo MLS_SOURCE_URL; ?>/mls_guli/images/v1.0/s_ico.png">
                        </td>
                        <td>
                            <p class="left" style="color:#666;">您未选择门店，请先选择门店</p>
                        </td>
                    </tr>
                </table>
            </div>
      ...
      <div class="up_inner">
        <table class="del_table_pop" style="padding-left:47px;width:auto;">
          <tr>
            <td class="c14" valign="top"><img alt="" src="<?php echo MLS_SOURCE_URL;?>/mls_guli/images/v1.0/s_ico.png"></td>
            <td>
              <p class="left" style="color:#666;">预约<span id="number"></span>将转换为正式成交合同？</p>
            </td>
          </tr>
        </table>
        <button class="btn-lv1 btn-left JS_Close" type="button" onclick="report_this();">确定</button>
        <button class="btn-hui1 JS_Close" type="button">取消</button>
      </div>
    </div>
  </div>
...
<!--预约转正处理成功提示框-->
<div class="pop_box_g pop_see_inform pop_no_q_up" style="display:none;" id="js_pop_report_success">
  <div class="hd">
    <div class="title">提示</div>
      ...
      <div class="up_inner">
        <table class="del_table_pop">
          <tr>
            <td class="c14" valign="top"><img alt="" src="<?php echo MLS_SOURCE_URL;?>/mls_guli/images/v1.0/s_ico.png"></td>
            <td>
              <p class="left" style="color:#666;">预约<span id="number"></span>已转为成交合同！</p>
              <p class="left" style="color:#666;width:100%;">是否去添加成交信息？</p>
            </td>
          </tr>
        </table>
          <button class="btn-lv1 btn-left JS_Close" type="button"
                  onclick="location.href='/bargain/modify_bargain/2/'+$('#bargain_id').val();return false;">去添加
          </button>
...
<!--预约取消提示框-->
<div class="pop_box_g pop_see_inform pop_no_q_up" style="display:none;" id="js_pop_cancel">
    <div class="hd">
        <div class="title">提示</div>
        <div class="close_pop"><a href="javascript:void(0);" title="关闭" class="JS_Close iconfont" onclick="check_list(<?=$page?>,<?=$type?>)"></a></div>
    </div>
    <div class="mod">
	    ...
	    <div class="up_inner">
                ...
                <button class="btn-lv1 btn-left JS_Close" type="button" onclick="cancel_this();">确定</button>
                <button class="btn-hui1 JS_Close" type="button" onclick="check_list(<?=$page?>,<?=$type?>)">取消</button>
            </div>
         </div>
    </div>
...
<!--同意预约提示框-->
<div class="pop_box_g pop_see_inform pop_no_q_up" style="display:none;" id="js_pop_agree">
  <div class="hd">
    <div class="title">提示</div>
    <div class="close_pop"><a href="javascript:void(0);" title="关闭" class="JS_Close iconfont" onclick="check_list(<?=$page?>,<?=$type?>)"></a></div>
  </div>
      ...
      <div class="up_inner">
        <table class="del_table_pop" style="padding-left:47px;width:auto;">
          <tr>
            <td class="c14" valign="top"><img alt="" src="<?php echo MLS_SOURCE_URL;?>/mls_guli/images/v1.0/s_ico.png"></td>
            <td>
              <p class="left" style="color:#666;"> 同意本次预约？</p>
            </td>
          </tr>
        </table>
        <button class="btn-lv1 btn-left JS_Close" type="button" onclick="agree_this();">确定</button>
        <button class="btn-hui1 JS_Close" type="button" onclick="check_list(<?=$page?>,<?=$type?>)">取消</button>
      </div>
    </div>
  </div>
...
<!--不同意预约提示框-->
<div class="pop_box_g pop_see_inform pop_no_q_up" style="display:none;" id="js_pop_disagree">
  <div class="hd">
    <div class="title">提示</div>
    <div class="close_pop"><a href="javascript:void(0);" title="关闭" class="JS_Close iconfont" onclick="check_list(<?=$page?>,<?=$type?>)"></a></div>
  </div>
      ...
      <div class="up_inner">
        <table class="del_table_pop" style="padding-left:47px;width:auto;">
          <tr>
            <td class="c14" valign="top"><img alt="" src="<?php echo MLS_SOURCE_URL;?>/mls_guli/images/v1.0/s_ico.png"></td>
            <td>
              <p class="left" style="color:#666;"> 不同意本次预约？</p>
            </td>
          </tr>
        </table>
        <button class="btn-lv1 btn-left JS_Close" type="button" onclick="disagree_this();">确定</button>
        <button class="btn-hui1 JS_Close" type="button" onclick="check_list(<?=$page?>,<?=$type?>)">取消</button>
      </div>
    </div>
  </div>
...
<!--预约取消成功提示框-->
<div class="pop_box_g pop_see_inform pop_no_q_up" style="display:none;" id="js_pop_cancel_success">
    <div class="hd">
        <div class="title">提示</div>
        <div class="close_pop"><a href="javascript:void(0);" title="关闭" class="JS_Close iconfont" onclick="check_list(<?=$page?>,<?=$type?>)"></a></div>
    </div>
...
<!--添加和修改弹框-->
<div id="js_modify_box" class="iframePopBox" style="width: 580px;height:422px;">
    <a class="JS_Close close_pop iconfont" href="javascript:void(0)" date-iframe="1">&#xe60c;</a>
    <iframe frameborder="0" scrolling="no" width="580" height="422px" class='iframePop' src="" name="childiframe"></iframe>
</div>
