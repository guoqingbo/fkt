<div class="achievement_money_pop zws_second_w">
    ...
    <!-- 上部菜单选项，按钮-->
    <div>
    <div class="search_box clearfix zws_position " id="js_search_box_02" style="padding:0 0 10px 0;width: 100%;">
      ...
      <div class="fg_box">
        ...
        <div class="fg_box">
          <p class="fg zws_input_p">门店：</p>

          <div class="fg mr10" style="*padding-top:10px;">
            <input type="text" name="agency_name" value="<?=$post_param['agency_name'];?>" class="input w90 ui-autocomplete-input" autocomplete="off"><span role="status" aria-live="polite" class="ui-helper-hidden-accessible"></span>
            <input name="agency_id" id="agency_id" value="<?=$post_param['agency_id'];?>" type="hidden">
            <script type="text/javascript">
              $(function(){
                $.widget( "custom.autocomplete", $.ui.autocomplete, {
                  _renderItem: function( ul, item ) {
                ...
                $("input[name='agency_name']").autocomplete({
                  source: function( request, response ) {
                    var term = request.term;
                  ...
                  select: function(event,ui) {
                    if(ui.item.id > 0){
                      ...
                      $("input[name='agency_name']").val(agencyname);
                      removeinput = 2;
                    }else{
                      removeinput = 1;
                    }
          ...
          <p class="fg zws_input_p">预约人：</p>
          <div class="fg mr10" style="*padding-top:10px;">
            <input type="text" name="broker_name" value="<?=$post_param['broker_name'];?>" class="input w90 ui-autocomplete-input" autocomplete="off"><span role="status" aria-live="polite" class="ui-helper-hidden-accessible"></span>
            <input name="broker_id" id="broker_id" value="<?=$post_param['broker_id'];?>" type="hidden">
            <script type="text/javascript">
              $(function(){
                $.widget( "custom.autocomplete", $.ui.autocomplete, {
                  _renderItem: function( ul, item ) {
                ...
                $("input[name='broker_name']").autocomplete({
                  source: function( request, response ) {
                    var term = request.term;
                    var contract_agency_add = $("input[name='agency_id']").val();
                    $.ajax({
                  ...
                  select: function(event,ui) {
                    if(ui.item.id > 0){
                      ...
                      //操作
                      $("input[name='broker_id']").val(id);
                      $("input[name='broker_name']").val(brokername);
                  ...
                  close: function(event) {
                    if(typeof(removeinput)=='undefined' || removeinput == 1){
                      $("input[name='broker_id']").val("");
                      $("input[name='broker_name']").val("");
                    }
                  }
                });
              });
	    ...
	    <div class="fg"> <a href="javascript:void(0)" onclick="$('#search_form').submit();return false;" class="btn"><span class="btn_inner">搜索</span></a> </div>
            <div class="fg"><a href="/contract/get_house/1" class="reset">重置</a></div>
        </div>
