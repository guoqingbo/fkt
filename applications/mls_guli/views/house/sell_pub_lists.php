...
//                        $("#block_id").val("");
//                        $.ajax({
//                            url: "<?php echo MLS_SIGN_URL;?>/community/get_cmtinfo_by_kw/",
//                            type: "GET",
//                            dataType: "json",
//                            data: {
//                                keyword: term
    ...
    <div  id="js_innerHouse" class="inner" style="overflow-y: scroll; *position:relative;height:331px;">
		...
		<?php
            if($list)
            {
                ...
                {
            ?>
            <tr info_id = "<?php echo $val['id'];?>" <?php if($key % 2 == 1){ ?>class="bg" <?php }?> id="tr<?php echo $val['id'];?>" date-url="<?php echo MLS_SIGN_URL;?>/sell/details_house/<?php echo $val['id'];?>/2" controller="sell" _id="<?php echo $val['id'];?>" min_title="<?php echo $val['block_name'].'-'.intval($val['price']).'万'.'-'.intval($val['buildarea']).'平米'; ?>">
				<td class="c3">
                    <div class="info">
