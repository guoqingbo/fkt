        ...
        <div class="inner inner02">
            ...
            <div class="inner_ky_box">
			 <form action="" method="post" id="myform">
                <div class="title">客户姓名：<input type="text" class="input_t" name="cname" value="<?php echo empty($cname)?'可搜索姓名或电话':$cname; ?>" onfocus="Onfocus()" onblur="Onblur()" id="search"><button class="btn" onclick="$('#myform').submit();">查询</button>                </div>
			</form>
                <table class="table">
                    <tr>
                        <th class="w45">&nbsp;</th>
				 ...
				 ?>
                    <tr class="bg">
                        ...
                        <td>租</td>
                        <td><?php echo 'QZ'.$var['id']?></td>
                        <td ><?php echo $var['truename']?></td>
                        <td><?php echo strip_end_0($var['price_min'])?>-<?php echo strip_end_0($var['price_max'])?>元/月</td>
                    </tr>
				 <?php } }else{?>
			...
			<?php }?>
            <a class="btn-lv1 btn-mid JS_Close mt10" onclick="opensource(<?php echo $type; ?>)"  href="javascript:void(0)">确定</a>
		</div>
    </div>
</div>
