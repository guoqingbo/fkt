...
<div class="pop_box_g" id="js_genjin" style="display:block;border:none;">
    ...
    <div class="mod">
        <div class="mod_zn_inner">
            <h3 class="title">房源写跟进<span class="text">(<?php echo $house_info['block_name'].' '.$house_info['district_name'].'-'.$house_info['street_name'].' '.'/'.strip_end_0($house_info['buildarea']).'㎡'.'/'.strip_end_0($house_info['price']).'元/月'.'/'.$house_info['room'].'室'.$house_info['hall'].'厅'.$house_info['toilet'].'卫/'.$config['forward'][$house_info['forward']].'/'.$config['fitment'][$house_info['fitment']].'/'.$house_info['buildyear']; ?>)</span></h3></span></h3>
            <div class="inner inner02">
                ...
                <div class="item_fg_h clearfix" id="qing">
                    ...
                    <div class="left" id="kename">

                    </div>
                </div>
				 <input type="hidden" value="<?php echo $house_id?>" id="house_id" name="house_id"/>
            ...
            <a class="save_btn"  href="javascript:void(0)" onclick="save(1)">确定</a> </div>
    </div>
	</div>
	<script type="text/javascript">
	function save(foll_type){
    ...
    <div class="mod">
        <div class="inform_inner">
            <div class="up_inner">
				 ...
				 <button type="button" id = 'dialog_share' class="btn-lv1 btn-mid JS_Close" onclick="$('#search_form').submit();return false;">确定</button>
            </div>
        </div>
    </div>
</div>
