...
<div class="search_box clearfix" id="js_search_box_02"> <a href="javascript:void(0)" class="s_h" onClick="show_hide_info(this , 'rent_pub_list_extend')" data-h="0" id="extend">更多<span class="iconfont">&#xe609;</span></a>
    <div class="fg_box">
        <p class="fg fg_tex">时间：</p>
        <div class="fg">
            <select class="select" id="searchtime" name="set_share_time">
                <option value="1" <?php if($post_param['set_share_time'] == 1){echo "selected";}?>>一个月内</option>
                <option value="2" <?php if($post_param['set_share_time'] == 2){echo "selected";}?>>一季内</option>
                <option value="3" <?php if($post_param['set_share_time'] == 3 || empty($post_param['set_share_time'])){echo "selected";}?>>半年内</option>
                <option value="4" <?php if($post_param['set_share_time'] == 4){echo "selected";}?>>一年内</option>
                <option value="5" <?php if($post_param['set_share_time'] == 5){echo "selected";}?>>一年以上</option>
            </select>
        </div>
    </div>
    <div class="fg_box">
    ...
    <div class="fg_box">
        ...
        <div class="fg">
            <select class="select" id='district' name='district' onchange="districtchange(this.value,'rent');">
			   <option value='0'>不限</option>
	...
	<?php if($friend == 'friend' || $friend == 'manage'){?>
	<div class="fg_box">
...
//            });
        ...
        {
            $.ajax({
                url: "/rent/del_search_cookie/rent_lists_pub",
                type: "POST",
                dataType: "json",
                success: function(data) {
...
<!--<div class="fun_btn clearfix" id="js_fun_btn">
	<label class="btn btn_del"><input type="checkbox" id="js_checkbox" name="items">全选</label>
...
<div class="table_all">
     <div class="title" id="js_title">
        <table class="table">
            <tr>
                <td class="c5"><div class="info">特色</div></td>
                ...
                <td class="c7"><div class="info"><a href="javascript:void(0)" onclick="renntlist_order(7);return false;" id="order_price" class="i_text <?php if($post_param['orderby_id'] == 8 ){ echo 'i_down'; }elseif($post_param['orderby_id'] == 7){ echo 'i_up'; } ?>">租金<br>
                        </a></div></td>
    ...
    <div  id="js_innerHouse" class="inner" style="overflow-y: scroll; *position:relative;height:331px;">
        ...
        if($list)
        {
            ...
            {
        ?>
        <tr info_id = "<?php echo $val['id'];?>" <?php if($key % 2 == 1){ ?>class="bg" <?php }?> id="tr<?php echo $val['id'];?>" date-url="/rent/details_house/<?php echo $val['id'];?>/2" controller="rent" _id="<?php echo $val['id'];?>" min_title="<?php echo $val['block_name'].'-'.intval($val['price']).'万'.'-'.intval($val['buildarea']).'平米'; ?>">
                <td class="c5">
                    <div class="info">
                ...
                <td class="c7"><div class="info f13 fblod"><?php echo strip_end_0($val['price']); ?><?php echo ('1'==$val['price_danwei'])?'元/㎡*天':'元/月';?></div></td>
                <td class="c8 js_info broker" data-brokerId ="<?php echo $val['broker_id'];?>" data_id="<?php echo $val['id'];?>" type="rent_house"><div class="info"><?php echo $val['broker_name']; ?><!--<span class="onper iconfont">&#xe616;</span>--></div></td>
                <!--
                <td class="c5">
                    <div class="info">
                    <?php
                ...
                <div class="info_p_r">
				<?php if($friend == 'manage'){?>
				<a href="javascript:void(0)" onclick = "sharecancel(0,'rent',<?php echo $val['id'];?>,'friend');" class="js_input_7">从朋友圈下架</a>
				<?php }else{?>
                ...
                <a href="javascript:void(0)" title = "当前公司未设置合作中心" style="color:#b2b2b2;text-decoration:none;">合作申请</a>
                <?php } else { ?>
                ...
                <a href="javascript:void(0)" onclick="open_match('rent',1,<?php echo $val['id'];?>)">智能匹配</a>
                <span style="margin:0 2px;color:#b2b2b2;">|</span>
                ...
                ?>
                <a href="javascript:void(0)" class="shcang" id = "cang<?php echo $val['id']?>"onclick="shcang('house_collect','rent_house',<?php echo $val['id'] ?>)">收藏</a>
                <?php
                }
                ?>
    ...
    <script type="text/javascript">
		...
		$(function(){
            ...
            //'收起'，‘更多’按钮，获得cookie值
            var rent_pub_list_extend = getCookie('rent_pub_list_extend');
            if(1==rent_pub_list_extend){
    ...
    <div class="zws_bottom_nav" style="margin-top:2px;">
        <div class="zws_bottom_nav_dao">
            <ul id="window_min">
                ...
                <input type="hidden" value="<?php echo '/rent/details_house/'.$v['house_id'].'/2' ?>"/>
