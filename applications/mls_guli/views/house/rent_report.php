<!--我要举报-->
<div class="pop_box_g report_box" id="js_woyaojubao" style="display:block; border:none;">
	<div class="hd">
    ...
    <div class="report_box">
		...
		<p class="left" style="line-height:20px;">
    		为了共同打造真实可靠的合作平台，举报经核实后将奖励您一定的积分

		</p>
    	</div>
    	<form name="fileform_head" id="fileform_head" action="/rent/add_report/" enctype="multipart/form-data" method="post">
    		<table class="table report_table">
    			<tr class="retrbg">
					...
					<input type="hidden" value="<?php echo $house_id ?>" name="hosue_id"/>
			        <input type="hidden" value="<?php echo $broker_id ?>" name="broker_id"/>
	...
	$.ajax({
		...
		success:(function(data){
			if(data['select_num']>0){
				 $("#dialog_do_warnig_tip").html("该房源的类型你已经举报过了");
				 openWin('js_pop_do_warning');
				 return false;
			}
			if(data['brokerinfo_id']==data['broker_id']){
				 $("#dialog_do_warnig_tip").html("不能对自己的房源举报");
				 openWin('js_pop_do_warning');
				 return false;
			}
			if(data['return_id']>0){
