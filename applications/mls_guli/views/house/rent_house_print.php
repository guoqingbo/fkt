...
<div class="house_info_print">
    ...
    <!--打印信息-->
    <form action="<?php echo MLS_SIGN_URL;?>/rent/pic_deal" method="post" target="_blank">
    ...
    <div class="house_info_print_con">
        ...
        <div class="house_info_print_con_left">
            <dl>
                <dd>样式：</dd>
                <dt id="tab_v_h">
                    <p class="house_info_print_con_left_p"><input type="radio" checked name="type" value="3" /><b>横向</b></p>
                    <p class="house_info_print_con_left_p"><input type="radio" name="type" value="4" /><b>竖向</b></p>
        ...
        <div class="house_info_print_con_right">
            ...
            <div class="print_con_right_horizontal">
                ...
                <div class="horizontal_con_l">
                    <span class="horizontal_con_name">
                        ...
                        <p class="pcolor_n price_pic"><?php echo intval($house_details['price']);?>元/月</p>
                        <input type="hidden" value="<?php echo intval($house_details['price']);?>元/月" name="price"/>
                    </span>
            ...
            <div class="print_con_right_vertical">
                ...
                <div class="vertical_con_date">
                    ...
                    <span class="horizontal_con_date_line padd_H broker_name_pic">经纪人 <?php echo $house_details['broker_name'];?> <?php echo $house_details['broker_phone'];?><img id="qrcode_shu" style="width: 25px;height:25px;padding-left:4px;" src="<?php echo $house_details['qrcode']; ?>"/></span>
                </div>
                <!--图形部分-->
...
$(function(){
    ...
    //微店是否展示
    $('input[name="is_qrcode"]').on('click',function(){
        var is_qrcode = $(this).val();
        if(2==is_qrcode){
            $('#qrcode').hide();
        }else{
            $('#qrcode').show();
    ...
    $('#print_click').on('click',function(){
        var cmt_name = $('.cmt_name_pic').html();//楼盘名
        var price = $('.price_pic').html();//价格
        var buildarea = $('.buildarea_pic').html();//面积
        var room_pic = $('.room_pic_pic').html();//户型
        var fitment = $('.fitment_pic').html();//装修
        var forward = $('.forward_pic').html();//朝向
        var broker_name = $('.broker_name_pic').html();//经纪人信息
        var remark = $('.remark_pic').html();//备注
        var agency_name = $('.agency_name_pic').html();//门店名
        var shinei = $('#shineipic').attr('src');//室内图
        var huxing = $('#huxingpic').attr('src');//户型图

        $.ajax({
           url: "<?php echo MLS_SIGN_URL;?>/rent/pic_deal",
           type: "GET",
           //dataType: "json",
           data:{
               'cmt_name': cmt_name,
