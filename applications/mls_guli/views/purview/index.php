...
<div class="limit-set">
	<div class="limit-right" id="content" style="overflow-y:auto;position:relative;">
        <!--			<a class="zws_power_btn" href="javascript:void(0);" onclick = "$('#js_rule_pop .iframePop').attr('src','/purview/purview_rule');openWin('js_rule_pop');"><span>权限说明</span></a>-->
        <a class="btn-lan right"
           href="/purview/set_group_func/<?php echo str_replace(",", '-', $user_arr['role_id']); ?>/<?php echo $user_arr['department_id']; ?>"><span>角色配置</span></a>

	</div>
	<div class="limit-left">
        <select name="store_name" class="select" id="store_name">

            <option value="no">不限</option>

            <?php if ($agency) {
                foreach ($agency as $key => $val) { ?>
                    <option value="<?php echo $val['id']; ?>" <?php if ($store_name == $val['store_name']) {
                        echo selected;
                    } ?>><?php echo $val['store_name']; ?></option>
                <?php }
            } ?>
		...
		<ul id="js_left">
                    <?php if($list){foreach($list as $key =>$val) {?>
                        <li id="group_list">
                            <a title="<?php echo $val['role_name'] ?>" href="javascript:void(0);"
                               onclick="get_company_func('<?php echo $val['role_id']; ?>');"
                               id="func_link<?php echo $val['role_id']; ?>"
                               name="<?php echo $val['role_level']; ?>"><?php echo $val['truename']; ?>
                            </a>
                        </li>
                    <?php }}?>
		</ul>
...
<script>
        ...
        $("#store_name").change(function(){
            var department_id = $("#store_name").val();
		$.ajax({
            url: "<?php echo MLS_SIGN_URL;?>/purview/get_group",
                    type:"GET",
                    dataType:"json",
                    data:{
                    ...
                    success:function(data){
                        ...
                        if(data && data.length > 0){
                           for(var i in data){
                               html += "<li><a title='" + data[i]['role_name'] + "' href='javascript:void(0);' onclick='get_company_func('" + data[i]['role_id'] + "');' id='func_link" + data[i]['role_id'] + "' name ='" + data[i]['role_level'] + "'>" + data[i]['truename'] + "</a></li>";
                           }
                           $("#js_left").html(html);
        ...
        function get_company_func(role_id) {
            $("#content").children("div").remove();
            $.ajax({
                url: "<?php echo MLS_SIGN_URL;?>/purview/get_company_func",
                type: "POST",
                    dataType:"json",
                    data:{
                        isajax: 1,
