...
<div class="tab_box" id="js_tab_box">
    <?php echo $user_menu;?>
    <a style="float:right; margin-right:10px;" class="btn-lv" href="/purview/index"><span>&lt;&lt;返回角色权限</span></a>
</div>
<!--主要内容-->
                    ...
                    <?php foreach($group as $key => $val) {?>
                        <li>
                            <a href="javascript:void(0);"
                               onclick="get_system_func(<?php echo $val['system_group_id']; ?>,<?php echo $val['id']; ?>);return false;"
                               id="func_link<?php echo $val['id']; ?>" name="<?php echo $val['system_group_id'] ?>">
                                <?php echo $val['name'] ?>
                                <span style="height: 30px;width: 30px;" class="l-edit"></span>
                            </a>
                        </li>
                    <?php }?>
		</ul>
...
<script>
	...
	$(function () {
                ...
                $('input[name="save"]').live('click',function(){
                    ...
                    $.ajax({
                        url: "<?php echo MLS_SIGN_URL;?>/purview/save_button_submit/",
                            type:"GET",
                            dataType:"json",
                            data:send_data,
        ...
        function get_system_func($sid,$role_id){
            $.ajax({
                url: "<?php echo MLS_SIGN_URL;?>/purview/get_system_group/" + $sid,
                    type:"GET",
                    dataType:"json",
                    data:{
                       isajax:1
                    ...
                    success:function(data){
                       ...
                       $("#content").html(html);
                       get_group_func($role_id);
                       choose_authority();
                    }
                });
        }
        function get_group_func($id){
            $.ajax({
                url: "<?php echo MLS_SIGN_URL;?>/purview/get_group_func/" + $id,
                    type:"GET",
                    dataType:"json",
                    data:{
                       isajax:1
        ...
        function submit_agency_per_area_form()
        {
            ...
            $.ajax({
                type: "POST",
                url: "/purview/save_button_submit_acency/",
    ...
    $("#js_left li").hover(function () {
        $(this).find(".l-edit").toggleClass("display-block");
    });
    //门店板块对应id获取
    $("#js_left li a span").click(function (event) {
        event.stopPropagation();
        var system_group_id = $.trim($(this).parent().attr('name'));
        var role_name = $.trim($(this).parent().text());
    ...
    function modify_role() {
        ...
        var data = {role_name: role_name, system_group_id: system_group_id};
        $.ajax({
            type: "POST",
            url: "/purview/modify_role_name",
            dataType: "json",
            data: data,
            error: function () {
                $("#dialog_do_warnig_tip").html("系统错误");
...
<div class="pop_box_g pop_box_add_shop" id="js_r_shop">
    <div class="hd">
        <div class="title">修改角色名称</div>
        <div class="close_pop"><a class="JS_Close iconfont" title="关闭" href="javascript:void(0);"></a></div>
    </div>
    <div class="mod">
        <label class="label clearfix"><span class="text">角色名称：</span><input class="text_input" id="role_name" value=""
                                                                            type="text"></label>

        <input type="hidden" value="" id="system_group_id">
        <button class="btn-lv1 btn-mid" style="margin-top:10px;" type="button" onclick="modify_role()">保存</button>
    </div>
</div>
...
<div id="js_pop_do_success" class="pop_box_g pop_see_inform pop_no_q_up" style="display: block">
    <div class="hd">
        ...
        <div class="close_pop">
            <a href="#" onclick="sub_form();" title="关闭" class="JS_Close iconfont"></a>
        </div>
    </div>
    <div class="mod">
...
<div class="pop_box_g pop_box_add_shop" id="js_set_per_agency_area" style="width:600px;height:300px;">
	...
	<form name="agency_per_area_form" id="agency_per_area_form" method="post">
        ...
        <div style="position:relative; text-align:center; width:100%;"><label
                    style="position:absolute; left:13px; top:0;"><input type="checkbox" onclick="checkallagency(this);">
                全选</label><a title="根据勾选设置关联指定部门" class="btn-lv btn-left" style="padding-left: 10px;"
                             href="javascript:void(0)" onclick="submit_agency_per_area_form();"><span class="btn_inner"
                                                                                                      style="padding-right: 10px;">保存设置</span></a><a
                    class="btn-hui1 JS_Close" href="/purview/index/"><span>取消</span></a></div>
	</form>
    </div>
</div>


<img id="mainloading" src="<?php echo MLS_SOURCE_URL;?>/mls_guli/images/v1.0/009.gif"><!--遮罩 loading-->

<script src="<?php echo MLS_SOURCE_URL;?>/min/?b=mls_guli/js/v1.0&f=openWin.js,house.js,backspace.js,calculate.js"></script>
