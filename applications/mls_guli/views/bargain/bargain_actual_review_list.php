...
<div class="bargain-wrap clearfix">
	...
	<div class="forms_scroll2">
		<!-- 上部菜单选项，按钮-->
		<div class="search_box clearfix" style="margin-top:0;" id="js_search_box_02">
			<form name="search_form" id="subform" method="post" action="">
				...
				<div class="fg_box">
					...
					<div class="fg mr10" style="*padding-top:10px;">
						<select class="select" name="entry_department_id" id="entry_department_id">
                            <?php foreach($departments as $val){?>
                                        <option value="<?=$val['id'];?>" <?php if ($val['id'] == $post_param['entry_department_id']) {echo 'selected';}?>><?=$val['name'];?></option>
                                <?php }?>
						</select>
					</div>
				</div>
				<div class="fg_box">
					...
					<div class="fg mr10" style="*padding-top:10px;">
						<select class="select" name="entry_signatory_id" id="entry_signatory_id">
                            <?php  foreach($signatorys as $val){?>
                                <option value="<?=$val['signatory_id'];?>" <?php if ($val['signatory_id'] == $post_param['entry_signatory_id']) {echo 'selected';}?>><?=$val['truename'];?></option>
                                <?php  }?>
						</select>
					</div>
				</div>
                <script>
                $("#entry_department_id").change(function(){
                    ...
                    $.ajax({
                        ...
                        if(data['result'] == 1){
                            ...
                            $('#entry_signatory_id').append(html);
                        }
                        }
                    })
                    }else{
                    $('#entry_signatory_id').html("<option value=''>请选择</option>");
                    }
                })
                </script>
				<div class="fg_box">
				...
				<div class="fg_box">
					...
					<div class="fg"> <a href="/bargain_flow/replace_review_list" class="reset">重置</a> </div>
				</div>
			</form>
		</div>
<script>
			...
			<div class="inner shop_inner" id="js_inner">
				<table class="table"  style="*+width:98.5%;*+padding:0 1.5% 0 0;_width:98.5%;_padding:0 1.5% 0 0;">
					...
					<tr class="<?=$key%2==1?'bg':''?>">
                        ...
                        <td class="c10"><div class="info c227ac6"><a href="/bargain/bargain_detail/<?=$val['id'];?>"><?=$val['number'];?></a></div></td>
                        ...
                        <td class="c6"><div class="info"><?=$val['entry_department_name'];?></div></td>
                        <td class="c6"><div class="info"><?=$val['entry_signatory_name'];?></div></td>
                        ...
                        <td class="c10">
                            <?php if($val['flow_status']==0){?>
                            <a href="javascript:void(0);" <?php if($auth['review']['auth']){?>onclick="$('#flow_id').val(<?=$val['f_id'];?>);$('#review_type').val('1');openWin('js_review_pop');"<?php }else{?>onclick="purview_none();"<?php }?>>通过</a>&nbsp;&nbsp;|&nbsp;&nbsp;
                            <a href="javascript:void(0);" <?php if($auth['review']['auth']){?>onclick="$('#flow_id').val(<?=$val['f_id'];?>);$('#review_type').val('2');openWin('js_review_pop');"<?php }else{?>onclick="purview_none();"<?php }?>>拒绝</a>
                            <?php }elseif($val['flow_status']==1){?>
                            <a href="javascript:void(0);" <?php if($auth['fanreview']['auth']){?>onclick="$('#flow_id').val(<?=$val['f_id'];?>);openWin('js_cancel_review_pop');"<?php }else{?>onclick="purview_none();"<?php }?>>反审核</a>
                            <?php }?>
                        </td>
					</tr>
				    <?php }}else{?>
...
<div id="js_review_pop" class="pop_box_g pop_see_inform pop_no_q_up" >
	...
	<div class="inform_inner">
	    <div class="up_inner">
		<p class="text"><img alt="" src="<?php echo MLS_SOURCE_URL;?>/mls_guli/images/v1.0/s_ico.png">&nbsp;&nbsp;审核完成后应收应付信息将不可修改,<br/>是否确定当前操作？</p>
...
<div id="js_cancel_review_pop" class="pop_box_g pop_see_inform pop_no_q_up" >
	...
	<div class="inform_inner">
	    <div class="up_inner">
		<p class="text"><img alt="" src="<?php echo MLS_SOURCE_URL;?>/mls_guli/images/v1.0/s_ico.png">&nbsp;&nbsp;反审核后该信息将可修改，是否确定此操作</p>
...
$(function () {
    ...
    function cancel_this(){
        var flow_id = $('#flow_id').val();
        $.ajax({
            url:"/bargain_flow/cancel_review",
            type:"POST",
            dataType:"json",
            data:{
                flow_id:flow_id,
