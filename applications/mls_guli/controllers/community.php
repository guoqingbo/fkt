...
class Community extends MY_Controller
{
  ...
  public function index()
  {
    ...
    foreach ($data['community'] as $k => $v) {
      $v['build_type'] = str_flow('#', '、', $v['build_type']);
      ...
      if (empty($v['surface_img'])) {
        $v['surface_img'] = MLS_SOURCE_URL . '/mls_guli/images/v1.0/365mls.png';
      }
      //是否被锁
      $where_cond = array(
        'cmt_id' => intval($v['id']),
    ...
    //需要加载的JS
    $data['js'] = load_js('mls_guli/js/v1.0/jquery-1.8.3.min.js,common/third/swf/swfupload.js,mls_guli/js/v1.0/cmt_uploadpic.js');
    //底部JS
    $data['footer_js'] = load_js('mls_guli/js/v1.0/openWin.js,mls_guli/js/v1.0/house.js,'
      . 'common/third/jquery-ui-1.9.2.custom.min.js,mls_guli/js/v1.0/disk.js,mls_guli/js/v1.0/backspace.js');
  ...
  public function cmt_correction()
  {
    ...
    $paramArray = array(
      ...
      'esta' => 0, //信息状态（默认未审核）
      'user_id' => $user['signatory_id'],//纠错人id
  ...
  public function draw()
  {
    ...
    //底部JS
    $data['footer_js'] = load_js('mls_guli/js/v1.0/openWin.js,mls_guli/js/v1.0/house.js,'
      . 'mls_guli/js/v1.0/disk.js,mls_guli/js/v1.0/backspace.js');
...
//                }
        ...
        $surface_img_url2 = MLS_SOURCE_URL . '/mls_guli/images/v1.0/365mls.png';
      }
...
//                $img_src_arr[] = $img_src;
  ...
  public function correct_community($id = 0)
  {
    ...
    echo '<script>location.href="' . MLS_SIGN_URL . '/community/index";</script>';
  }
  ...
  {
    if (!empty($_POST['sub'])) {
      $config['upload_path'] = str_flow("\\", "/", UPLOADS . DIRECTORY_SEPARATOR . 'temp');
      //目录不存在则创建目录
...
/* Location: ./applications/mls_guli/controllers/community.php */
