...
class User_request extends MY_Controller
{
  ...
  public function index($deviceid, $osid)
  {
    ...
    if (isset($infoarr[0]['signatoryid']) && $infoarr[0]['signatoryid'] > 0) {
  ...
  {
    //先做单点登录判断
    $signatory_session = $this->signatory_model->get_user_session();
    $deviceid = $signatory_session['deviceid'];
    $osid = $signatory_session['osid'];

    //web端用户无设备ID
    if ($deviceid == '' && $osid == '') {
      $infoarr[0]['signatoryid'] = $signatory_session['signatory_id'];
    } else {
      $this->load->model('user_request_model');
      //此处需要优化，可考虑用MC减少心跳导致的数据库查询频率
      $infoarr = $this->user_request_model->get_online_info($deviceid, $osid);
    ...
    if (isset($infoarr[0]['signatoryid']) && $infoarr[0]['signatoryid'] > 0) {
      ...
      $this_signatory_id = intval($this_user_data['signatory_id']);
      //获取弹窗消息
      $pop_message = $this->checknotice_model->get_new_pop_message($this_signatory_id);
      if ('1' == $is_remind_open) {
  ...
  public function pclogin($deviceid, $verson, $phone, $password)
  {
    ...
    $result = $this->signatory_model->md5login($phone, $password);
    ...
    ) //登录成功
    {
      ...
      if (empty($signatory_info['login_time'])) {//首次登录
        //引入SMS类库，并发送短信
        ...
        $this->message_base_model->add_message('8-52', $signatory_id, $signatory_info['truename'], '/my_info/');
      }
      $this->signatory_info_model->update_by_signatory_id(array('login_time' => time()), $signatory_id);

      $this->load->model('user_request_model');
      $online_stat = $this->user_request_model->get_online_state($deviceid, $signatory_id);
      if (isset($online_stat[0]['id']) && $online_stat[0]['id'] > 0) {
        ...
        $osid = $this->user_request_model->add_online_state($deviceid, $signatory_id, $result['city_id']);
  ...
  public function signin()
  {
    ...
    if ($is_online) //判断是否登录
    {
      ...
      ) //登录成功
      {
        ...
        $this->load->model('city_model');
        $city = $this->city_model->get_by_id($result['city_id']);
        $init_data_session = array('signatory_id' => $result['id'],
          'city_spell' => $city['spell']);
        $this->signatory_model->set_user_session($init_data_session);
        //获取经纪人详细信息
        $this->load->model('signatory_info_model');
        $signatory_info = $this->signatory_info_model->
        get_baseinfo_by_signatory_id($result['id']);
        //重置session值
        $signatory_info['city_spell'] = $city['spell'];
