...
//        $data['district'][$v['id']] = $v;
//        $data['district_name'][$v['district']] = $v['id'];
//      }
//    }
//
...
//    $where = array();
//    // 获取所有门店
//    $data['agencys'] = $this->agency_model->get_all_by_district($where);
    /**
     * 数据范围
     * 1、店长以上权限看公司
     * 2、店长及店长秘书权限查看本门店
    ...
    $report_add_per = $this->signatory_purview_model->check('7');
    $report_edit_per = $this->signatory_purview_model->check('8');
    $report_delete_per = $this->signatory_purview_model->check('9');
    $report_report_per = $this->signatory_purview_model->check('10');
    $report_cancel_per = $this->signatory_purview_model->check('11');
    $data['auth'] = array(
      'add' => $report_add_per, 'edit' => $report_edit_per,
      'delete' => $report_delete_per, 'report' => $report_report_per,
      'cancel' => $report_cancel_per,
    );
    //清除条件头尾多余空格
    $cond_where = trim($cond_where);
    //符合条件的总行数
    $this->_total_count = $this->contract_model->count_by($cond_where);

    //计算总页数
    $pages = $this->_total_count > 0 ? ceil($this->_total_count / $this->_limit) : 0;

    //获取列表内容
    $list = $this->contract_model->get_all_by($cond_where, $this->_offset, $this->_limit);
    ...
    $data['css'] = load_css('mls_guli/css/v1.0/base.css,'
      ...
      . 'mls_guli/css/v1.0/contract_manage.css,'
      . 'mls_guli/css/v1.0/contract.css');
    //需要加载的JS
    $data['js'] = load_js('mls_guli/js/v1.0/jquery-1.8.3.min.js,common/third/jquery-ui-1.9.2.custom.min.js,'
      ...
      . 'mls_guli/js/v1.0/verification_contract.js');
    //底部JS
    ...
    $this->view('contract/contract_report', $data);
  }
  ...
  public function get_agency_info_by_kw()
  {
    ...
    if (empty($cmt_info) || empty($search_arr['role_level'])) {
      $cmt_info[0]['id'] = 0;
  ...
  public function get_broker_info_by_kw()
  {
    ...
    $keyword = $this->input->get('keyword', TRUE);
    $this->load->model('agency_model');
      $select_fields = array('broker_id as id', 'truename as name', 'phone', 'company_id', 'agency_id');
    $this->agency_model->set_select_fields($select_fields);
  ...
  public function modify_report_index($type = 1, $id = 0)
  {
    ...
    $data['type'] = $type;
    //查询出报备数据
    $data['report'] = $this->contract_model->get_by_id($id);
    ...
    $data['footer_js'] = load_js('common/third/My97DatePicker/WdatePicker.js,mls_guli/js/v1.0/openWin.js,mls_guli/js/v1.0/house.js'
      . 'mls_guli/js/v1.0/backspace.js');
    $this->view('contract/contract_modify_report', $data);
  }
  ...
  public function modify_warrant_index($c_id = 0, $id = 0)
  {
    ...
    $data['config'] = $this->house_config_model->get_config();
    //权限
    $warrant_add_per = $this->signatory_purview_model->check('68');
    ...
    //合同详情
    $data['contract'] = $this->contract_model->get_by_id($c_id);
    ...
    $data['footer_js'] = load_js('common/third/My97DatePicker/WdatePicker.js,mls_guli/js/v1.0/openWin.js,mls_guli/js/v1.0/house.js'
      . 'mls_guli/js/v1.0/backspace.js');
    $this->view('contract/contract_warrant_modify', $data);
  }
  ...
  public function contract_warrant_detail($id)
  {
    ...
    //合同详情
    $data['contract'] = $this->contract_model->get_by_id($data['warrant_list']['contract_id']);
    ...
    $data['footer_js'] = load_js('mls_guli/js/v1.0/openWin.js,mls_guli/js/v1.0/house.js'
      . 'mls_guli/js/v1.0/backspace.js');
    if ($data['warrant_list']['is_remind'] == 1) {
      $this->view('/contract/contract_warrant_detail', $data);
    } else {
      $this->view('/contract/contract_warrant_detail1', $data);
    }
  }
  ...
  public function contract_list($type = 1)
  {
    ...
    //权限
    $contract_add_per = $this->signatory_purview_model->check('114');
    $contract_edit_per = $this->signatory_purview_model->check('115');
    $contract_delete_per = $this->signatory_purview_model->check('116');
    $contract_cancel_per = $this->signatory_purview_model->check('117');
    $data['auth'] = array(
    ...
    //获取列表内容
    $list = $this->contract_model->get_all_by($cond_where, $this->_offset, $this->_limit, $order_key, $order_by);
    ...
    $list1 = $this->contract_model->get_all_by($cond_where, 0, $this->_total_count);
    ...
    $data['footer_js'] = load_js('common/third/My97DatePicker/WdatePicker.js,mls_guli/js/v1.0/openWin.js,mls_guli/js/v1.0/house.js,'
      . 'mls_guli/js/v1.0/backspace.js');
    $this->view('contract/contract_list', $data);
  }
  ...
  public function add_report()
  {
    ...
    $number = date('YmdHis') . str_pad(rand(0, 9999), 4, '0', STR_PAD_LEFT);
    if ($post_param['id']) {
      //检查合同编号唯一性
      $where = "enter_company_id = {$this->user_arr['company_id']} and number = '{$number}' and id != {$post_param['id']} and is_del = 0";
      $result = $this->contract_model->get_one_by($where);
    } else {
      //检查合同编号唯一性
      $where = "enter_company_id = {$this->user_arr['company_id']} and number = '{$number}' and is_del= 0 ";
      $result = $this->contract_model->get_one_by($where);
    }
    if (is_full_array($result)) {
      ...
      if ($post_param['id']) {
        $data['result'] = $this->contract_model->update_by_id($data_info, $post_param['id']);
        if ($data['result']) {
          $data['msg'] = '预约修改成功！';
          //操作日志
          $add_log_param = array(
        ...
        $data['result'] = $this->contract_model->add_info($data_info);
        if ($data['result']) {
          $data['msg'] = '预约添加成功！';
          //操作日志
          $add_log_param = array(
  ...
  public function edit()
  {
    ...
    $data['arr'] = $this->contract_model->get_by_id($id);
...
//    } else {//预约添加信息数组
    ...
    $data['result'] = $this->contract_model->update_by_id($data_info, $id);
    if ($data['result']) {
      $data['msg'] = '预约修改成功！';
      //操作日志
      $add_log_param = array(
...
//    }
  ...
  {
    $id = $this->input->get('id');
    $info = $this->contract_model->get_by_id($id);
    $rs = $this->contract_model->update_by_id(array('status' => $status, 'is_check' => 1), $id);
...
//      $this->contract_model->set_select_fields(array('number'));
//      $result = $this->contract_model->get_by_id($id);
//      //操作日志
//      $add_log_param = array(
//        'company_id' => $this->user_arr['company_id'],
//        'agency_id' => $this->user_arr['agency_id'],
...
//      $this->operate_log_model->add_operate_log($add_log_param);
      ...
      if($status == 2) {
        $this->sms->send($phone, array('stage' => '已通过', 'order_sn' => $info["number"], 'signing_time' => date('Y-m-d H:i:s', $info['signing_time'])));
      } elseif($status == 3) {
        $this->sms->send($phone, array('stage' => '未通过', 'order_sn' => $info["number"], 'signing_time' => date('Y-m-d H:i:s', $info['signing_time'])));
      }
   ...
   * @return void
   */
  public function update_report_status1($status)
  {
    $id = $this->input->get('id');
    $info = $this->contract_model->get_by_id($id);
    $rs = $this->contract_model->update_by_id(array('status' => $status, 'is_check' => 1), $id);
...
//      $this->operate_log_model->add_operate_log($add_log_param);
  ...
  public function modify_contract($type = 1, $id = 0)
  {
     ...
     */
    //获取访问菜单
    $range_menu_a = $this->contract_model->get_range_menu_by_role_level(
    ...
    //获取访问菜单
    $range_menu_b = $this->contract_model->get_range_menu_by_role_level(
    ...
    $data['footer_js'] = load_js('common/third/My97DatePicker/WdatePicker.js,mls_guli/js/v1.0/openWin.js,mls_guli/js/v1.0/house.js'
      . 'mls_guli/js/v1.0/backspace.js');
    if ($type == 1) {
      $this->view("contract/contract_sell_add", $data);
    } else {
      $this->view("contract/contract_rent_add", $data);
    }
  }
  ...
  public function save_contract()
  {
    ...
    if ($submit_flag == "add") {
      ...
      //添加
      $id = $this->contract_model->add_info($datainfo);
      if ($id) {
        //合同跟进——添加业绩分成
        $add_data = array(
          'c_id' => $id,
      ...
      //添加
      $rs = $this->contract_model->update_by_id($datainfo, $post_param['id']);
      if ($rs) {
  ...
  public function contract_detail($id = 0)
  {
    ...
    $actual_add_per = $this->signatory_purview_model->check('61');
    ...
    $data['footer_js'] = load_js('common/third/My97DatePicker/WdatePicker.js,mls_guli/js/v1.0/openWin.js,mls_guli/js/v1.0/house.js,'
      . 'mls_guli/js/v1.0/backspace.js');
    if ($contract['type'] == 1) {
      $this->view("contract/contract_sell_detail", $data);
    } else {
      $this->view("contract/contract_rent_detail", $data);
    }
  }
  ...
  public function get_house($type = 1, $house_id = '')
  {
    ...
    $o_agency_id = $this->input->get('agency_id', TRUE);
    $o_broker_id = $this->input->get('broker_id', TRUE);

    $agency_id = $post_param['agency_id'];
    $broker_id = $post_param['broker_id'];

    if (!$agency_id) {
      $agency_id = $o_agency_id;
      $post_param['agency_id'] = $o_agency_id;
    }

    if (!$broker_id) {
      $broker_id = $o_broker_id;
      $post_param['broker_id'] = $o_broker_id;
    }

    $select_fields = array('id', 'name');
    $this->agency_model->set_select_fields($select_fields);
    $agency_info = $this->agency_model->get_by_id($post_param['agency_id']);
    if ($agency_info) {
      $post_param['agency_name'] = $agency_info['name'];
    }

    $select_fields = array('id', 'truename');
    $this->broker_info_model->set_select_fields($select_fields);
    $broker_info = $this->broker_info_model->get_by_id($post_param['broker_id']);
    if ($broker_info) {
      $post_param['broker_name'] = $broker_info['truename'];
    }
...
//    $this->signature_permission_model->set_agency_id($agency_id, $company_id, $this->user_arr['role_level']);
//    $access_agency_ids_data = $this->agency_permission_model->get_agency_id_by_main_id_access($agency_id, 'is_view_house');
...
//      }
//      $all_access_agency_ids .= $agency_id;
...
//    array_unshift($data['brokers'], array('broker_id' => '', 'truename' => '不限'));
    ...
    $data['footer_js'] = load_js('mls_guli/js/v1.0/openWin.js,mls_guli/js/v1.0/house.js,'
      . 'mls_guli/js/v1.0/backspace.js');
    $this->view("contract/contract_choose_house", $data);
  }
  ...
  public function get_cooperate($type = 1, $order_sn = '')
  {
    ...
    //需要加载的css
    $data['css'] = load_css('mls_guli/css/v1.0/base.css,mls_guli/third/iconfont/iconfont.css,'
      . 'mls_guli/css/v1.0/house_manage.css,' . 'mls_guli/css/v1.0/contract_manage.css');
    //需要加载的JS
    $data['js'] = load_js('mls_guli/js/v1.0/jquery-1.8.3.min.js,common/third/jquery-ui-1.9.2.custom.min.js');
    //底部JS
    $data['footer_js'] = load_js('mls_guli/js/v1.0/openWin.js,mls_guli/js/v1.0/house.js,'
      . 'mls_guli/js/v1.0/backspace.js');
    $this->view("contract/contract_choose_cooperate", $data);
  }
  ...
  public function get_customer($type = 1, $customer_id = '')
  {
    ...
    $data['footer_js'] = load_js('mls_guli/js/v1.0/openWin.js,mls_guli/js/v1.0/house.js,'
      . 'mls_guli/js/v1.0/backspace.js');
    $this->view("contract/contract_choose_customer", $data);
  }
  ...
  {
    return $this->update_report_status(2);
  }
   ...
   * @return  void
   */
  public function disagree_report()
  {
    return $this->update_report_status(3);
  }
  ...
  {
    return $this->update_report_status(6);
  }
  ...
  {
    $update_res = $this->update_report_status1(4);
    $id = $this->input->get('id');
    //如果预约编号存在
    if ($update_res["result"] == "ok" && $id != 0) {
      $contract_info = $this->contract_model->get_by_id($id);
        $agency_a = $this->agency_model->get_one_by(array('id' => $contract_info['agency_id_a']));
        if ($agency_a['agency_type'] == 3) {
            $contract_info['agency_name_a'];
        }
        //插入成交信息
      $add_res = $this->save_add_bargain($contract_info);
      if ($add_res["result"] == "ok") {
          //更改房源状态为成交
          if ($contract_info['house_id'] > 0) {
              $arr = array(
                  'status' => 3,//1.有效 2.预定 3.成交 4.无效 5.删除
              );
              $cond_where = "id =" . $contract_info['house_id'];
              $up_num = $this->sell_house_model->update_info_by_cond($arr, $cond_where);
          }
          //生成成交过户流程
          $template_id = 1;//系统模板
          $up_num = $this->select_transfer_template($template_id, $add_res['bargain_id']);
        echo json_encode(array('result' => '1', 'id' => $id, 'bargain_id' => $add_res["bargain_id"]));
      } else {
        echo json_encode(array('result' => '0'));
      }
    }
...
//添加成交记录
  public function save_add_bargain($contract_info)
  {
    $datainfo = array(
      ...
      'reserve_number' => $contract_info['number'],
      'company_id_a' => $contract_info['company_id_a'],
      'agency_id_a' => $contract_info['agency_id_a'],
      'agency_name_a' => $contract_info['agency_name_a'],
      'broker_id_a' => $contract_info['broker_id_a'],
      'broker_name_a' => $contract_info['broker_name_a'],
      'broker_tel_a' => $contract_info['broker_tel_a'],
      'company_id_b' => $contract_info['company_id_b'],
      'agency_id_b' => $contract_info['agency_id_b'],
      'agency_name_b' => $contract_info['agency_name_b'],
      'broker_id_b' => $contract_info['broker_id_b'],
      'broker_name_b' => $contract_info['broker_name_b'],
      'broker_tel_b' => $contract_info['broker_tel_b'],
      'block_name' => $contract_info['block_name'],
      'house_addr' => $contract_info['house_addr'],
      'house_id' => $contract_info['house_id'],
      'buildarea' => $contract_info['buildarea'],
      'owner' => $contract_info['owner'],
      'owner_idcard' => $contract_info['owner_idcard'],
      'owner_tel' => $contract_info['owner_tel'],
      'customer' => $contract_info['customer'],
      'customer_idcard' => $contract_info['customer_idcard'],
      'customer_tel' => $contract_info['customer_tel'],
        'signing_time' => time(),
      'bargain_status' => 1,
        'bargain_type' => 1,
        'type' => 2,
    ...
    //检查成交编号唯一性
//            $where = "enter_company_id = {$this->user_arr['company_id']} and number = '{$post_param['number']}' and is_del = 0 and status = 2";
...
//            }
    ...
    if ($id) {
      ...
      return array('result' => 'ok', "msg" => "成交录入成功", "bargain_id" => $id);
      exit;
    } else {
      return array('result' => 'ok', "msg" => "成交录入失败");
      exit;
    }
  }
  ...
  {
    $id = $this->input->get('id');
    $rs = $this->contract_model->update_by_id(array('is_del' => 1, 'is_template' => 0), $id);
    if ($rs) {
      //合同跟进——取消合同
      $data = array(
        'c_id' => $id,
  ...
  {
    $id = $this->input->get('id');
    $rs = $this->contract_model->update_by_id(array('is_check' => 4), $id);
    if ($rs) {
      //合同跟进——删除合同
      $data = array(
        'c_id' => $id,
  ...
  public function contract_review($type = 1)
  {
    ...
    //权限
    $contract_review_per = $this->signatory_purview_model->check('118');
    $contract_fanreview_per = $this->signatory_purview_model->check('119');
    $data['auth'] = array(
    ...
    //获取列表内容
    $list = $this->contract_model->get_all_by($cond_where, $this->_offset, $this->_limit, $order_key = 'is_check', $order_by = 'ASC');

    $data['list'] = $list;
    //查询所有待审核合同的数量
    $cond_where1 = $cond_where . ' AND is_check = 1';
    //符合条件的总行数
    $data['total'] = $this->contract_model->count_by($cond_where1);
    //分页处理
    $params = array(
      'total_rows' => $this->_total_count, //总行数
      'method' => 'post', //URL提交方式 get/html/post
    ...
    $data['footer_js'] = load_js('common/third/My97DatePicker/WdatePicker.js,mls_guli/js/v1.0/openWin.js,mls_guli/js/v1.0/house.js,'
      . 'mls_guli/js/v1.0/backspace.js');
    $this->view('contract/contract_review', $data);
  }
  ...
  public function sure_review()
  {
    ...
    //更改合同表中合同状态
    $result = $this->contract_model->update_by_id($update_data, $contract_id);
    //审核通过后发送消息
    if ($result) {
      //记入日志
      if ($review_type == 2) {
      ...
      $this->contract_log_model->add_info($add_data);
      //操作日志
      $info = $this->contract_model->get_by_id($contract_id);
      $add_log_param = array(
        'company_id' => $this->user_arr['company_id'],
        'agency_id' => $this->user_arr['agency_id'],
        'broker_id' => $this->user_arr['broker_id'],
  ...
  {
    $data = $this->contract_model->get_info_by_id($id);
  ...
  public function confirm_all_commission()
  {
    ...
    if ($get_total['percent_total'] == 100) {
      ...
      $this->contract_model->update_by_id($update_data, $c_id);
      $add_data = array(
        'c_id' => $c_id,
        'type_name' => "业绩分成",
        'content' => "对所有业绩分成完成结佣。",
    ...
    if ($divide_id) {
      ...
      if ($new_total < 100) {
        if ($rs) {
          ...
          $return_data['result'] = 'ok';
          //操作日志
          $info = $this->contract_model->get_by_id($c_id);
          $add_log_param = array(
            'company_id' => $this->user_arr['company_id'],
            'agency_id' => $this->user_arr['agency_id'],
            'broker_id' => $this->user_arr['broker_id'],
      ...
      $contract = $this->contract_model->get_by_id($c_id);
  ...
  public function warrant_template_add($id = 1, $key1 = 1, $c_id = 0)
  {
    ...
    $data['footer_js'] = load_js('mls_guli/js/v1.0/openWin.js,mls_guli/js/v1.0/house.js,'
      . 'mls_guli/js/v1.0/backspace.js');
    $this->view('contract/warrant_add_template', $data);
  }
  ...
  public function warrant_template($id = 1)
  {
    ...
    $data['footer_js'] = load_js('mls_guli/js/v1.0/openWin.js,mls_guli/js/v1.0/house.js,'
      . 'mls_guli/js/v1.0/backspace.js');
    $this->view('contract/contract_template', $data);
  }
  ...
  {
    $data['contract'] = $this->contract_model->get_by_id($id);
    //获取配置项
    $data['config'] = $this->contract_config_model->get_config();
    //权证流程
    $warrant_step = $this->contract_warrant_model->get_all_by_cid($id);
    ...
    $warrant_add_per = $this->signatory_purview_model->check('68');
    $warrant_edit_per = $this->signatory_purview_model->check('69');
    $warrant_delete_per = $this->signatory_purview_model->check('70');
    $warrant_complete_per = $this->signatory_purview_model->check('71');
    $warrant_complete_all_per = $this->signatory_purview_model->check('71');
    ...
    $data['footer_js'] = load_js('mls_guli/js/v1.0/openWin.js,mls_guli/js/v1.0/house.js,'
      . 'mls_guli/js/v1.0/backspace.js');
    $this->view('contract/contract_warrant_manage', $data);
  }
  ...
  public function contract_actual_manage($id)
  {
    ...
    $actual_add_per = $this->signatory_purview_model->check('61');
    $actual_edit_per = $this->signatory_purview_model->check('62');
    $actual_delete_per = $this->signatory_purview_model->check('63');
    $actual_complete_per = $this->signatory_purview_model->check('67');
    ...
    $data['footer_js'] = load_js('mls_guli/js/v1.0/openWin.js,mls_guli/js/v1.0/house.js,'
      . 'mls_guli/js/v1.0/backspace.js');
    $this->view('contract/contract_actual_manage', $data);
  }
  ...
  public function contract_actual_modify($c_id = 0, $id = 0)
  {
    ...
    $data['footer_js'] = load_js('common/third/My97DatePicker/WdatePicker.js,mls_guli/js/v1.0/openWin.js,mls_guli/js/v1.0/house.js,'
      . 'mls_guli/js/v1.0/backspace.js');
    $this->view('contract/contract_actual_modify', $data);
  }
  ...
  public function contract_actual_detail($id = 0)
  {
    ...
    $data['footer_js'] = load_js('mls_guli/js/v1.0/openWin.js,mls_guli/js/v1.0/house.js,'
      . 'mls_guli/js/v1.0/backspace.js');
    $this->view('contract/contract_actual_detail', $data);
  }
  ...
  public function contract_should_modify($c_id = 0, $id = 0)
  {
    ...
    $data['footer_js'] = load_js('common/third/My97DatePicker/WdatePicker.js,mls_guli/js/v1.0/openWin.js,mls_guli/js/v1.0/house.js,'
      . 'mls_guli/js/v1.0/backspace.js');
    $this->view('contract/contract_should_modify', $data);
  }
  ...
  public function contract_should_detail($id = 0)
  {
    ...
    $data['footer_js'] = load_js('mls_guli/js/v1.0/openWin.js,mls_guli/js/v1.0/house.js,'
      . 'mls_guli/js/v1.0/backspace.js');
    $this->view('contract/contract_should_detail', $data);
  }
  ...
  public function contract_divide_modify($c_id = 0, $id = 0)
  {
    ...
    $range_menu = $this->contract_model->get_range_menu_by_role_level(
      $this->user_arr, $agency_id);
    $range_menu1 = $this->contract_model->get_range_menu_by_role_level(
    ...
    $data['footer_js'] = load_js('common/third/My97DatePicker/WdatePicker.js,mls_guli/js/v1.0/openWin.js,mls_guli/js/v1.0/house.js,'
      . 'mls_guli/js/v1.0/backspace.js');
    $this->view('contract/contract_divide_modify', $data);
  }
  ...
  public function contract_divide_detail($id)
  {
    $data['config'] = $this->contract_config_model->get_config();
    $data['divide_list'] = $this->contract_divide_model->get_by_id($id);
    $data['contract'] = $this->contract_model->get_by_id($data['divide_list']['c_id']);
    //需要加载的css
    $data['css'] = load_css('mls_guli/css/v1.0/base.css,mls_guli/third/iconfont/iconfont.css,'
      . 'mls_guli/css/v1.0/house_manage.css,'
      . 'mls_guli/css/v1.0/contract_manage.css');
    ...
    $data['footer_js'] = load_js('mls_guli/js/v1.0/openWin.js,mls_guli/js/v1.0/house.js,'
      . 'mls_guli/js/v1.0/backspace.js');
    $this->view('contract/contract_divide_detail', $data);
  }
  ...
  public function contract_should_manage($id)
  {
    ...
    $data['footer_js'] = load_js('mls_guli/js/v1.0/openWin.js,mls_guli/js/v1.0/house.js,'
      . 'mls_guli/js/v1.0/backspace.js');
    $this->view('contract/contract_should_manage', $data);
  }
  ...
  public function contract_follow_manage($id = 0)
  {
    ...
    $data['footer_js'] = load_js('mls_guli/js/v1.0/openWin.js,mls_guli/js/v1.0/house.js,'
      . 'mls_guli/js/v1.0/backspace.js');
    $this->view('contract/contract_follow_manage', $data);
  }
  ...
  public function contract_divide_manage($id)
  {
    ...
    $data['footer_js'] = load_js('mls_guli/js/v1.0/openWin.js,mls_guli/js/v1.0/house.js,'
      . 'mls_guli/js/v1.0/backspace.js');
    $this->view('contract/contract_divide_manage', $data);
  }
  ...
  public function warrant_list()
  {
    ...
    $data['stage'] = $this->contract_warrant_model->get_all_stage();
    //获取列表内容
    $list = $this->contract_model->get_all_by($cond_where, $this->_offset, $this->_limit2);
    if (is_full_array($list)) {
      foreach ($list as $key => $val) {
        //权证流程
    ...
    $data['footer_js'] = load_js('common/third/My97DatePicker/WdatePicker.js,mls_guli/js/v1.0/openWin.js,mls_guli/js/v1.0/house.js'
      . 'mls_guli/js/v1.0/backspace.js');
    $this->view('contract/contract_warrant_list', $data);
  }
  ...
  public function get_all_template($id = 0)
  {
    ...
    $data['footer_js'] = load_js('mls_guli/js/v1.0/openWin.js,mls_guli/js/v1.0/house.js,'
      . 'mls_guli/js/v1.0/backspace.js');
    $this->view('contract/contract_choose_template', $data);
  }
  ...
  public function sel_choose()
  {
    ...
    if ($num == $total) {
      $this->contract_model->update_by_id(array('template_id' => $template_id, 'is_template' => 1), $contract_id);
      $json_data['status'] = 1;
    } else {
      $json_data['status'] = 0;
    }
    ...
    if ($stage_id) {
      ...
      if (!empty($update_result) && is_int($update_result)) {
        ...
        if ($is_remind == 1) {
          ...
          $this->message_base_model->add_message('8-50', $paramArray['remind_broker_id'], $broker_info['truename'], '/contract/contract_detail/' . $c_id, $params);
        }
      ...
      if ($total_step_old < 10) {
        //如果之前没有添加过步骤，则改变合同创建模版的状态
        if ($total_step_old == 0) {
          $this->contract_model->update_by_id(array('is_template' => 1), $c_id);
        }
  ...
  public function confirm_all_complete()
  {
    ...
    if ($total == $num) {
      ...
      //修改合同表中该合同的相应字段
      $rs = $this->contract_model->update_by_id($data, $contract_id);
      if ($rs) {
        $add_data = array(
          'c_id' => $contract_id,
          'type_name' => "权证流程",
  ...
  public function delete_temp_step()
  {
    ...
    if ($rs) {
      ...
      if ($steps_total == 0) {
        $this->contract_model->update_by_id(array('is_template' => 0, 'template_id' => 0), $c_id);
      }
      $add_data = array(
        'c_id' => $c_id,
        'type_name' => "权证流程",
  ...
  public function add_flow()
  {
    ...
    if ($id) {
      ...
      //获取合同详情
      $contract = $this->contract_model->get_by_id($paramArray['c_id']);
...
//    $cond_where .= " type = " . $type . " and is_check > 0";
  ...
  public function exportReport($type)
  {
    ...
    $cond_where = trim($cond_where);
    //符合条件的总行数
    $this->_limit = $this->contract_model->count_by($cond_where);

    $productlist = $this->contract_model->get_all_by($cond_where, $this->_offset, $this->_limit);
    $list = array();
...
/* Location: ./application/mls_guli/controllers/contract.php */
