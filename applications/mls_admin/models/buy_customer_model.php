...
class Buy_customer_model extends Customer_base_model
{
   ...
   */
  public function get_buylist_by_cond($cond_where, $offset = 0, $limit = 10,
  ...
  public function get_total_by_cond($cond_where)
  {
    ...
    if ($cond_where != '') {
      ...
      $this->dbback_city->from('buy_customer as a');
   ...
   * @author kang
   */
  public function get_buycustomerlist_by_cond($cond_where, $offset = 0, $limit = 10,
   ...
   * 获取所有门店信息（用于联想）
   */
  public function get_total_agency($keyword)
  {
    $this->dbback_city->select('*');
    $this->dbback_city->from('agency');
    $this->dbback_city->like('name', $keyword);
    $this->dbback_city->limit(10, 0);

    $agencys = $this->dbback_city->get()->result_array();
   ...
   * 获取所有楼盘（用于联想）
   */
  public function get_total_community($keyword)
  {
    $this->dbback_city->select('*');
    $this->dbback_city->from('community');
    $this->dbback_city->like('cmt_name', $keyword);
    $this->dbback_city->limit(10, 0);

    $communitys = $this->dbback_city->get()->result_array();
  ...
  public function get_follows_by_cond($condition)
  {
    ...
    $this->dbback_city->join('buy_customer as b', 'a.customer_id = b.id');
  ...
  public function upd_info_by_id($customer_id)
  {
    ...
    $rs = $this->db_city->update('buy_customer', $data, $where);
...
/* Location: ./applications/models/buy_customer_model.php */
