...
class Collect_solve_model extends MY_Model
{
  ...
  //获取采集连接列表
  public function collect_url_list($city = '', $source_from = '58')
  {
    if ($city != '' && $source_from != '') {
      $where = array('city' => $city, 'source_from' => $source_from, 'is_run' => 0);
    } else if ($source_from != '') {
      $where = array('source_from' => $source_from, 'is_run' => 0);
    } else {
  ...
  //获取采集列表
  public function collect_url($city = '', $source_from = '58')
  {
    if ($city != '' && $source_from != '') {
      $where = array('city' => $city, 'source_from' => $source_from);
    } else if ($source_from != '') {
      $where = array('source_from' => $source_from);
    } else {
      $where = array();
    }
    $limit = 1;
  ...
  //修改采集链接列表
  public function update_collect_url_list($type, $city = '', $source_from = '')
  {
    if ($type == 'change') {
      if ($city != '' && $source_from != '') {
        $where = array('is_run' => 2, 'city' => $city, 'source_from' => $source_from);
      } else if ($source_from != '') {
        $where = array('is_run' => 2, 'source_from' => $source_from);
      } else {
        $where = array('source_from' => $source_from);
      }
      $data = array('is_run' => 0);
  ...
  public function collect_info($data, $con)
  {
    ...
    switch ($data['kind_type']) {
      case 1://住宅
        $result = $this->collect_details_model->collect_info_zhuzhai($data, $con);
        break;
      case 3://商铺
        $result = $this->collect_details_model->collect_info_shangpu($data, $con);
        break;
      case 4://写字楼
        $result = $this->collect_details_model->collect_info_xiezilou($data, $con);
        break;
      default:
        $result = $this->collect_details_model->collect_info_zhuzhai($data, $con);
    }
  ...
  public function collect_list($data, $con)
  {
    ...
    } else if ($data['source_from'] == '58') {
      if ($data['type'] == 1 && $data['kind_type'] == 1) {
        preg_match_all('/<p class="bthead">.*<a href="http:\/\/' . $data['host'] . '\/ershoufang\/(.*)\?psid/siU', $con, $prj);
      } elseif ($data['type'] == 2 && $data['kind_type'] == 1) {
        preg_match_all('/<div class="img_list">.*href="http:\/\/' . $data['host'] . '\/zufang\/(.*)\?version/siU', $con, $prj);
      } else {
        preg_match_all('/<td class="t">.*href="http:\/\/' . $data['host'] . '\/' . $data['urllist'] . '\/(.*)\?psid/siU', $con, $prj);
      }
    } else if ($data['source_from'] == 'sfang') {
      preg_match_all('/<dt class="img rel floatl".*href="(.*)"/siU', $con, $prj);
    }
