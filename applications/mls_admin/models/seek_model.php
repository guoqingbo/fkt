...
class Seek_model extends MY_Model
{
   ...
   */
  public function get_list_by_cond($cond_where, $tbl, $offset = 0, $limit = 10, $order_by1 = "is_check asc", $order_by2 = "ctime desc")
  {
    if ($cond_where) {
    ...
    $sql = "select s.id as id,s.phone,realname,d.district as district,s.type,larea,harea,lprice,hprice,ctime,s.status,grab_times,is_check from " . $tbl . " s
                left join district d on s.district_id = d.id {$cond_where} order by {$order_by1}{$order_by2} limit {$offset},{$limit}";
   ...
   * @return  int   符合条件的房源信息
   */
  public function get_list_by_grab($cond_where, $tbl, $offset = 0, $limit = 10, $order_key = "createtime", $order_by = "DESC")
  {
    if ($cond_where) {
      $cond_where = "where " . $cond_where;
    }
  ...
  public function get_rentlist_by_cond($cond_where, $tbl, $offset = 0, $limit = 10, $order_by1 = "is_check asc", $order_by2 = "ctime desc")
  {
    if ($cond_where) {
      $cond_where = "where " . $cond_where;
    }
    ...
    $sql = "select s.id as id,s.phone,realname,d.district as district,s.type,larea,harea,lprice,hprice,ctime,s.status,room,hall,toilet,is_check from " . $tbl . " s
                left join district d on s.district_id = d.id {$cond_where} order by {$order_by1}{$order_by2} limit {$offset},{$limit}";
    return $this->dbback_city->query($sql)->result_array();
  }
  ...
  {
    $sql1 = "select s.id as id,s.uid,s.phone,realname,district_id,s.type,larea,harea,lprice,hprice,ctime,s.status,room,hall,toilet,is_check,a.name as agency_name,b.truename as broker_name,s.company_id,s.agency_id,s.broker_id,s.status,is_check from " . $tbl . " s
