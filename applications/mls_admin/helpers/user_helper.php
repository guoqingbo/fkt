...
function init_user_auth($purview, $role) {
	...
	$_SESSION[WEB_AUTH]["now_city"] = $_SESSION[WEB_AUTH]["open_citys"][0];
        print_r($_SESSION[WEB_AUTH]["now_city"]);die();
}
...
function gotoUrl($url, $msg='') {
	echo "<p>".$msg."</p>";
	echo "<script>";
	echo "setTimeout(function(){if(parent.window.location.href != ''){parent.window.location.href='".$url."';} window.location.href='".$url."';});";
	echo "</script>";
	exit;
}
				...
				if(is_array($page_purview_arr[$key]) && !empty($page_purview_arr[$key]))
				{
					foreach($page_purview_arr[$key] as $p)
					{
						if($p['fid'] == $key && in_array($p['id'], $user_auth))
						{
							$auth_arr = array(
								"title" => $p['title'],
								"path"	=> MLS_ADMIN_URL."/module".$p['path']
							);
