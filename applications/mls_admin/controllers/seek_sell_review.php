 ...
 * @author      sun
 */
class Seek_sell_review extends MY_Controller
{
  ...
  public function index()
  {
    ...
    $data['list'] = $this->seek_model->get_list_by_cond($where, $tbl = 'seek_sell', $data['offset'], $data['pagesize']);
    ...
    $this->load->view("seek_sell_review/index", $data);
  }
  ...
  public function edit_entrust($id)
  {
    ...
    if ($save == "save") {
      ...
      if ('1' == $data['list']['status'] && '2' == $update_array['is_check'] && 1 == $result) {
        ...
        $list = $this->seek_model->get_list_by($sid, $tbl = "seek_sell");
        ...
        if (is_full_array($broker_list)) {
          foreach ($broker_list as $key => $val) {
            //$this->message_model->add_message('3-24',$val['broker_id'],'','/customer_demand/seek_buy',array('district'=>$district,'lprice'=>$list['lprice'],'hprice'=>$list['hprice']));
            $this->push_func_model->send(1, 11, 1, 0, $val['broker_id'], array(), array('district' => $district, 'lprice' => $list['lprice'], 'hprice' => $list['hprice'], 'price_danwei' => '万元'), '');
          }
        }
    ...
    $data['result'] = $result;
    $this->load->view('seek_sell_review/edit_entrust', $data);
  }
  ...
  public function edit_entrust_review($id)
  {
    ...
    $data['list'] = $this->seek_model->get_list_by($id, $tbl = "seek_sell");
    $data['district'] = $this->district_model->get_district();
    $this->load->view('seek_sell_review/edit_entrust_review', $data);
  }
