 ...
 * @author      angel_in_us
 */
class Sell_house extends MY_Controller
{
  ...
  public function index()
  {
    ...
    $this->load->model('sell_house_model');
    $data['district'] = $this->sell_house_model->community_info('');
    ...
    if (!empty($post_param['search_value'])) {
      if (!empty($post_param['type'])) {
        if ($post_param['type'] == 'broker_id') {
          ...
          $result = $this->sell_house_model->broker($cond_where);
    ...
    //分页开始
    $data['sold_num'] = $this->sell_house_model->get_sell_house_num_by_cond($where);
    ...
    $data['sell_list'] = $this->sell_house_model->get_list_by_cond($where, $data['offset'], $data['pagesize'], $order_key, $order_by);
//
    //如果查询到成交数据
    if (count($data['sell_list']) >= 1) {
      foreach ($data['sell_list'] as $key => $val) {
        $cond_where = 'a.broker_id = ' . $val['broker_id'];
        $result = $this->sell_house_model->broker_info($cond_where);
//                       print_r($result);
    ...
    $this->load->view('sell_house/sell_list', $data);
  }
  ...
  public function kft($house_id = 0, $price = 0)
  {
          ...
          if (is_full_array($house_info)) {
            ...
            if ('0' == $house_info[0]['is_kft']) {
              ...
              if (is_full_array($broker_info)) {
                ...
                //佣金处理
                if ($commission_source != 2 && (empty($sell_house_data['commission_ratio']) || empty($sell_house_data['commission_price']))) {
                  $data_view['mess_error'] = '佣金比例不能为空';
                } else {
                  if ($commission_source != 2 && $sell_house_data['price'] < $sell_house_data['commission_price']) {
                    $sell_house_data['commission_price'] = $sell_house_data['price'] * $sell_house_data['commission_ratio'] * 0.01;
                  }

                  //房源类型
                  if (in_array($house_info[0]['sell_type'], array('1', '2', '3', '4'))) {
                    $sell_house_data['property_type'] = $house_info[0]['sell_type'];
                  } else {
                    $sell_house_data['property_type'] = '1';
                  }

                  $url = MLS_ADMIN_URL . '/collect/add_house/';
                  $add_house_result = $this->curl->vpost($url, $sell_house_data);
                  if (isset($add_house_result) && !empty($add_house_result)) {
                    ...
                    $this->sell_house_model->update_house($update_arr, $where_cond);
                  }
                }
    ...
    $this->load->view('sell_house/kft', $data_view);
  }
  ...
  public function house_detail($house_id)
  {
    ...
    $this->load->view('sell_house/sell_house_detail', $data);
  }
...
//                   }
      ...
      if (!$sell_backinfo['keys'] && $sell_dataifno['keys'] && $sell_dataifno['key_number']) {
        $this->add_key($house_id, $sell_dataifno['key_number']);
      }
    ...
    if ($house_id > 0) {
      ...
      $this->sell_house_model->insert_house_pic($pics, 'sell_house', $house_id, $datainfo['block_id'], 1);
...
//	}
  ...
  public function del_sell($sell_id)
  {
    if (!empty($sell_id)) {
      $this->load->model('sell_house_model');
      $rs = $this->sell_house_model->del_sell($sell_id);
    ...
    echo "<script>location.href='" . FREME_URL . "/sell_house/';</script>";
  }
  ...
  public function xiajia($sell_id)
  {
    if (!empty($sell_id)) {
      $this->load->model('sell_house_model');
      $rs = $this->sell_house_model->xiajia($sell_id);
  ...
  public function follow($id)
  {
    ...
    $data['title'] = "出售房源跟进信息";
    $data['conf_where'] = 'index';
    //type=1 在数据库表示的是出售房源跟进信息
    $this->load->model('sell_house_model');
    $condition = "a.type = 1 AND b.id = " . $id;
    //print_r($condition);
    //echo "<br>";
    $data['follows'] = $this->sell_house_model->get_follows_by_cond($condition);
    //print_r($data['follows']);

    $this->load->view("sell_house/follow", $data);
  }

  public function cooperate_check()
  {
    $this->load->model('house_config_model');
    $data['config'] = $this->house_config_model->get_config();
    $this->load->model('sell_house_model');
    ...
    //筛选条件
    $where = " status != 5 and cooperate_check = 2 and (isshare=1 or isshare=3) ";
    $post_param = $this->input->post(NULL, TRUE);
    if (!empty($post_param['id'])) {
      $where .= ' and id = ' . $post_param['id'];
    }
...
//                       print_r($result);
        ...
        $data['sell_list'][$key]['status'] = $data['config']['status'][$val['status']];
        $data['sell_list'][$key]['nature'] = $data['config']['nature'][$val['nature']];
        $data['sell_list'][$key]['sell_type'] = $data['config']['sell_type'][$val['sell_type']];
      }
    } else {
      $data['sell_list'] = array();
    }

    $this->load->view('sell_house/cooperate_check_list', $data);
  }

  public function cooperate_check_detail($house_id = 0)
  {
    //加载出售基本配置MODEL
    $this->load->model('house_config_model');
    $this->load->model('pic_model');
    $submit_flag = $this->input->post('submit_flag');
    $cooperate_check = $this->input->post('cooperate_check');
    if ('modify' == $submit_flag && intval($cooperate_check) > 0) {
      $update_data = array('cooperate_check' => intval($cooperate_check));

      $arr = array('id' => $house_id);
      $where_in = array('id', $arr);
      $house_info = $this->sell_house_model->get_house_info_byids($where_in);
      if (is_full_array($house_info)) {
        $house_data = $house_info[0];
        //获得房源委托协议书、身份证 、房产证
        $pic_ids = trim($house_info[0]['pic_ids'], ',');
        $pic_id_arr = explode(',', $pic_ids);
        ...
        if (isset($pid_data) && is_full_array($pid_data)) {
          foreach ($pid_data as $key => $value) {
            if (3 == $value['type']) {
              $pic_type_3 = $value['url'];
              $pic_type_3 = str_replace('thumb/', '', $pic_type_3);
            }
            if (4 == $value['type']) {
              $pic_type_4 = $value['url'];
              $pic_type_4 = str_replace('thumb/', '', $pic_type_4);
            }
            if (5 == $value['type']) {
              $pic_type_5 = $value['url'];
              $pic_type_5 = str_replace('thumb/', '', $pic_type_5);
            }
          }
        }
      }
      if (isset($house_data) && is_full_array($house_data)) {
        $reward_type = $house_data['reward_type'];
        //设置赏金
        if ('2' == $reward_type) {
          if (3 == intval($cooperate_check)) {
            $update_data['isshare'] = 1;
            $update_data['house_degree'] = 3;
          } else if (4 == intval($cooperate_check)) {
            $update_data['isshare'] = 0;
          }
          //佣金分配
        } else if ('1' == $reward_type) {
          //三证齐全
          if (!empty($pic_type_3) && !empty($pic_type_4) && !empty($pic_type_5)) {
            //审核通过
            if (3 == intval($cooperate_check)) {
              $update_data['house_degree'] = 3;
              //审核不通过
            } else if (4 == intval($cooperate_check)) {
              $update_data['house_degree'] = 1;
            }
            //两证齐全（身份证、房产证）
          } else if (empty($pic_type_3) && !empty($pic_type_4) && !empty($pic_type_5)) {
            //审核通过
            if (3 == intval($cooperate_check)) {
              $update_data['house_degree'] = 2;
              //审核不通过
            } else if (4 == intval($cooperate_check)) {
              $update_data['house_degree'] = 1;
            }
      ...
      $this->sell_house_model->set_id($house_id);
      $result = $this->sell_house_model->update_info_by_id($update_data);
      if ($result === 1) {
        //系统消息
        $broker_id = $house_data['broker_id'];
        $broker_name = $house_data['broker_name'];
        ...
        if (3 == intval($cooperate_check)) {
          $msg_type = '1-51-1';
          $url_a = '/sell/lists_pub/';
          $func = 1;
        } else if (4 == intval($cooperate_check)) {
          $msg_type = '1-51-2';
          $func = 2;
        }
        $params['block_name'] = $house_data['block_name'];
        $params['price'] = $house_data['price'];
        $params['buildarea'] = $house_data['buildarea'];
        ...
        $send_arr = array(
          ...
          'block_name' => $house_data['block_name'],
          'buildarea' => $house_data['buildarea'],
          'price' => $house_data['price']
        );
        $this->load->model('push_func_model');
        $this->push_func_model->send(1, 12, $func, 0, $broker_id, array('msg_id' => $msg_id), $send_arr);
    ...
    if (isset($pid_data) && is_full_array($pid_data)) {
      foreach ($pid_data as $key => $value) {
        if (3 == $value['type']) {
          $pic_type_3_url = $value['url'];
          $pic_type_3[] = str_replace('thumb/', '', $pic_type_3_url);
        }
        if (4 == $value['type']) {
          $pic_type_4 = $value['url'];
          $pic_type_4 = str_replace('thumb/', '', $pic_type_4);
    ...
    $data['pic_type_3'] = $pic_type_3;
    $data['pic_type_4'] = $pic_type_4;
    $data['pic_type_5'] = $pic_type_5;
    $this->load->model('community_model');
    $cmt_info = $this->community_model->auto_cmtname($house_info[0]['block_name'], 10);
    $house_info[0]['districtname'] = $cmt_info[0]['districtname'];
    $house_info[0]['streetname'] = $cmt_info[0]['streetname'];
    $data['house_detail'] = $house_info[0];
    $this->load->view('sell_house/cooperate_check_detail', $data);
  }
  ...
  {
    $this->load->model('sell_house_model');
    $where_cond = 'select id,isshare,set_share_time,createtime from sell_house where isshare = 1 and set_share_time = 0';
    $house_list = $this->sell_house_model->query($where_cond)->result_array();
    if (is_full_array($house_list)) {
      $update_arr = array();
      foreach ($house_list as $k => $v) {
        $update_arr['set_share_time'] = $v['createtime'];
        $update_result = $this->sell_house_model->update_house($update_arr, array('id' => $v['id']));
        echo 'id:' . $v['id'];
  ...
  public function deal_house_level($page = 1)
  {
    ...
    $data['page'] = ($data['page'] > $data['pages'] && $data['pages'] != 0) ? $data['pages'] : $data['page'];  //判断跳转页数
    $data['offset'] = $data['pagesize'] * ($data['page'] - 1);   //计算记录偏移量
    $this->sell_house_model->set_search_fields(array('id', 'pic_ids'));
    $sell_list = $this->sell_house_model->get_list_by_cond($where, $data['offset'], $data['pagesize']);
...
//                    var_dump(count($pic_type_2));
          ...
          $result = $this->sell_house_model->update_house($update_data, $where_cond);
    ...
    public function add_recommend_house()
    {
        ...
        //判断该房源是否为有效房源
        $this->sell_house_model->set_id($house_id);
        $houseinfo = $this->sell_house_model->get_info_by_id();
        ...
        if ($recommend_house_id = $this->recommend_house_base_model->insert($data)) {
            if ($this->sell_house_model->update_house(['recommend_house_id' => $recommend_house_id], array('id' => $house_id))) {
    ...
    {
        $house_id = $this->input->post("house_id");
        $recommend_house_id = $this->input->post("recommend_house_id");

        //更新该房源is_recommend字段为0
        if ($this->sell_house_model->update_house(['recommend_house_id' => 0], array('id' => $house_id))) {
            $this->load->model("recommend_house_base_model");
            $data = [
                'city' => $_SESSION[WEB_AUTH]["city"],
                'house_id' => $house_id,
    ...
    public function add_like_house()
    {
        ...
        $houseinfo = $this->sell_house_model->get_info_by_id();
        if ($houseinfo['status'] != 1) {
            $res = ['status' => 'fail', 'msg' => '非有效房源不可设为猜你喜欢的房源'];
            echo json_encode($res);
            return;
        }
        ...
        $likeHouseNum = $this->sell_house_model->get_sell_house_num_by_cond($condWhere);
        if ($likeHouseNum > 9) {
        ...
        if ($this->sell_house_model->update_house(['is_set_like' => 1], array('id' => $house_id))) {
    ...
    public function cancel_like_house()
    {
        ...
        //更新该房源is_recommend字段为0
        if ($this->sell_house_model->update_house(['is_set_like' => 0], array('id' => $house_id))) {
            echo json_encode(['status' => 'success', 'msg' => '取消成功']);
        } else {
...
/* End of file sell_house.php */
/* Location: ./application/controllers/sell_house.php */
