 ...
 * @author      杨锐
 */
class Signatory_basetab_menu extends MY_Controller
{
  ...
  public function index($module_id = 0)
  {
    ...
    //分页开始
    $data['permission_tab_menu_num'] = $this->signatory_basetab_menu_model->count_by($where);
    ...
    $data['signatory_basetab_menu'] = $this->signatory_basetab_menu_model->get_list_by($where, $data['offset'], $data['pagesize']);
    $this->load->view('signatory_basetab_menu/index', $data);
  }
  ...
  public function add()
  {
    ...
    if ('add' == $submit_flag) {
      ...
      if (!empty($paramArray['module_id']) && !empty($paramArray['name']) && !empty($paramArray['url'])) {
        $addResult = $this->signatory_basetab_menu_model->insert($paramArray);
      } else {
    ...
    $this->load->view('signatory_basetab_menu/add', $data);
  }
  ...
  public function modify($id)
  {
    ...
    $submit_flag = $this->input->post('submit_flag');
    if (!empty($id)) {
      $data['signatory_basetab_menu'] = $this->signatory_basetab_menu_model->get_by_id($id);
    }
    if ('modify' == $submit_flag) {
      ...
      if (!empty($paramArray['module_id']) && !empty($paramArray['name']) && !empty($paramArray['url'])) {
        $modifyResult = $this->signatory_basetab_menu_model->update_by_id($paramArray, $id);
      } else {
        $data['mess_error'] = '权限模块/菜单名称/链接地址不能为空';
      }
    }
    ...
    $this->load->view('signatory_basetab_menu/modify', $data);
  }
  ...
  public function del($id)
  {
    ...
    if (!empty($id)) {
      $permission_tab_menuData = $this->signatory_basetab_menu_model->delete_by_id($id);
    ...
    $data['delResult'] = $delResult;
    $this->load->view('signatory_basetab_menu/del', $data);
  }
