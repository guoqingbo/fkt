...
class Permission extends MY_Controller
{
  ...
  public function index()
  {
    ...
    //获取系统默认权限
    $permission_role = $this->permission_system_group_model->get_role();
    $data_view['permission_role'] = $permission_role;
    $this->load->view('permission/index', $data_view);
  }
  ...
  public function set_role_func($role_id)
  {
    ...
    //用户默认权限角色
    $this->permission_system_group_model->set_select_fields(array("func_auth", "range"));
    $system_role_info = $this->permission_system_group_model->get_by_id($role_id);
    ...
    //权限模块、菜单、功能数组
    $this->permission_modules_model->set_select_fields(array("id", "name"));
    $module = $this->permission_modules_model->get_all_by_id("status = '1'", -1, -1, "id", "ASC");

    foreach ($module as $key => $val) {
      $this->permission_list_model->set_select_fields(array("pid", "pname", 'tab_id', 'secondtab_id'));
      $func = $this->permission_list_model->get_all_by("mid = $val[id] AND status = '1'", -1, 20, "pid");
      $func_list = array();
      foreach ($func as $v) {
        if (!isset($func_list[$v['tab_id']])) {
          if ($v['tab_id'] == 0) {
            ...
            //获取一级菜单名称
            $tab = $this->permission_modules_model->get_tab_id($v['tab_id']);
            $tab_name = $tab['name'];
          }
        ...
        if (!isset($func_list[$v['tab_id']]['list'][$v['secondtab_id']])) {
          if ($v['secondtab_id'] == 0) {
            ...
            $secondtab = $this->permission_modules_model->get_secondtab_id($v['secondtab_id']);
    ...
    if ('set' == $submit_flag) {
      ...
      if ('1' == $system_group_id) {
        //公司权限表变更对应角色权限
        $this->permission_company_group_model->update_by_id($update_array, $system_group_id);
        //门店权限表变更对应角色权限
        $num = $this->permission_agency_group_model->update_by_id($update_array, $system_group_id);
      }
      ...
      $setResult = $this->permission_system_group_model->update_by_id($paramArray, $role_id);
    }

    $data_view['setResult'] = $setResult;
    $this->load->view('permission/set_role_func', $data_view);
  ...
  public function addgroup()
  {
    ...
    $result = $this->permission_system_group_model->add_group($insert_data);
    //添加成功后初始化角色权限
    //新角色基本信息
    $group_info = $this->permission_system_group_model->get_by_id($result);
    //初始化公司表
    $company_id_arr = $this->permission_company_group_model->get_all_company_id();
    ...
    $this->permission_company_group_model->insert($insert_company);
    //初始化门店表
    $agency_id_arr = $this->permission_agency_group_model->get_all_agency_id();
    $insert_agency = array();
    if ($agency_id_arr) {
      $j = 0;
      foreach ($agency_id_arr as $key => $vo) {
        if ($vo['company_id'] > 0 && $vo['agency_id'] > 0) {
          $insert_agency[$j]['agency_id'] = $vo['agency_id'];
    ...
    $this->permission_agency_group_model->insert($insert_agency);
    echo $result;
  }
  ...
  public function get_group($id)
  {
    ...
    if ($isajax) {
      $list = $this->permission_system_group_model->get_by_id($id);
      if ($list) {
  ...
  public function del_group($id)
  {
    $isajax = $this->input->get('isajax', TRUE);
    if ($isajax) {
      $result = $this->permission_system_group_model->delete_by_id($id);
      echo json_encode($result);
    }
  }
  ...
  public function update_group()
  {
    ...
    $result = $this->permission_system_group_model->update_by_id($update_data, $id);
    echo $result;
  }
}
