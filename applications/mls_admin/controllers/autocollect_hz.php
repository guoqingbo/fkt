...
/**
 * autocollect_hz controller CLASS
 *
 * 自动采集控制器类
 *
 * @package         datacenter
 ...
 * @author          angel_in_us
 */
class Autocollect_hz extends My_Controller
{
  ...
  public function sell_ganji_house_lists()
  {
    ...
    for ($num = 1; $num <= $page; $num++) {
      if ($num == 1) {
        $url = "http://hz.ganji.com/fang5/a1/";
        $max = 4;
      } else {
        $url = "http://hz.ganji.com/fang5/a1o" . $num . "/";
        $max = 3;
      }
      $compress = 'gzip';
      ...
      foreach ($prj[1] as $key => $val) {
        ...
        $lists['url'] = "http://hz.ganji.com" . $val;
  ...
  public function sell_ganji_house()
  {
    ...
    $hash = array();
    $limit = 10;
    $like = array('url' => 'http://hz.ganji.com/fang5/');
    $orlike = array();
    ...
    foreach ($result as $key => $value) {
      ...
      if (!empty($ress)) {
        ...
        if (!empty($address)) {
          //区属
          preg_match('/置：<\/span>.*杭州<\/a>.*<a href=".*">(.*)<\/a>/siU', $con, $district);
          $data['district'] = $this->autocollect_model->con_replace(strip_tags($district[1]));
          //板块
          preg_match('/<p class="map\-top"><i class="ico\-coordinate"><\/i>杭州 \-(.*)<\/p>/siU', $con, $block);
          if (!empty($block[1])) {
          ...
          //区属
          preg_match('/<p class="map\-top"><i class="ico\-coordinate"><\/i>杭州 \-(.*)<\/p>/siU', $con, $district);
          if (!empty($district[1])) {
          ...
          $data['house_name'] = $this->autocollect_model->con_replace(strip_tags($building[1]));
          //楼盘地址
          preg_match('/<p class="map\-top"><i class="ico\-coordinate"><\/i>杭州 \-(.*)<\/p>/siU', $con, $address);
          if (!empty($address)) {
...
//					}
  ...
  public function rent_ganji_house_lists()
  {
    ...
    for ($num = 1; $num <= $page; $num++) {
      if ($num == 1) {
        $url = "http://hz.ganji.com/fang1/a1/";
      } else {
        $url = "http://hz.ganji.com/fang1/a1o" . $num . "/";
      }
      $compress = 'gzip';
      $content = $this->autocollect_model->vcurl($url, $compress);
      preg_match_all('/<li class="list-img clearfix".*>.*<a class="list.*" href="(.*)" target="_blank".*<\/li>/siU', $content, $prj);
  ...
  public function rent_ganji_house()
  {
    ...
    $hash = array();
    $limit = 10;
    $like = array('url' => 'http://hz.ganji.com/fang1/');
    $orlike = array('url' => 'http://hz.ganji.com/fang3/');
    $result = $this->autocollect_model->check_collect_house_lists($limit, $like, $orlike, $database = 'db_city');
    //开始遍历列表页中相对应的详情页=》房源信息
    foreach ($result as $key => $value) {
      $where = array('hash' => md5($value['url']));
...
//					}
  ...
  public function sell_wuba_house_lists()
  {
    ...
    for ($num = 1; $num <= $page; $num++) {
      if ($num == 1) {
        $url = "http://hz.58.com/ershoufang/0/";
        $max = 4;
      } else {
        $url = "http://hz.58.com/ershoufang/0/pn" . $num . "/";
        $max = 0;
      }
      $compress = 'gzip';
      $content = $this->autocollect_model->vcurl($url, $compress);
  ...
  public function sell_wuba_house()
  {
    ...
    $hash = array();
    $limit = 10;
    $like = array('url' => 'http://hz.58.com/ershoufang/');
    $orlike = array();
    $result = $this->autocollect_model->check_collect_house_lists($limit, $like, $orlike, $database = 'db_city');
    //开始遍历列表页中相对应的详情页=》房源信息
    foreach ($result as $key => $value) {
...
//                                  }
  ...
  public function rent_wuba_house_lists()
  {
    ...
    for ($num = 1; $num <= $page; $num++) {
      if ($num == 1) {
        $url = "http://hz.58.com/zufang/0/";
        $max = 4;
      } else {
        $url = "http://hz.58.com/zufang/0/pn" . $num . "/";
        $max = 3;
      }
      $compress = 'gzip';
      $content = $this->autocollect_model->vcurl($url, $compress);
  ...
  public function rent_wuba_house()
  {
    ...
    $hash = array();
    $limit = 10;
    $like = array('url' => 'http://hz.58.com/zufang/');
    $orlike = array();
    $result = $this->autocollect_model->check_collect_house_lists($limit, $like, $orlike, $database = 'db_city');
    //开始遍历列表页中相对应的详情页=》房源信息
    foreach ($result as $key => $value) {
...
/* End of file autocollect_hz.php */
/* Location: ./application/mls_admin/controllers/autocollect_hz.php */
