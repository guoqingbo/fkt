 ...
 * @author      kang
 */
class Rent_customer_info extends MY_Controller
{
  /**
   * 城市参数
   *
  ...
  public function index($page = 1)
  {
    ...
    //符合条件的总行数
    $this->_total_count =
      $this->rent_customer_model->get_total_by_cond($cond_where);

    $data['rent_customer_num'] = $this->_total_count;
    ...
    $customer_list = $this->rent_customer_model->get_rentcustomerlist_by_cond($cond_where, $data['offset'],
    ...
    $this->load->view('rent_customer_info/index', $data);
  }
  ...
  public function modify($id)
  {
    ...
    //获取求购信息基本配置资料
    $data['conf_customer'] = $this->rent_customer_model->get_base_conf();
    ...
    //客源数据
    $customer_info = $this->rent_customer_model->get_customer_info_by_id($id);
    ...
    $this->load->view('rent_customer_info/modify', $data);
  }
  ...
  public function modify_info()
  {
    ...
    //列表页面地址
    $url_manage = MLS_ADMIN_URL . "/rent_customer_info/index";
  ...
  public function follow($id)
  {
    ...
    $this->load->model('rent_customer_model');
    $condition = "a.type = 4 AND b.id = " . $id;
    //print_r($condition);
    //echo "<br>";
    $data['follows'] = $this->rent_customer_model->get_follows_by_cond($condition);
    //print_r($data['follows']);

    $this->load->view("rent_customer_info/follow", $data);
  }
  ...
  public function del_customer($customer_id)
  {
    ...
    if (!empty($customer_id)) {
      //$this->load->model('buy_customer_model');
      $rs = $this->rent_customer_model->upd_info_by_id($customer_id);
    ...
    echo "<script>location.href='" . MLS_ADMIN_URL . "/rent_customer_info/index';</script>";
  }
  ...
  public function set_share_time($start = 0, $limit = 1000)
  {
    ...
    $customer_list = $this->rent_customer_model->get_rentlist_by_cond($where_cond, $start, $limit);
    if (is_full_array($customer_list)) {
      $update_arr = array();
      foreach ($customer_list as $k => $v) {
        $update_arr['set_share_time'] = $v['creattime'];
        $update_result = $this->rent_customer_model->update_customerinfo_by_cond($update_arr, array('id' => $v['id']));
        echo 'id:' . $v['id'];
