 ...
 * @author
 */
class signatory_purview_modules extends MY_Controller
{
  ...
  public function index()
  {
    ...
    //分页开始
    $data['permission_menu_num'] = $this->signatory_purview_modules_model->count_by($where);
    ...
    $data['signatory_purview_modules'] = $this->signatory_purview_modules_model->get_all_by($whereAll, $data['offset'], $data['pagesize']);
    //print_r($data['signatory_purview_modules']);
    $this->load->view('signatory_purview_modules/index', $data);
  }
  ...
  public function add()
  {
    $modules = $this->signatory_purview_modules_model->get_all_by_modules();
    ...
    if ('add' == $submit_flag) {
      ...
      if (empty($paramArray['pname'])) {
        ...
        $addResult = $this->signatory_purview_modules_model->insert($paramArray);
      }
    }
...
//            $tab = $this->signatory_basetab_menu_model->get_all_by(array("module_id"=>$first_module_id),-1);
//            $data['tab'] = $tab;
//        }
    $data['addResult'] = $addResult;
    $this->load->view('signatory_purview_modules/add', $data);
  }
  ...
  public function modify($id)
  {
    ...
    $submit_flag = $this->input->post('submit_flag');
    if (!empty($id)) {
      $data['permission_menu'] = $this->signatory_purview_modules_model->get_by_id($id);
      $module = $this->signatory_purview_modules_model->get_modules_by($data['permission_menu']['mid']);
      $module_id = $module['mid'];
      //查找模块下的一级菜单
      $tab = $this->signatory_purview_modules_model->get_all_tab($module_id);
      $data['tab'] = $tab;
      //如果设置了一级菜单刚查询二级菜单
      if (isset($data['permission_menu']['tab_id']) && $data['permission_menu']['mid'] > 0) {
        $tab_id = $data['permission_menu']['tab_id'];
        $secondtab = $this->signatory_purview_modules_model->get_secondtabs_by_tab_id($tab_id);
    ...
    if ('modify' == $submit_flag) {
      ...
      if (empty($paramArray['pname'])) {
        ...
        $modifyResult = $this->signatory_purview_modules_model->update_by_id($paramArray, $id);
      }
    }
    $data['modifyResult'] = $modifyResult;
    $this->load->view('signatory_purview_modules/modify', $data);
  }
  ...
  public function get_tab_list()
  {
    ...
    $tabs = $this->signatory_purview_modules_model->get_tabs_by_module_id($module_id);
  ...
  public function get_secondtab_list()
  {
    ...
    $secondtabs = $this->signatory_purview_modules_model->get_secondtabs_by_tab_id($tab_id);
    $result = array();
  ...
  public function del_del()
  {
    ...
    if ($pid) {
      $num = $this->signatory_purview_modules_model->update_by_id($paramArray, $pid);
    }
  ...
  public function del($id)
  {
    ...
    if (!empty($id)) {
      $permission_menuData = $this->signatory_purview_modules_model->delete_by_id($id);
...
//                }
    ...
    $data['delResult'] = $delResult;
    $this->load->view('signatory_purview_modules/del', $data);
  }
