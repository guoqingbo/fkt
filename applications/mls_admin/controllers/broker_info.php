 ...
 * @author      sun
 */
class Broker_info extends MY_Controller
{
  /**
   * 每页条目数
   *
  ...
  {
    $data_view = array();
    $data_view['role'] = $_SESSION[WEB_AUTH]['role'];
    ...
    $this->load->view('broker_info/index', $data_view);
  }
  ...
  public function add()
  {
    ...
    if ($submit_flag == 'add') {
      ...
      if ($this->form_validation->run() === true) {
        ...
        if ($is_exist_phone) //已注册
        {
          ...
          if ($insert_id > 0) {
            $data_view['addResult'] = $this->broker_info_model->init_broker($insert_id, $phone);
    ...
    $this->load->view('broker_info/add', $data_view);
  }
  ...
  public function batch_add()
  {
    ...
    $data_view['js'] = load_js('mls/js/v1.0/jquery-1.8.3.min.js,'
      ...
      . 'mls/js/v1.0/jquery.validate.min.js,'
      . 'mls/js/v1.0/house.js');
    $this->load->view('broker_info/batch_add', $data_view);
  }
  ...
  public function modify($id, $type = 0)
  {
    ...
    $this_role_level = $broker_info['role_level'];
    ...
    //$code_img_url = get_qrcode(MLS_URL . '/' . $_SESSION[WEB_AUTH]["city"] . '/broker_info/broker_details/' . $broker_info['broker_id'], $_SESSION[WEB_AUTH]["city"]);
    ...
    if ($submit_flag == 'modify') {
        //注销账号，推送金品生活app同步注销;
        if ($broker_info['status'] == 2) {
            $this->load->library('Curl');
            $url = JINPIN_URL . '/user/brokerLogout';
            //生成加密签名
            $this->load->library('DES3');
            $time = time();
            $sign = $this->des3->encrypt($broker_info['broker_id'] . $time);
            $params = [
                'sign' => $sign,
                'time' => $time,
                'brokerId' => $broker_info['broker_id']
            ];
      ...
      $this->broker_model->update_by_id($broker_update_data, $broker_info['broker_id']);
      //获取权限
      ...
      if ($this_role_level != '1' && $broker_info['role_id'] != $role_id && $per_where_cond['system_group_id'] == 1) {
        //找到公司下所有的门店下的总经理权限id，
        $where_cond = array(
      ...
      $this->broker_info_model->update_by_broker_id($broker_info_update_data, $broker_info['broker_id']);

      //修改经纪人所属公司、门店时，相应房客源数据跟上
      if ($broker_info['company_id'] != $company_id) {
        //出售
        $this->load->model('sell_house_model');
        $this->sell_house_model->change_company_id_by_borker_id($broker_info['broker_id'], $company_id);
        //出租
        $this->load->model('rent_house_model');
        $this->rent_house_model->change_company_id_by_borker_id($broker_info['broker_id'], $company_id);
        //求购
        $this->load->model('buy_customer_model');
        $this->buy_customer_model->update_private_customer_info_by_companyid($broker_info['broker_id'], $company_id);
        //求租
        $this->load->model('rent_customer_model');
        $this->rent_customer_model->update_private_customer_info_by_companyid($broker_info['broker_id'], $company_id);
      }
      ...
      $register_id = $this->input->post('register_id');
      if ($register_id > 0) $this->broker_info_model->update_register_broker_status($register_id);

      /***
       * if($package_id == '1'){
       * $this->broker_info_model->update_agent_roleid($broker_info['broker_id'],$broker_info['company_id']);
       * }**/
      echo '修改成功！';
      if ('2' == $group_id) {
        $ag_status = 4;
      ...
      if ($broker_info['agency_id']) {
        ...
        if ($broker_info['company_id'] != 0) {
          $this->load->model('agency_model');
          $company_info = $this->agency_model->get_by_id($broker_info['company_id']);
          $broker_info['company_name'] = $company_info['name'];
          $broker_info['company_id'] = $company_info['id'];
          $company_children = $this->agency_model->get_children_by_company_id($company_info['id']);
          $broker_info['agencys'] = $company_children;
        } else {
      ...
      $broker_role_level = $broker_info['role_level'];
      ...
      $data_view['broker_info'] = $broker_info;
      //if($broker_info['company_id']<>0){
      //    $data['permission_initialize_num'] = $this->broker_info_model->permission_initialize_num($broker_info['company_id']);
      //}
  ...
  public function enter_pc($broker_id)
  {
    //查找经纪人所属的城市
    $this->load->model('broker_model');
    $result = $this->broker_model->get_by_id($broker_id);
  ...
  public function exportReport()
  {
    ...
    //分页开始
    $data_view['pagesize'] = $this->broker_info_model->count_by($where);
    //计算记录偏移量
    $data_view['offset'] = 0;
    //经纪人列表
    $broker_info = $this->broker_info_model->get_all_by($where, $data_view['offset'], $data_view['pagesize']);
    ...
    if (is_full_array($broker_info)) {
      ...
      foreach ($broker_info as $key => $value) {
        $broker_info[$key]['group_str'] = $group[$value['group_id']];
        $broker_info[$key]['package_str'] = $permission_arr[$value['role_level']];
        ...
        if ($value['group_id'] == 1) {
          ...
          //获取经纪人所属公司名称
          $company_one = $value['company_id'] > 0 ? $this->agency_model->get_by_id($value['company_id']) : array();
          $broker_info[$key]['company_name'] = is_full_array($company_one) ? $company_one['name'] : '';

          //获取经纪人所属门店名称
          $agency_one = $value['agency_id'] > 0 ? $this->agency_model->get_by_id($value['agency_id']) : array();
          $broker_info[$key]['agency_name'] = is_full_array($agency_one) ? $agency_one['name'] : '';
        }
        $broker_info[$key]['status_str'] = $value['status'] == 1 ? '有效' : '无效';

        $broker_info[$key]['regtime'] = date('Y-m-d H:i:s', $value['register_time']);

        $last_login = $this->broker_login_log_model->get_last_log($value['phone']);
        $broker_info[$key]['last_login'] = $last_login[0]['dateline'] > 0 ? date('Y-m-d H:i:s', $last_login[0]['dateline']) : '尚未登录';

        $broker_info[$key]['no_login_days'] = $last_login[0]['dateline'] > 0 ? floor(($nowtime - $last_login[0]['dateline']) / 86400) : '尚未登录';
        $broker_info[$key]['infofrom'] = '';
        if ($last_login[0]['dateline'] > 0) {
          $broker_info[$key]['infofrom'] = $last_login[0]['infofrom'] == 1 ? 'PC' : 'APP';
        }
        $broker_info[$key]['master'] = $masters[$value['master_id']]['truename'];
        if ($value['auth_time'] > 0) {
          $broker_info[$key]['authtime'] = date('Y-m-d H:i:s', $value['auth_time']);
        }
        $broker_info[$key]['level'] = $this->api_broker_level_base_model->get_level($value['level']);
      }
    }
    ...
    //设置表格的值
    for ($i = 2; $i <= count($broker_info) + 1; $i++) {
      $objPHPExcel->getActiveSheet()->setCellValue('A' . $i, $broker_info[$i - 2]['broker_id']);
      $objPHPExcel->getActiveSheet()->setCellValue('B' . $i, $broker_info[$i - 2]['truename']);
      $objPHPExcel->getActiveSheet()->setCellValue('C' . $i, $broker_info[$i - 2]['phone']);
      $objPHPExcel->getActiveSheet()->setCellValue('D' . $i, $broker_info[$i - 2]['company_name']);
      $objPHPExcel->getActiveSheet()->setCellValue('E' . $i, $broker_info[$i - 2]['agency_name']);
      $objPHPExcel->getActiveSheet()->setCellValue('F' . $i, $broker_info[$i - 2]['package_str']);
      $objPHPExcel->getActiveSheet()->setCellValue('G' . $i, $broker_info[$i - 2]['group_str']);
      $objPHPExcel->getActiveSheet()->setCellValue('H' . $i, $broker_info[$i - 2]['status_str']);
      $objPHPExcel->getActiveSheet()->setCellValue('I' . $i, $broker_info[$i - 2]['regtime']);
      $objPHPExcel->getActiveSheet()->setCellValue('J' . $i, $broker_info[$i - 2]['last_login']);
      $objPHPExcel->getActiveSheet()->setCellValue('K' . $i, $broker_info[$i - 2]['no_login_days']);
      $objPHPExcel->getActiveSheet()->setCellValue('L' . $i, $broker_info[$i - 2]['infofrom']);
      $objPHPExcel->getActiveSheet()->setCellValue('M' . $i, $broker_info[$i - 2]['master']);
      $objPHPExcel->getActiveSheet()->setCellValue('N' . $i, $broker_info[$i - 2]['authtime']);
      $objPHPExcel->getActiveSheet()->setCellValue('O' . $i, 'Lv ' . $broker_info[$i - 2]['level']['level']);
      $objPHPExcel->getActiveSheet()->setCellValue('P' . $i, $broker_info[$i - 2]['credit']);
    }
  ...
  public function import($type)
  {
    if (!empty($_POST['sub'])) {
      ...
      if ($this->upload->do_upload('upfile')) {
        ...
        if ($type == 1) {
          $result = $this->read_model->read_house('sell_model', $broker_info, $data['upload_data'], 2, 1);
          unlink($data['upload_data']['full_path']); //删除文件
        } elseif ($type == 2) {
          $result = $this->read_model->read_house('rent_house_model', $broker_info, $data['upload_data'], 2, 1);
          unlink($data['upload_data']['full_path']); //删除文件
        }
      } else {
  ...
  public function broker_sure()
  {
    ...
    $data['where']['id'] = $id;
    $result = $this->broker_info_model->get_tmp($data['where']);
      ...
      foreach ($content as $key => $value) {
        ...
        if ($is_exist_phone == 0) //已注册
        {
          ...
          if ($insert_id > 0) {
            $role_id = 445;
            $role_level = 10;
            $add_result = $this->broker_info_model->batch_init_broker($insert_id, $phone, $agency_id, $role_id, $role_level, $truename);
  ...
  /*public function check_house($block_id,$door,$unit,$dong){
            ...
            $agency_ids = implode(',',$arr_agency_id);
            $cond_where = "status != 5 and agency_id in (".$agency_ids.") and block_id = $block_id and door = '$door' ";
            $tbl="rent_house";
            $this->rent_house_model->set_tbl($tbl);
            $house_num = $this->rent_house_model->get_housenum_by_cond($cond_where);
