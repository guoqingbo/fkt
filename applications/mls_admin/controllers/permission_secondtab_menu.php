 ...
 * @author      杨锐
 */
class permission_secondtab_menu extends MY_Controller
{
  ...
  public function __construct()
  {
    ...
    $this->load->model('permission_secondtab_menu_model');
  ...
  public function index($module_id = 0, $tab_id = 0)
  {
    ...
    if ($module_id) {
      $data['tab_list'] = $this->permission_tab_menu_model->get_all_by("module_id = {$module_id}", -1);
      $tab_ids = array();
      $tab_ids[] = 0;
    ...
    //分页开始
    $data['permission_secondtab_menu_num'] = $this->permission_secondtab_menu_model->count_by($where);
    ...
    $permission_secondtab_menu = $this->permission_secondtab_menu_model->get_list_by($where, $data['offset'], $data['pagesize']);
  ...
  public function add($module_id = 0, $tab_id = 0)
  {
    ...
    if ('add' == $submit_flag) {
      ...
      if (!empty($paramArray['tab_id']) && !empty($paramArray['name'])
        ...
        $addResult = $this->permission_secondtab_menu_model->insert($paramArray);
      } else {
  ...
  public function modify($id)
  {
    ...
    $submit_flag = $this->input->post('submit_flag');
    if (!empty($id)) {
      $data['permission_secondtab_menu'] = $this->permission_secondtab_menu_model->get_by_id($id);
    ...
    if ('modify' == $submit_flag) {
      ...
      if (!empty($paramArray['tab_id']) && !empty($paramArray['name'])
        ...
        $modifyResult = $this->permission_secondtab_menu_model->update_by_id($paramArray, $id);
      } else {
        $data['mess_error'] = '菜单/功能名称/类名/方法名不能为空';
      }
    }
    //权限模块数据
    $data['module_list'] = $this->permission_module_model->get_all_by("init_auth = '0'", -1);
    $menu_info = $this->permission_tab_menu_model->get_by_id($data['permission_secondtab_menu']['tab_id']);
  ...
  public function del($id, $module_id = 0, $tab_id = 0)
  {
    ...
    if (!empty($id)) {
      $permission_secondtab_menuData = $this->permission_secondtab_menu_model->delete_by_id($id);
  ...
  public function get_tab_list()
  {
    ...
    if ($module_id) {
      ...
      $list = $this->permission_tab_menu_model->get_all_by(array("module_id" => $module_id), -1);
