                        ...
                        <div class="panel-body">
                            <div class="table-responsive">
                                                ...
                                                <div class="dataTables_length" id="dataTables-example_length">
                                                    <label>&nbsp&nbsp 区属
                                                        <select name="dist_id" aria-controls="dataTables-example" class="form-control input-sm">
                                                            ...
                                                            <option value="<?php echo $v['id'];?>"<?php if(isset($where_cond['dist_id'])){if($v['id']==$where_cond['dist_id']){echo 'selected="selected"';}}?>><?php echo $v['district'];?></option>
                                                            <?php }?>
                                                        </select>
                                                    </label>
                                                    <label>
                                ...
                                <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                                    ...
                                    <tbody>
                                    <?php
                                        if(isset($street_list2) && !empty($street_list2)){
                                            foreach($street_list2 as $key=>$value){
                                    ?>
                                        <tr class="gradeA">
                                            <td><?php echo $value['id'];?></td>
                                            <td><?php echo $value['streetname'];?></td>
                                            <td><?php echo $value['name_spell'];?></td>
                                            <td><?php echo $value['dist_name'];?></td>
                                            <td><?php echo $value['order'];?></td>
                                            <td><?php echo ($value['is_show']==1)?'是':'否';?></td>
											<td><?php echo $value['b_map_x'];?></td>
											<td><?php echo $value['b_map_y'];?></td>
                                            <td>
                                            	<a href="<?php echo MLS_ADMIN_URL;?>/street/modify/<?php echo $value['id'];?>" >修改</a>
                                            </td>
                                        </tr>
                                    <?php }}?>
