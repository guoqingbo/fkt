                            ...
                            <div class="table-responsive">
                                ...
                                <div role="grid" class="dataTables_wrapper form-inline" id="dataTables-example_wrapper">
                                    ...
                                    <div class="col-sm-6" style="width:100%">
                                        <div class="dataTables_length" id="dataTables-example_length">
                                            <label>
                                             板块名<font color="red">*</font>:&nbsp&nbsp&nbsp&nbsp<input type="search" name="streetname" class="form-control input-sm" aria-controls="dataTables-example" value="<?php echo $street_data['streetname']?>">
                                            </label>
                                            <label>
                                             板块拼音<font color="red">*</font>:&nbsp&nbsp&nbsp&nbsp<input type="search" name="name_spell" class="form-control input-sm" aria-controls="dataTables-example" value="<?php echo $street_data['name_spell']?>">
                                            </label>
                                            <label>
                                                对应58区属板块:&nbsp&nbsp&nbsp
                                                <select id="wb_district" name = 'wb_dist' class="form-control input-sm" aria-controls="dataTables-example">
                                                    <option value="0">请选择...</option>
                                                    <?php foreach ($wb_district as $value) {?>
                                                        <option value="<?=$value['district_id'];?>" <?php if($value['district_id']==$message['wuba_dist_id']){echo 'selected="selected"';}?> ><?=$value['district_name'];?></option>
                                                    <?php }?>
                                                </select>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp
                                                <select id="wb_street" name = 'wb_street' class="form-control input-sm" aria-controls="dataTables-example">
                                                    <option value="0" >请选择...</option>
                                                    <?php foreach ($wb_street as $value) {?>
                                                        <option value="<?=$value['street_id'];?>" <?php if($value['street_id']==$message['wuba_street_id']){echo 'selected="selected"';}?> ><?=$value['street_name'];?></option>
                                                    <?php }?>
                                                </select>
                                            </label>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp
                                            <label>
                                                对应赶集区属板块:&nbsp&nbsp&nbsp
                                                <select id="gj_district" name = 'gj_dist' class="form-control input-sm" aria-controls="dataTables-example">
                                                    <option value="0">请选择...</option>
                                                    <?php foreach ($gj_district as $value) {?>
                                                        <option value="<?=$value['district_id'];?>" <?php if($value['district_id']==$message['ganji_dist_id']){echo 'selected="selected"';}?>><?=$value['district_name'];?></option>
                                                    <?php }?>
                                                </select>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp
                                                <select id="gj_street" name = 'gj_street' class="form-control input-sm" aria-controls="dataTables-example">
                                                    <option value="0">请选择...</option>
                                                     <?php foreach ($gj_street as $value) {?>
                                                        <option value="<?=$value['street_id'];?>" <?php if($value['street_id']==$message['ganji_street_id']){echo 'selected="selected"';}?> ><?=$value['street_name'];?></option>
                                                    <?php }?>
                                                </select>
                                            </label>
                                        </div>
                                    ...
                                    <script>
                                        $('#wb_district').change(function(){
                                            ...
                                            $.ajax({
                                                type: 'post',
                                                url : '<?php echo MLS_ADMIN_URL; ?>/street/find_street_bydis/',
                                                data:{districtID:districtID,type:1},
                                                dataType:'json',
                                                success: function(msg){
                                                    ...
                                                    if(msg.result=='no result'){
                                                        ...
                                                        for(var i=0;i<msg.length;i++){
                                                            str +='<option value="'+msg[i].street_id+'">'+msg[i].street_name+'</option>';
                                                        }
                                                    }
                                                    $('#wb_street').empty();
                                        ...
                                        $('#gj_district').change(function(){
                                            var districtID = $(this).val();
                                            $.ajax({
                                                type: 'post',
                                                url : '<?php echo MLS_ADMIN_URL; ?>/street/find_street_bydis/',
                                                data:{districtID:districtID,type:2},
                                                dataType:'json',
                                                success: function(msg){
                                                    var str = '';
                                                    if(msg.result=='no result'){
                                    ...
                                    <div class="col-sm-6" style="width:100%">
                                        <div class="dataTables_length" id="dataTables-example_length">
                                            <label>
                                            所属区属：
                                            <select name="dist_id" class="form-control input-sm" aria-controls="dataTables-example">
                                            <?php  foreach($all_district as $k => $v){?>
                                            <option value="<?php echo $v['id'];?>" <?php if($v['id']==$street_data['dist_id']){echo 'selected="selected"';}?>><?php echo $v['district'];?></option>
                                            <?php }?>
                                            </select>
                                            </label>
                                        </div>
                                    ...
                                    <div class="col-sm-6" style="width:100%">
                                        <div class="dataTables_length" id="dataTables-example_length">
                                            <label>
                                             排序:&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp<input type="search" name="order" class="form-control input-sm" aria-controls="dataTables-example" value="<?php echo $street_data['order']?>">
                                            </label>
                                        </div>
                                    </div>
                                    <div class="col-sm-6" style="width:100%">
                                        <div class="dataTables_length" id="dataTables-example_length">
                                            <label>
                                                ...
                                                <select name="is_show" class="form-control input-sm" aria-controls="dataTables-example">
                                                    <option value="1"<?php if($street_data['is_show']=='1'){echo 'selected="selected"';}?>>有效</option>
                                                    <option value="2"<?php if($street_data['is_show']!='1'){echo 'selected="selected"';}?>>无效</option>
                                                </select>
                                            </label>
                                        </div>
                                    </div>
                                    <div class="col-sm-6" style="width:100%">
                                        <div class="dataTables_length" id="dataTables-example_length">
                                            ...
                                            <input type="text" id="txtCity" value="" aria-controls="dataTables-example" class="form-control input-sm " style="width:180px;display: inline-block;" >
                                                &nbsp;&nbsp;&nbsp;&nbsp;
                                                <span class="text1"><font color="red">*</font>经度:</span><input type="text" value="<?php echo $street_data['b_map_x'];?>" aria-controls="dataTables-example" class="form-control input-sm " style="width:90px;display: inline-block;"  name="b_map_x" id="b_map_x"> <!--<input type="search" name="b_map_x" class="form-control input-sm" aria-controls="dataTables-example" value=""/>-->
                                                &nbsp;&nbsp;&nbsp;&nbsp;
                                                <span class="text1"><font color="red">*</font>纬度:</span><input type="text" value="<?php echo $street_data['b_map_y'];?>" aria-controls="dataTables-example" class="form-control input-sm " style="width:90px;display: inline-block;"  name="b_map_y" id="b_map_y"><!--<input type="search" name="b_map_y" class="form-control input-sm" aria-controls="dataTables-example" value=""/>-->
                                                &nbsp;&nbsp;
                                            </label>
                                        </div>
                                    </div>
                                    ...
                                    <script type="text/javascript">
                                            var lng = "<?php echo $street_data['b_map_x'] > 0 ? $street_data['b_map_x'] : $lng;?>";
                                            var lat = "<?php echo $street_data['b_map_y'] > 0 ? $street_data['b_map_y'] : $lat;?>";
                                            // 百度地图API功能
