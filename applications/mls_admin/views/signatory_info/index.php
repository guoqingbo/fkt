                                                    ...
                                                    <select name="department_id" id="department_id"
                                                        ...
                                                        <?php if ($departments) {
                                                            foreach ($departments as $v) { ?>
                                                                <option value="<?= $v['id'] ?>" <?php if ((!empty($where_cond['department_id']) && $where_cond['department_id'] == $v['id'])) {
...
<!--                                                        <input class="btn btn-primary" onclick="$('#search_form').attr('action', '/broker_info/index/')" type="submit" value="查询">-->
                                                        <?php if(!$is_user_manager){ ?><a class="btn btn-primary" href='/signatory_info/add'>添加</a><?php }?>
                                                        <?php if($role=='3' || $role=='4'){ ?>
<!--														<input class="btn btn-primary" onclick="$('#search_form').attr('action', '/signatory_info/exportReport/')" type="submit" value="导出">-->
                                                        <?php } ?>
                                                    </div>
                                                </label>
                                            </div>
                        ...
                        <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                            ...
                            <tbody>
                            <?php if (isset($signatory_info) && !empty($signatory_info)) {
                                foreach ($signatory_info as $key => $value) { ?>
                                    <tr class="gradeA">
                                        <td><a href="#" target="_blank"><?php echo $value['signatory_id']; ?></a></td>
                                        ...
                                        <td>
                                            <?php if(!$is_user_manager){ ?>
                                                <a href="<?php echo MLS_ADMIN_URL; ?>/signatory_info/modify/<?php echo $value['id']; ?>/<?php echo $type = 0; ?>"
                                                   target="_blank">修改</a>
                                            <?php }else{ ?>
<!--                                                <a href="--><?php //echo MLS_ADMIN_URL; ?><!--/signatory_info/modify/--><?php //echo $value['id']; ?><!--/--><?php //echo $type=1; ?><!--" target="_blank">查看</a> |-->
                                            <?php } ?>
                                        </td>
                                    </tr>
                            <?php }} ?>
                            ...
                            <div class="col-sm-6 clearfix" style="width:100%;">
                                ...
                                <div class="dataTables_paginate paging_simple_numbers" id="dataTables-example_paginate">
                                    <ul class="pagination" style="margin:-8px 0;padding-left:20px">
                                        <?php echo page_uri($page,$pages,MLS_ADMIN_URL.'/signatory_info/');?>
                                    </ul>
                                </div>
                            </div>
                         </div>
...
<div id="jss_pop_import" class="pop_box_g pop_see_inform" style=" display:none;" >
    <div class="hd">
        ...
        <div class="close_pop"><a href="/signatory_info/" title="关闭" class="JS_Close iconfont">&#xe60c;</a></div>
    </div>
    <div class="mod">
        ...
        <div class="up_m_b_file clearfix" id='import_form'>
            <form action="/signatory_info/import/1" enctype="multipart/form-data" target="new" method="post">
...
function openn_import_new(type,signatory_id)
{
	if(type == 'rent'){
		$("#import_title").html('出租房源导入');
		$('#import_form form').attr("action", "/signatory_info/import/2");
...
function openn_sure_new(type)
{
     ...
     openWin('jss_pop_sure',ajax_import_new(id,type,signatory_id));
     }else{
        openWin('jss_pop_error');
    }
}

function ajax_import_new(id,type,signatory_id)
{
    var url = "/signatory_info/sure/"+type;

     $.ajax({
           url: url,
           type: "POST",
           dataType: "json",
           data: {id:id,signatory_id:signatory_id},
           success: function(data) {
               if(data.status == 'ok')
               {
