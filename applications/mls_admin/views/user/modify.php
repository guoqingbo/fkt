							  ...
							  <div class="row">
						        ...
						        <div class="col-sm-6" style="width:100%">
								    <div class="dataTables_length" id="dataTables-example_length">
									  ...
									  <label>
									   用户名:&nbsp&nbsp&nbsp&nbsp<input type="search" name="username" class="form-control input-sm" aria-controls="dataTables-example" value="<?php echo $user['username']?>">
									  </label>
									  </div>
									  </div>								  
						        <div class="col-sm-6" style="width:100%">
								    <div class="dataTables_length" id="dataTables-example_length">
									  <label>
									   真实姓名:<input type="search" name="truename" class="form-control input-sm" aria-controls="dataTables-example" value="<?php echo $user['truename']?>">
									  </label>
									  </div>
									  </div>
						        <div class="col-sm-6" style="width:100%">
								    <div class="dataTables_length" id="dataTables-example_length">
									  <label>
									   用户组:
                                            ...
                                            <?php foreach($user_group_data as $k => $v){?>
                                                    <label>&nbsp&nbsp <?php echo $v['city_name'];?>
                                                        <select name="user_group<?php echo $k;?>" aria-controls="dataTables-example" class="form-control input-sm">
                                                            <option value="0">本站不开通</option>
                                                            <?php foreach($v['user_group'] as $k=>$v){ ?>
                                                            <option value="<?php echo $v['id']?>"<?php if(strstr($user['user_group_ids'], ','.$v['id'].',')!=false){echo 'selected=selected';}?>><?php echo $v['name']?></option>
                                                            <?php } ?>
                                                        </select> 
                                                    </label>
                                            <?php }?>
						        <div class="col-sm-6" style="width:100%">
								    <div class="dataTables_length" id="dataTables-example_length">
									  <label>
									   联系方式:<input type="search" name="telno" class="form-control input-sm" aria-controls="dataTables-example" value="<?php echo $user['telno']?>">
									  </label>
									  </div>
									  </div>
								<div class="col-sm-6" style="width:100%">
								    <div class="dataTables_length" id="dataTables-example_length">
                                      ...
                                      <input type="text" style="display:none;">
									   密码:<input type="password" name="password" class="form-control input-sm" aria-controls="dataTables-example" value="" autocomplete ="off">
									  </label>
									</div>
								</div>
						        <div class="col-sm-6" style="width:100%">
								    <div class="dataTables_length" id="dataTables-example_length">
									  <label>
									   备注:&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp<input type="search" name="message" class="form-control input-sm" aria-controls="dataTables-example" value="<?php echo $user['message']?>">
									  </label>
									  </div>
									  </div>	
						        <div class="col-sm-6" style="width:100%; height:30px; line-height:30px;">
								    <div class="dataTables_length" id="dataTables-example_length">
									  <label>
									    客户经理:&nbsp;&nbsp;&nbsp;&nbsp;
                                        <input type='radio' name='master' value='2' <?php if ($user['am_cityid'] == 0) {echo 'checked';}?>/>&nbsp;否&nbsp;
                                        <input type='radio' name='master' value='1' <?php if ($user['am_cityid'] != 0) {echo 'checked';}?>/>&nbsp;是&nbsp;
                                        <select name="am_cityid" aria-controls="dataTables-example" class="form-control input-sm" style="<?php if ($user['am_cityid'] == 0) {echo 'display:none;';} else {echo 'display:inline;';}?>" id="am_cityid">
                                            <?php foreach($city_arr as $k=>$v){ ?>
                                            <option value="<?php echo $v['city_id']?>" <?php if ($user['am_cityid'] == $v['city_id']) {echo 'selected';}?>><?php echo $v['name']?></option>
                                            <?php } ?>
                                        </select> 
									  </label>
									  </div>
...
<script type="text/javascript">
    $(function(){
        ...
        $("input[name='master']").click(function(){
            if ($(this).val() == 1) {
                $('#am_cityid').css('display', 'inline');
            } else {
                $('#am_cityid').css('display', 'none');
            }
        });
    });
</script>
