...
<link href="<?=MLS_SOURCE_URL ?>/min/?b=mls&f=css/v1.0/base.css,third/iconfont/iconfont.css,css/v1.0/house_manage.css,css/v1.0/contract_manage.css" rel="stylesheet" type="text/css">
...
<!--权证添加和修改弹窗-->
<div class="achievement_money_pop qz_add_step_w610" id="js_temp_pop" style="width:842px;height:502px;border-color: #3E444;">
    <dl class="title_top" style="background: #3E444B;width:100%">
        <dd id="title_top3">查看图片</dd>
    </dl>
    <div style="overflow-y: auto;overflow-X: hidden;width: 100%;">
        <div class="col-sm-6" style="width:100%">室内图：
            <?php foreach($shinei as $key=>$val){?>
                <img width="150" height="100" broker="1" src="<?=$val['url']?>">
            <?php }?>
        </div>
        <div class="col-sm-6" style="width:100%">户型图：
            <?php foreach($huxing as $key=>$val){?>
                <img width="150" height="100" broker="1" src="<?=$val['url']?>">
            <?php }?>
        </div>
    </div>
        ...
        Bigic.prototype.init = function(){
            ...
            this.$obj.off('.bigic').on('click.bigic', function(){
                ...
                if(sTagName == 'img'){
                    ...
                    oThis.sImgSrc = oThis.sImgSrc.replace('thumb/',"")
