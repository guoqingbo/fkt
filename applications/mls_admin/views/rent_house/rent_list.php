                        ...
                        <div class="table-responsive">
                            <form name="search_form" method="post" action="" id="search_form">
                                <div role="grid" class="dataTables_wrapper form-inline" id="dataTables-example_wrapper">
                                    ...
                                    <div class="col-sm-6" style="width:100%">
                                        ...
                                        <input name="company_name" id="company_name"
                                               ...
                                               type="text" placeholder="输入汉字筛选" style="height:30px; line-height: 30px;">
                                        ...
                                        <input type="text" name="search_value" id="search_value" style="width:183px"
                                               class="form-control input-sm" aria-controls="dataTables-example" value=""
                                        <?php
                                ...
                                <input class="btn btn-primary" type="submit" value="查询">&nbsp;&nbsp;&nbsp;<input
                                        ...
                                        onclick="javascript:location.href = '/rent_house/'" value="重置">
                                <!--<a class="btn btn-primary" href="/rent_house/add">添加</a>-->
                            </form>
                        </div>
                            ...
                            if (isset($sell_list) && !empty($sell_list)) {
                                foreach ($sell_list as $key => $value) {
                                    ...
                                    <tr class="gradeA">
                                        ...
                                        <td><?php echo $value['sell_type']; ?></td>
                                        ...
                                        <td>
                                            <?php if ($value['price_danwei'] > 0) {
                                                echo $value['price_buildarea'];//($value['price']/$value['buildarea']/30)."元/㎡*天";
                                            } else {
                                                echo $value['price'];
                                                /*floor($value['price'])==$value['price']?intval($value['price'])."元/月":$value['price']."元/月";*/
                                            }
                                        ...
                                        <td>
                                            ...
                                            <!--                                                <a href="-->
                                            <?php //echo MLS_ADMIN_URL; ?><!--/rent_house/del_sell/-->
                                            <?php //echo $value['id']; ?><!--">关联失效</a>-->
                                            <!--                                                --><?php //if('0'==$value['is_kft']){ ?>
                                            <!--                                                    <a href="#" onclick="add_data(--><?php //echo $value['id']; ?>
...
<script type="text/javascript">
    function add_data(house_id, price) {
        layer.open({
            ...
            area: ['750px', '400px'],
            content: '/rent_house/kft/' + house_id + '/' + price,
            end: function () {
                $("#search_form").submit();
            }
        });
    ...
    function districtchange(districtid) {
        $.ajax({
            type: 'get',
            url: '/rent_house/find_street_bydis/' + districtid,
            dataType: 'json',
            success: function (msg) {
                var str = '';
        ...
        if (real_price == "") {
            ...
            $.ajax({
                ...
                success: function (msg) {
                    if (msg == '123') {
                        alert('改动失败，请稍后重试~！');
                        location.href = '<?=MLS_ADMIN_URL?>/rent_house_sold/';
                        return;
                    } else {
                        location.href = '<?=MLS_ADMIN_URL?>/rent_house_sold/';
                        return;
    ...
    function add_recommend_house(house_id) {
        $.ajax({
            type: 'POST',
            url: '<?=MLS_ADMIN_URL?>/rent_house/add_recommend_house',
            data: {'house_id': house_id},
            dataType: "json",
            success: function (data) {
                if (data.status == "success") {
                    layer.msg(data.msg);
                    location.href = '<?=MLS_ADMIN_URL?>/rent_house/';
                } else {
                    layer.msg(data.msg);
                }
            }
    ...
    function cancel_recommend_house(house_id, recommend_house_id) {
        $.ajax({
            type: 'POST',
            url: '<?=MLS_ADMIN_URL?>/rent_house/cancel_recommend_house',
    ...
    function add_like_house(house_id) {
        $.ajax({
            type: 'POST',
            url: '<?=MLS_ADMIN_URL?>/rent_house/add_like_house',
            data: {'house_id': house_id},
            dataType: "json",
            success: function (data) {
                if (data.status == "success") {
    ...
    function cancel_like_house(house_id) {
        $.ajax({
            type: 'POST',
            url: '<?=MLS_ADMIN_URL?>/rent_house/cancel_like_house',
            data: {'house_id': house_id},
            dataType: "json",
            success: function (data) {
                if (data.status == "success") {
