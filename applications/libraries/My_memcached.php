...
class My_memcached
{
  ...
  private $connstate = FALSE; //mc连接状态（非必要时不连接状态）
  ...
  */
  public function add_server($server)
  {
    if ($this->connstate === FALSE) {
      extract($server);
      $this->connstate = $this->client_type == 'Memcache' ? $this->m->connect($host, $port) : $this->m->addServer($host, $port, $weight);
    }
