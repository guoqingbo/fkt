 ...
 * @author      angel_in_us
 */
class House_collections extends MY_Controller
{
  /**
   * 城市参数
   *
  ...
  public function collect_sell($page = 1)
  {
    ...
    $judge = $this->collections_model->get_agent_house($arr);
    $house_ids = array();
...
//        {
    ...
    $judge = $this->collections_model->get_agent_house($arr);
    //符合条件的总行数
    $this->_total_count = $count = $this->collections_model->get_sell_num($data['where_cond'], $data['like'], $data['or_like']);
    //计算总页数
    ...
    $data['blacklist'] = $this->collections_model->get_house_sell($data['where_cond'], $data['where_in'] = array(), $data['like'], $data['or_like'], $data['order_by'], $this->_limit, $this->_offset);
    //$data['blacklist_all'] = $this->collections_model->get_house_sell_allids();
    $k = 0;
...
//		}
    ...
    $judge = $this->collections_model->get_agent_house($arr);
    //符合条件的总行数
    $this->_total_count = $count = $this->collections_model->get_rent_num($data['where_cond'], $data['like'], $data['or_like']);
    //计算总页数
    $pages = $this->_total_count > 0 ? ceil($this->_total_count / $this->_limit) : 0;
    //分页处理000000000000
    $params = array(
    ...
    $data['page_list'] = $this->page_list->show('jump');
    $data['order_by'] = 'createtime';
    $data['blacklist'] = $this->collections_model->get_house_rent($data['where_cond'], $data['where_in'] = array(), $data['like'], $data['or_like'], $data['order_by'], $this->_offset, $this->_limit);
    //$data['blacklist_all'] = $this->collections_model->get_house_rent_allids();
  ...
  public function good_sell_details()
  {
    ...
    if (!empty($house_id)) {
      ...
      $data['house_info'] = $this->collections_model->get_housesell_byid($data['where_cond']);
      //获取房源详情
  ...
  public function good_sell_phone()
  {
    ...
    if (!empty($house_id) && !empty($broker_id) && !empty($tbl_name)) {
      ...
      $result = $this->collections_model->check_agent_house($where);
      $ahouse_info = array(
        'house_id' => $house_id,
        'broker_id' => $broker_id,
        'tbl_name' => $tbl_name,
      ...
      if (!empty($result)) {
        ...
        $this->collections_model->add_agent_house($ahouse_info);
      }
    }
    $data['conf_where'] = 'index';
    $data['where_cond'] = array();
  ...
  public function good_rent_details()
  {
    ...
    if (!empty($house_id)) {
      $data['where_cond'] = array('id' => $house_id);
      $data['house_info'] = $this->collections_model->get_houserent_byid($data['where_cond']);//获取获取房源详情
      if (!empty($data['house_info'])) {
        foreach ($data['house_info'] as $key => $v) {
          //数据重构
          $info = array();
  ...
  public function good_rent_phone()
  {
    ...
    if (!empty($house_id)) {
      $data['where_cond'] = array('id' => $house_id);
      $data['house_info'] = $this->collections_model->get_houserent_byid($data['where_cond']);
      //获取房源详情
      if (!empty($data['house_info'])) {
        foreach ($data['house_info'] as $key => $v) {
          //数据重构
