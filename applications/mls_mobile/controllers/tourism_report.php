 ...
 * @author      Lion
 */
class Tourism_report extends MY_Controller
{
  ...
  public function __construct()
  {
    ...
    //加载海外报备模型类
    $this->load->model('tourism_report_model');
  }
  ...
  public function index()
  {
     ...
     */
    //符合条件的总行数
    $this->_total_count = $this->tourism_report_model->count_by($cond_where);

    //计算总页数
    $pages = $this->_total_count > 0 ? ceil($this->_total_count / $this->_limit) : 0;

    //获取列表内容
    $list = $this->tourism_report_model->get_all_by($cond_where, $this->_offset, $this->_limit);
    ...
    if (is_full_array($list)) {
      foreach ($list as $key => $val) {
        ...
        if ($val['house_id']) {
          $house_info = $this->tourism_model->get_by_id($val['house_id']);
          $list[$key]['house_info']['block_name'] = $house_info['block_name'];
          $province = $this->tourism_model->get_by_province_id($house_info['province_id']);
          $city = $this->tourism_model->get_by_city_id($house_info['city_id']);
          $list[$key]['house_info']['city_info'] = $province['province_name'] . $city['city_name'];
          $list[$key]['house_info']['avg_price'] = $house_info['avg_price'];
        } else {
  ...
  public function add()
  {
    ...
    if ($this->user_arr['group_id'] == 2) {
      if ($name && $phone) {
        ...
        $result = $this->tourism_report_model->add_info($insert_data);
        if ($result) {
          $result = 1;
          $msg = '报备成功';
        } else {
