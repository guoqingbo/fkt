 ...
 * @author      angel_in_us
 */
class My_collections_new extends MY_Controller
{
  /**
   * 城市参数
   *
  ...
  public function my_collect_sell($page = 1)
  {
    ...
    $count = $this->collections_model_new->get_sell_num($data['where_cond'], $data['like'], $data['or_like']);
    //计算总页数
    ...
    $data['blacklist'] = $this->collections_model_new->get_house_sell($data['where_cond'], $data['where_in'], $data['like'], $data['or_like'], $data['order_by'], $this->_limit, $this->_offset);
   ...
   * author yuan
   */
  public function my_collect_sell_cooperate($page = 1)
  {
    ...
    if (is_string($city_spell) && !empty($city_spell)) {
      ...
      $result_str = $this->input->get('result_str');
      $result = array();
      if (!empty($result_str)) {
        $result = unserialize($result_str);
      }
   ...
   * author yuan
   */
  public function my_collect_rent_cooperate($page = 1)
  {
    //该方法不设置登录检测，根据经纪人信息设置城市分库
    $city_spell = $this->input->get('city_spell');
    if (is_string($city_spell) && !empty($city_spell)) {
      ...
      //符合条件的总行数
      $this->_total_count = count($judge);
      $count = $this->collections_model_new->get_rent_num($data['where_cond'], $data['like'], $data['or_like']);
      //计算总页数
      $pages = $this->_total_count > 0 ? ceil($this->_total_count / $this->_limit) : 0;
      //分页处理000000000000
      $params = array(
      ...
      $data['page_list'] = $this->page_list->show('jump');
      $data['order_by'] = 'createtime';
      $data['blacklist'] = $this->collections_model_new->get_house_rent($data['where_cond'], $data['where_in'], $data['like'], $data['or_like'], $data['order_by'], $this->_limit, $this->_offset);
