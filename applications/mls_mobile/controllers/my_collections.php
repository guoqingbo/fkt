 ...
 * @author      angel_in_us
 */
class My_collections extends MY_Controller
{
  /**
   * 城市参数
   *
  ...
  public function my_collect_sell($page = 1)
  {
    ...
    $count = $this->collections_model->get_sell_num($data['where_cond'], $data['like'], $data['or_like']);
    //计算总页数
    ...
    $data['blacklist'] = $this->collections_model->get_house_sell($data['where_cond'], $data['where_in'], $data['like'], $data['or_like'], $data['order_by'], $this->_limit, $this->_offset);
  ...
  public function my_collect_report()
  {
    ...
    if (!empty($report_type) && $report_type == 1) {
      $house_info = $this->collections_model->get_housesell_byid($where_cond);

    }
    if (!empty($report_type) && $report_type == 2) {
      $house_info = $this->collections_model->get_houserent_byid($where_cond);
    }
    $rel = '';
    $telno1 = '';
    if ($house_info) {
      ...
      $rel = $this->collections_model->agent_reportlist($info);
    }
    if ($rel > 0) {
      $this->result('1', '举报成功');
    } else {
  ...
  public function my_collect_rent($page = 1)
  {
    ...
    //符合条件的总行数
    $this->_total_count = count($judge);
    $count = $this->collections_model->get_rent_num($data['where_cond'], $data['like'], $data['or_like']);
    //计算总页数
    $pages = $this->_total_count > 0 ? ceil($this->_total_count / $this->_limit) : 0;
    //分页处理000000000000
    $params = array(
    ...
    $data['page_list'] = $this->page_list->show('jump');
    $data['order_by'] = 'createtime';
    $data['blacklist'] = $this->collections_model->get_house_rent($data['where_cond'], $data['where_in'], $data['like'], $data['or_like'], $data['order_by'], $this->_limit, $this->_offset);
