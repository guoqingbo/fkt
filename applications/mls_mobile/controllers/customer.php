...
class Customer extends MY_Controller
{
  ...
  public function modify()
  {
    ...
    if ($customer_id > 0) {
      ...
      $customers = $this->buy_customer_model->get_info_by_id();
      ...
      if (is_full_array($customers)) {
        ...
        $customer_info['price_min'] = strip_end_0($customers['price_min']);
        $customer_info['price_max'] = strip_end_0($customers['price_max']);
  ...
  public function manage()
  {
    ...
    //$lists_return = $this->get_func_permission('customer','manage');
    //新权限
    //范围（1公司2门店3个人）
    if ($company_id) {
    ...
    //符合条件的总行数
    $data['total_count'] = $this->buy_customer_model->get_buynum_by_cond($cond_where);

    //获取列表内容
    $this->buy_customer_model->set_search_fields(array('id', 'truename', 'property_type', 'public_type', 'is_share', 'area_min', 'area_max', 'price_min', 'price_max', 'dist_id1', 'street_id1', 'dist_id2', 'street_id2', 'dist_id3', 'street_id3', 'cmt_name1', 'cmt_name2', 'cmt_name3', 'telno1', 'room_min', 'room_max'));
    $customer_list =
      $this->buy_customer_model->get_list_by_cond($cond_where, $this->_offset,
    ...
    if (is_full_array($customer_list)) {
      ...
      foreach ($customer_list as $key => $value) {
        ...
        $customer_list_info[$key]['price'] = strip_end_0($value['price_min']) . '-' . strip_end_0($value['price_max']);
        ...
        $customer_list_info[$key]['room'] = $value['room_min'] . '-' . $value['room_max'] . '室';
  ...
  public function manage_pub()
  {
    ...
    //获取列表内容
    $this->buy_customer_model->set_search_fields(array('id', 'broker_id', 'truename', 'property_type', 'public_type', 'is_share', 'area_min', 'area_max', 'price_min', 'price_max', 'dist_id1', 'street_id1', 'dist_id2', 'street_id2', 'dist_id3', 'street_id3', 'cmt_name1', 'cmt_name2', 'cmt_name3'));
    $customer_list =
      $this->buy_customer_model->get_list_by_cond($cond_where, $this->_offset,
        $this->_limit, $order_arr['order_key'], $order_arr['order_by']);
    $customer_list_info = array();
  ...
  public function details()
  {
    ...
    if ($customer_id > 0) {
      $this->buy_customer_model->set_id($customer_id);
      $customer_infos = $this->buy_customer_model->get_info_by_id();
      /**
       * 新权限
       * 范围（个人或全公司）
       * 获得当前数据所属的经纪人id和门店id**/
      ...
      if (is_full_array($customer_infos)) {
        ...
        $customer_info['price'] = strip_end_0($customer_infos['price_min']) . '-' . strip_end_0($customer_infos['price_max']);
        ...
        if ($type == 1) {
          ...
          $is_applay_coop = $this->cooperate_model->check_is_cooped_by_cid(array($customer_id), 'sell', $this->user_arr['broker_id']);
  ...
  public function match()
  {
    ...
    if ($customer_id > 0) {
      $this->buy_customer_model->set_id($customer_id);
      $this->buy_customer_model->set_search_fields(array('truename', 'room_min', 'room_max', 'area_min', 'area_max',
        'price_min', 'price_max', 'property_type', 'dist_id1', 'street_id1', 'dist_id2', 'street_id2',
        'dist_id3', 'street_id3'));
      $customers = $this->buy_customer_model->get_info_by_id();
      ...
      if (is_full_array($customers)) {
        ...
        $customer_info['price'] = strip_end_0($customers['price_min']) . '-' . strip_end_0($customers['price_max']);
        ...
        //获取列表内容
        $this->sell_house_model->set_search_fields(array('id', 'block_name', 'isshare', 'nature', 'sell_type',
          'district_id', 'street_id', 'fitment', 'floor', 'totalfloor', 'room', 'hall', 'buildarea', 'price'));
        $houses = $this->sell_house_model->get_list_by_cond($cond_where, $this->_offset, $this->_limit, 'updatetime', 'DESC');
        ...
        if (is_full_array($houses)) {
          ...
          foreach ($houses as $key => $value) {
            ...
            $house_list[$key]['price'] = strip_end_0($value['price']);
          }
        }
  ...
  public function get_secret_info()
  {
    ...
    if ($customer_id) {
      ...
      $this->buy_customer_model->set_search_fields($select_feilds);
      $customer_info = $this->buy_customer_model->get_info_by_id();
      ...
      $check_baomi_time = $this->broker_model->check_baomi_time($this->company_basic_arr,
        $this->user_arr, 3, $customer_id, $is_insert);
...
//        }
    ...
    if (2 == $house_customer_system) {
      ...
      $private_where_cond = 'broker_id = "' . $broker_id . '"' . ' and status = 1 and public_type = 1';
      $private_num = $this->buy_customer_model->get_buynum_by_cond($private_where_cond);
      if ('2' == $old['public_type'] && '1' == $customer_info['public_type'] && $private_num >= $buy_customer_private_num) {
        $house_private_check = false;
        $house_private_check_text = '当前门店基本设置为公盘制';
      }
    } else {
  ...
  public function del_customerinfo_by_ids()
  {
    ...
    if (is_full_array($arr_id)) {
      ...
      if ($del_num > 0) {
        ...
        $this->operate_log_model->add_operate_log($add_log_param);
        $this->result(1, '求购客源删除成功');
        return;
      } else {
        $this->result(2, '求购客源删除失败');
        return;
      }
    } else {
      $this->result(0, '参数非法');
    ...
    if (is_full_array($arr_id)) {
      ...
      if ($up_num > 0) {
        if (1 == $flag) {
          ...
          $credit_result = $this->api_broker_credit_model->publish_cooperate_house(array('id' => $customer_ids_str), 3);
          $credit_msg = '';
          if (is_full_array($credit_result) && $credit_result['status'] == 1) {
            $credit_msg .= '+' . $credit_result['score'] . '积分';
          }
          ...
          $level_result = $this->api_broker_level_model->publish_cooperate_house(array('id' => $customer_ids_str), 3);
          if (is_full_array($level_result) && $level_result['status'] == 1) {
            $credit_msg .= ',+' . $level_result['score'] . '成长值';
          }
  ...
  private function _get_cond_str($form_param)
  {
    ...
    if ((isset($form_param["price_key"]) && $form_param["price_key"] > 0)) {
      $conf_customer = $this->buy_customer_model->get_base_conf();
      $price = $conf_customer['buy_price'][$form_param["price_key"]];
      if ($price) {
    ...
    if ((isset($form_param["area_key"]) && $form_param["area_key"] > 0)) {
      $conf_customer = $this->buy_customer_model->get_base_conf();
      $area = $conf_customer['buy_area'][$form_param["area_key"]];
      if ($area) {
  ...
  /**
   * 根据客源id判断当前经纪人是否有权限查看保密信息
   */
  public function purview_checkout_secret()
  {
  ...
  public function follow()
  {
    ...
    $this->buy_customer_model->set_id($customer_id);
    $owner_arr = $this->buy_customer_model->get_info_by_id();
    $view_follow_per = $this->broker_permission_model->check('19', $owner_arr);
    //求购客源跟进关联门店权限
    ...
    if ($follow_lists) {
      foreach ($follow_lists as $key => $val) {
        ...
        if ($val['follow_way'] == 4 || $val['follow_way'] == 5) {
          $this->load->model('sell_house_model');
          $this->sell_house_model->set_search_fields(array('block_name'));
          $this->sell_house_model->set_id($val['house_id']);
  ...
  public function addfollow()
  {
    ...
    if ($follow_text_num > 0 && (mb_strlen($follow_arr['text']) < $follow_text_num)) {
      ...
      if ($follow_arr['follow_way'] && $follow_arr['customer_id'] && $follow_arr['text']) {
        ...
        $result = $this->buy_customer_model->update_customerinfo_by_cond($customer_follow_date, $cond_where);
      }
      ...
      if ($follow_id > 0) {
        ...
        $this->operate_log_model->add_operate_log($add_log_param);
        $this->result('1', '添加跟进信息成功');
      } else {
        $this->result('0', '添加跟进信息失败');
      }
    }
  ...
  public function exportReport($page = 1)
  {
    ...
    for ($i = 2; $i <= count($customer_list) + 1; $i++) {
      ...
      $objPHPExcel->getActiveSheet()->setCellValue('A' . $i, '售');
  ...
  public function import()
  {
    if (!empty($_POST['sub'])) {
      ...
      if ($this->upload->do_upload('upfile')) {
        ...
        $result = $this->read_model->read('buy_customer_model', $broker_info, $data['upload_data'], 8);
  ...
  public function allocate_house($house_id)
  {
    ...
    if ($house_id) {
      ...
      $house_list = $this->buy_customer_model->get_customer($cond_where);
    }
  ...
  public function add_allocate_house()
  {
    ...
    $cond_where = "id IN(" . $house_id . ")";
    $return_id = $this->buy_customer_model->update_info_by_cond(array('broker_id' => $run_broker_id), $cond_where);
    if ($return_id) {
      echo '1';
    } else {
      echo '2';
  ...
  public function add_tasks()
  {
    ...
    $insert_date = strtotime(date('Y-m-d h:i:s'));//录入时间
    $start_date = strtotime($this->input->get('start_date', TRUE));//执行时间
    ...
    if (!empty($customer_arr) && is_array($customer_arr)) {
      foreach ($customer_arr as $val) {
        ...
        if (!empty($return_id)) {
          ...
          $allot_broker_info = $this->broker_model->get_by_id($allot_broker_id);//written by angel_in_us
          $run_broker_info = $this->broker_model->get_by_id($run_broker_id);//written by angel_in_us
          ...
          $this->message_base_model->add_message('7-40', $run_broker_info['id'], $run_broker_info['truename'], MLS_URL . '/my_task/', $params);//written by angel_in_us
        }
      }
    }
