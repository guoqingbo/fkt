...
class House_collect_model extends House_base_model
{
   ...
   * @return  int 取消条数
   */
  public function cancel_collection_by_id($c_id, $broker_id, $type = 'sell_house')
  {
    $up_num = 0;
   ...
   * @return  int 删除条数
   */
  public function del_collection_by_id($c_id, $broker_id, $type = 'sell_house')
  {
    $up_num = 0;
    $collect_tbl = $this->get_collect_tbl();
    $c_id = intval($c_id);
   ...
   * @return  array 收藏记录
   */
  public function get_collect_ids_by_bid($broker_id, $type = 'sell_house', $status = 1)
  {
    $broker_id = intval($broker_id);
    $type = strip_tags($type);
    ...
    if ($broker_id > 0 && $type != '') {
      ...
      $cond_where = "broker_id = '" . $broker_id . "' AND tbl = '" . $type . "'";
   ...
   * @return  int 收藏个数
   */
  public function get_collectionnum_by_cid($rows_id, $broker_id, $type = 'sell_house', $status = '')
  {
    ...
    $cond_where = "rows_id = '" . $rows_id . "' AND broker_id = '" . $broker_id . "' AND tbl = '" . $type . "'";
    $cond_where .= $status != 0 ? " AND status = '" . $status . "'" : "";
   ...
   * @return  int 收藏个数
   */
  public function get_collectionnum_by_bid($broker_id, $type = 'sell_house', $status = 1)
  {
    $broker_id = intval($broker_id);
    $type = strip_tags($type);
    $status = intval($status);
   ...
   * @return  int 数组数量
   */
  public function get_collection_num_by_cond($cond_where, $type = 'sell_house')
  {
    $collect_num = 0;
...
/* Location: ./applications/mls/models/house_collect_model.php */
