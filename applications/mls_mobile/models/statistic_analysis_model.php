...
class Statistic_analysis_model extends Statistic_base_model
{
  ...
  {
    $this->dbback_city->select("b.completed_time,a.type divide_type,a.price,c.name,a.broker_name,b.number,b.type contract_type,d.name register_name,e.truename register_truename,b.status,b.remarks");
    $this->dbback_city->from('contract_divide as a');
    $this->dbback_city->join('contract as b', "a.contract_id = b.id", 'left');
    $this->dbback_city->join('agency as c', 'a.agency_id = c.id', 'left');
    $this->dbback_city->join('agency as d', 'b.agency_id = d.id', 'left');
    $this->dbback_city->join('broker_info e', 'b.broker_id = e.broker_id', 'left');
    $this->dbback_city->where($where);
    $this->dbback_city->order_by($order_key, $order_by);
    $this->dbback_city->limit($limit, $offset);
   ...
   * @return array
   */
  public function get_broker_commission($files, $where, $offset, $limit, $order_key, $order_by)
  {
    //$files = "a.truename,b.name,sum(c.price) sell_price";
    $this->dbback_city->select($files);
    $this->dbback_city->from('broker_info as a');
    $this->dbback_city->join('agency as b', 'a.agency_id = b.id', 'left');
    $this->dbback_city->join('contract_divide as c', 'a.broker_id = c.broker_id', 'left');
    $this->dbback_city->join('contract as d', 'c.contract_id = d.id', 'left');
    if ($where) {
      $this->dbback_city->where($where);
    }
    ...
    $this->dbback_city->group_by('a.broker_id');
  ...
  public function get_broker_commission_count($where)
  {
    //$files = "a.truename,b.name,sum(c.price) sell_price";
    $this->dbback_city->select("count(*)");
    $this->dbback_city->from('broker_info as a');
   ...
   * @return mixed
   */
  public function get_broker_info($files, $where, $offset, $limit, $order_key, $order_by)
  {
    $this->dbback_city->select($files);
    $this->dbback_city->from('broker_info as a');
    $this->dbback_city->join('agency as b', 'a.agency_id = b.id', 'left');
