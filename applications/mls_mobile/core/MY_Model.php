...
class MY_Model extends CI_Model
{
   ...
   * @var Object
   */
  protected $dbback_tourism = NULL;

  //经纪人配置信息
  protected $broker_info = array();
  ...
  public function __construct()
  {
    ...
    $this->dbback = $this->mydbback->get_db_obj('dbback');

//        //加载db
//        $this->load->library('My_DB','','mydb_abroad');
//        $this->db_abroad = $this->mydb_abroad->get_db_obj('db_abroad');
//        //加载dbback
//        $this->load->library('My_DB','','mydbback_abroad');
//        $this->dbback_abroad = $this->mydbback_abroad->get_db_obj('dbback_abroad');
//
//        //加载db
//        $this->load->library('My_DB','','mydb_tourism');
//        $this->db_tourism = $this->mydb_tourism->get_db_obj('db_tourism');
//        //加载dbback
//        $this->load->library('My_DB','','mydbback_tourism');
//        $this->dbback_tourism = $this->mydbback_tourism->get_db_obj('dbback_tourism');
    ...
    //经纪人基本信息
    $this->broker_info = $this->config->item('broker_info');
  }
...
//        $abroad = $this->config->item('abroad');
//        $tourism = $this->config->item('tourism');
//        $this->set_city_db($login_city,$abroad,$tourism);
   ...
   */
  public function set_city_db($city_spell = '', $abroad = 0, $tourism = 0)
  {
  ...
  public function get_data($data = array(), $database = 'db')
  {
    ...
    if (isset($data['or_like']) AND !empty($data['or_like'])) {
      if (isset($data['or_like']['like_key']) AND !empty($data['or_like']['like_key']) AND is_array($data['or_like']['like_key'])) {//多个or条件
      ...
      } else if (isset($data['or_like']['like_value']) AND !empty($data['or_like']['like_value']) AND is_array($data['or_like']['like_value'])) {//多个or条件
        foreach ($data['or_like']['like_value'] as $v) {
          $this->db->or_like(array($data['or_like']['like_key'] => $v));
        }
        //特殊干预=>多个or_like加括号
        if (is_full_array($this->db->ar_like)) {
          $this->db->ar_like = array(sprintf("( %s )", implode(' ', $this->db->ar_like)));
    ...
    if (isset($data['or_like_2']) AND !empty($data['or_like_2'])) {
      if (isset($data['or_like_2']['like_key']) AND !empty($data['or_like_2']['like_key']) AND is_array($data['or_like_2']['like_key'])) {//多个or条件
        foreach ($data['or_like_2']['like_key'] as $v) {
          $this->db->or_like(array($v => $data['or_like_2']['like_value']));
        }
        //特殊干预=>多个or_like加括号
        if (is_full_array($this->db->ar_like)) {
          $this->db->ar_like = array(sprintf("( %s )", implode(' ', $this->db->ar_like)));
        }
      } else if (isset($data['or_like_2']['like_value']) AND !empty($data['or_like_2']['like_value']) AND is_array($data['or_like_2']['like_value'])) {//多个or条件
        foreach ($data['or_like_2']['like_value'] as $v) {
          $this->db->or_like(array($data['or_like_2']['like_key'] => $v));
        }
        //特殊干预=>多个or_like加括号
        if (is_full_array($this->db->ar_like)) {
          $this->db->ar_like = array(sprintf("( %s )", implode(' ', $this->db->ar_like)));
        ...
        $this->db->or_like($data['or_like_2']);
      }
    }

    if (isset($data['or_like_3']) AND !empty($data['or_like_3'])) {
      if (isset($data['or_like_3']['like_key']) AND !empty($data['or_like_3']['like_key']) AND is_array($data['or_like_3']['like_key'])) {//多个or条件
        foreach ($data['or_like_3']['like_key'] as $v) {
          $this->db->or_like(array($v => $data['or_like_3']['like_value']));
        }
        //特殊干预=>多个or_like加括号
        if (is_full_array($this->db->ar_like)) {
          $this->db->ar_like = array(sprintf("( %s )", implode(' ', $this->db->ar_like)));
        }
      } else if (isset($data['or_like_3']['like_value']) AND !empty($data['or_like_3']['like_value']) AND is_array($data['or_like_3']['like_value'])) {//多个or条件
        foreach ($data['or_like_3']['like_value'] as $v) {
          $this->db->or_like(array($data['or_like_3']['like_key'] => $v));
        }
        //特殊干预=>多个or_like加括号
        if (is_full_array($this->db->ar_like)) {
          $this->db->ar_like = array(sprintf("( %s )", implode(' ', $this->db->ar_like)));
        ...
        $this->db->or_like($data['or_like_3']);
      }
    }
  ...
  function dbselect($con)
  {
    switch ($con) {
      ...
      case 'dbback_city':
        ...
        break;
      case 'db_abroad':
        $this->db = $this->db_abroad;
        break;

      case 'dbback_abroad':
        $this->db = $this->dbback_abroad;
        break;

      case 'db_tourism':
        $this->db = $this->db_tourism;
        break;

      case 'dbback_tourism':
        $this->db = $this->dbback_tourism;
        break;
    }
  }
