...
class Autocollect_nj extends My_Controller
{
  ...
  public function sell_ganji_house_lists()
  {
    ...
    $this->na->post_job_notice("赶集—出售列表—" . $i);
    echo 'over';
  }
...
//					}
    ...
    $this->na->post_job_notice("赶集—出售详情—" . $i);
    echo 'over';
  }
  ...
  public function sell_ganji_house_picture()
  {
    ...
    if (!empty($res)) {
      ...
      foreach ($piclist as $key => $val) {
        $url = $val;
        $result = $this->autocollect_model->get_pic_url($url, 'nj');
        $pic[$key] = $result;
        $i++;
      }
      $picurl = implode("*", $pic);
      ...
      $this->na->post_job_notice("赶集—出售照片—" . $i);
      echo 'over';
    } else {
      echo "暂时无二手房房源照片";
    }
...
//        }
  ...
  public function rent_ganji_house_lists()
  {
    ...
    $this->na->post_job_notice("赶集—出租列表—" . $i);
    echo 'over';
  }
...
//					}
    ...
    $this->na->post_job_notice("赶集—出租详情—" . $i);
    echo 'over';
  }
  ...
  public function rent_ganji_house_picture()
  {
    ...
    if (!empty($res)) {
      ...
      $del = array('picid' => $res[0]['picid']);
      $result = $this->autocollect_model->del_picture_control($del, $database = 'db_city');
      $this->na->post_job_notice("赶集—出租照片—" . $i);
      echo 'over';
    } else {
      echo "暂时无租房房源照片";
    }
...
//        }
  ...
  public function sell_wuba_house_lists()
  {
    ...
    $this->na->post_job_notice("58—出售列表—" . $i);
    echo 'over';
  }
...
//                                  }
    ...
    $this->na->post_job_notice("58—出售临时详情—" . $i);
    echo 'over';
  }
  ...
  public function sell_wuba_telpic()
  {
    ...
    if (!empty($res)) {
      foreach ($res as $val) {
        $url = $val['telpic'];
        $picresult1 = $this->autocollect_model->get_telpic_url($url, 'nj', '&1');
        $picresult2 = $this->autocollect_model->get_telpic_url($url, 'nj', '&2');
        ...
        if (strlen($res1) > 10 && strlen($res2) > 10) {
          if ($res1 == $res2) {
            ...
            $info['picurl'] = '暂无资料';
            $rel = $this->autocollect_model->add_collect_house($info, $database = 'db_city');
            $this->na->post_job_notice("58—出售号码—成功（第一次：" . $res1 . " 第二次：" . $res2 . "）");
          } else {
            $this->na->post_job_notice("58—出售号码—失败（第一次：" . $res1 . " 第二次：" . $res2 . "）");
          }
          ...
          $info['picurl'] = '暂无资料';
          $rel = $this->autocollect_model->add_collect_house($info, $database = 'db_city');
          $this->na->post_job_notice("58—出售号码—成功（第一次成功：" . $res1 . " 第二次：" . $res2 . "）");
        } elseif (strlen($res2) > 10 && strlen($res1) < 11) {
          $i++;
          $info['telno1'] = $res2;
          $info['picurl'] = '暂无资料';
          $rel = $this->autocollect_model->add_collect_house($info, $database = 'db_city');
          $this->na->post_job_notice("58—出售号码—成功（第一次：" . $res1 . " 第二次成功：" . $res2 . "）");
        }
        $del = array('telpic' => $url);
      ...
      $this->na->post_job_notice("58—出售详情—" . $i);
      echo 'over';
    } else {
      echo "暂时无二手房房源电话图片";
    }
  ...
  public function sell_wuba_house_picture()
  {
    ...
    if (!empty($res)) {
      ...
      $del = array('picid' => $res[0]['picid']);
      $result = $this->autocollect_model->del_picture_control($del, $database = 'db_city');
      $this->na->post_job_notice("58—出售照片—" . $i);
      echo 'over';
    } else {
      echo "暂时无二手房房源照片";
    }
  ...
  public function rent_wuba_house_lists()
  {
    ...
    $this->na->post_job_notice("58—出租列表—" . $i);
    echo 'over';
  }
  ...
  public function rent_wuba_house()
  {
    ...
    foreach ($result as $key => $value) {
      ...
      if (!empty($ress)) {
        ...
        $data['oldurl'] = $val;
        //房源照片
        preg_match('/<div class="descriptionImg">.*<\/div>/siU', $con, $cons);
        if (!empty($cons)) {
          preg_match_all('/<img src="(.*)"\/>/siU', $cons[0], $photo);
          if (!empty($photo[1])) {
            $data['picurl'] = implode("*", $photo[1]);
          } else {
            $data['picurl'] = "暂无资料";
...
//					if(empty($result)){
            ...
            $info = array(
              ...
              'house_name' => urlencode($data['house_name']),
              'house_addr' => urlencode($data['house_addr']),
              'rent_type' => urlencode($data['rent_type']),
              'price' => urlencode($data['price']),
              'pricetype' => urlencode($data['pricetype']),
              'forward' => urlencode($data['forward']),
              'room' => urlencode($data['room']),
              'hall' => urlencode($data['hall']),
              'toilet' => urlencode($data['toilet']),
...
//					}
    ...
    $this->na->post_job_notice("58—出租临时详情—" . $i);
    echo 'over';
  }
  ...
  public function rent_wuba_telpic()
  {
    ...
    if (!empty($res)) {
      foreach ($res as $val) {
        ...
        if (strlen($res1) > 10 && strlen($res2) > 10) {
          if ($res1 == $res2) {
            ...
            $info['picurl'] = '暂无资料';
            $rel = $this->autocollect_model->add_rent_collect_house($info, $database = 'db_city');
            $this->na->post_job_notice("58—出租号码—成功（第一次：" . $res1 . " 第二次：" . $res2 . "）");
          } else {
            $this->na->post_job_notice("58—出租号码—失败（第一次：" . $res1 . " 第二次：" . $res2 . "）");
          }
        } elseif (strlen($res1) > 10 && strlen($res2) < 11) {
          $i++;
          $info['telno1'] = $res1;
          $info['picurl'] = '暂无资料';
          $rel = $this->autocollect_model->add_rent_collect_house($info, $database = 'db_city');
          $this->na->post_job_notice("58—出租号码—成功（第一次成功：" . $res1 . " 第二次：" . $res2 . "）");
        } elseif (strlen($res2) > 10 && strlen($res1) < 11) {
          $i++;
          $info['telno1'] = $res2;
          $info['picurl'] = '暂无资料';
          $rel = $this->autocollect_model->add_rent_collect_house($info, $database = 'db_city');
          $this->na->post_job_notice("58—出租号码—成功（第一次：" . $res1 . " 第二次成功：" . $res2 . "）");
        }
        $del = array('telpic' => $url);
        $result = $this->autocollect_model->del_wuba_telpic($del, $database = 'db_city');
        if (isset($rel) && $rel != '' && $picurl != '暂无资料') {
      ...
      $this->na->post_job_notice("58—出租详情—" . $i);
      echo 'over';
    } else {
      echo "暂时无租房房源电话图片";
    }
  ...
  public function rent_wuba_house_picture()
  {
    ...
    if (!empty($res)) {
      ...
      $del = array('picid' => $res[0]['picid']);
      $result = $this->autocollect_model->del_picture_control($del, $database = 'db_city');
      $this->na->post_job_notice("58—出租照片—" . $i);
      echo 'over';
    } else {
      echo "暂时无租房房源照片";
    }
