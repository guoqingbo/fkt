...
class Autocollect_model extends MY_Model
{
  ...
  public function __construct()
  {
    ...
    $this->collect_control = 'collect_control';
    $this->collect_url_hash = 'collect_url_hash';
    $this->picture_control = 'picture_control';
    $this->wuba_house_collect = 'wuba_house_collect';
    $this->stat_collect = 'stat_collect';
    $this->city = 'city'; //城市表
  }

  /********************跑数据 开始******************************/
  public function check_data($city, $database = 'db_city')
  {
    $where = array('house_id =' => 0);
    //出售
    $result_sell = $this->get_data(array('form_name' => $this->sell_house_collect, 'where' => $where, 'limit' => 100), $database);
    if (!empty($result_sell)) {
      foreach ($result_sell as $key => $val) {
        $res_sell = $this->add_sell_data($city, $val);
        if ($res_sell) {
          $wheres = array('id' => $val['id']);
          $data = array('house_id' => 1);
          $this->modify_data($wheres, $data, $database = 'db_city', $form_name = $this->sell_house_collect);
        }
      }
    }
    //出租
    $result_rent = $this->get_data(array('form_name' => $this->rent_house_collect, 'where' => $where, 'limit' => 100), $database);
    if (!empty($result_rent)) {
      foreach ($result_rent as $key => $val) {
        $res_rent = $this->add_rent_data($city, $val);
        if ($res_rent) {
          $wheres = array('id' => $val['id']);
          $data = array('house_id' => 1);
          $this->modify_data($wheres, $data, $database = 'db_city', $form_name = $this->rent_house_collect);
        }
      }
    }
  }

  public function add_sell_data($city, $data)
  {
    $city = $this->collect_city($city);
    $cityid = $city[0]['id'];
    $i = 0;
    //hash表
    $this->sell_house_collect_hash = 'sell_house_collect_hash';
    //详情hash
    if ($data['sell_type'] == 1 || $data['sell_type'] == 2) {
      //过滤重复房源hash 采集来源*小区名*电话*户型*面积*楼层
      $hash = md5($data['source_from'] . '*' . $data['house_name'] . '*' . $data['telno1'] . '*' . $data['room'] . '*' . $data['hall'] . '*' . $data['toilet'] . '*' . $data['buildarea'] . '*' . $data['floor'] . '*' . $data['totalfloor']);
    } else {
      //过滤重复房源hash 采集来源*小区名*电话*面积*区属*板块
      $hash = md5($data['source_from'] . '*' . $data['house_name'] . '*' . $data['telno1'] . '*' . $data['district'] . '*' . $data['block']);
    }
    $where = array('hash' => $hash);
    $res = $this->get_data(array('form_name' => $this->sell_house_collect_hash, 'where' => $where), 'db');
    if (!empty($res)) {
      $i = 1;
    } else {
      $i = 1;
      $data['id'] = '';
      $data['city'] = $cityid;
      $result = $this->add_data($data, 'db', $this->sell_house_collect);
      if ($result) {
        $hashdata = array(
          'house_id' => $result,
          'kind_type' => $data['sell_type'],
          'hash' => $hash,
          'source_from' => $data['source_from'] == 0 ? 'ganji' : '58',
          'createtime' => $data['createtime'],
          'city' => $cityid
        );
        $reshash = $this->add_data($hashdata, 'db', $this->sell_house_collect_hash);
        $this->add_url_hash(1, $data, 'sell_type');//添加列表hash
      }
    }
    return $i;
  }

  public function add_rent_data($city, $data)
  {
    $city = $this->collect_city($city);
    $cityid = $city[0]['id'];
    $i = 0;
    //hash表
    $this->rent_house_collect_hash = 'rent_house_collect_hash';
    //增加详情hash
    if ($data['source_from'] == 1 && $data['rent_type'] == 3) {
      //过滤重复房源hash 采集来源*电话*面积*区属*板块
      $hash = md5($data['source_from'] . '*' . $data['telno1'] . '*' . $data['district'] . '*' . $data['block']);
    } elseif ($data['rent_type'] == 1 || $data['rent_type'] == 2) {
      //过滤重复房源hash 采集来源*小区名*电话*户型*面积*楼层
      $hash = md5($data['source_from'] . '*' . $data['house_name'] . '*' . $data['telno1'] . '*' . $data['room'] . '*' . $data['hall'] . '*' . $data['toilet'] . '*' . $data['buildarea'] . '*' . $data['floor'] . '*' . $data['totalfloor']);
    } else {
      //过滤重复房源hash 采集来源*小区名*电话*面积*区属*板块
    ...
    $where = array('hash' => $hash);
    $res = $this->get_data(array('form_name' => $this->rent_house_collect_hash, 'where' => $where), 'db');
    if (!empty($res)) {
      $i = 1;
    } else {
      $i = 1;
      $data['id'] = '';
      $data['city'] = $cityid;
      $result = $this->add_data($data, 'db', $this->rent_house_collect);
      if ($result) {
        $hashdata = array(
          'house_id' => $result,
          'kind_type' => $data['rent_type'],
          'hash' => $hash,
          'source_from' => $data['source_from'] == 0 ? 'ganji' : '58',
          'createtime' => $data['createtime'],
          'city' => $cityid
        );
        $reshash = $this->add_data($hashdata, 'db', $this->rent_house_collect_hash);
        $this->add_url_hash(2, $data, 'rent_type');//添加列表hash
      }
    }
    return $i;
  }

  public function add_url_hash($type, $data, $kind_type)
  {
    $hashdata = array(
      'city' => $data['city'],
      'type' => $type,
      'kind_type' => $data[$kind_type],
      'url_hash' => md5($data['oldurl']),
      'source_from' => $data['source_from'] == 0 ? 'ganji' : '58',
      'createtime' => time()
    );
    //hash表
    $this->collect_url_hash = 'collect_url_hash';
    $this->add_data($hashdata, 'db', $this->collect_url_hash);
  }

  //获取城市
  public function collect_city($city)
  {
    $where = array('is_run' => 0, 'spell' => $city);
  ...
  /********************跑数据 结束******************************/
  /********************老版本列表去重 开始******************************/
  public function del_data($city, $database = 'db_city')
  {
    $i = 0;
    $result = $this->get_data(array('form_name' => $this->collect_control), $database);
    foreach ($result as $val) {
      $where = array('hash' => md5($val['url']));
      $res = $this->get_data(array('form_name' => $this->collect_url_hash, 'where' => $where, 'select' => array('hash')), $database);
      if (!empty($res)) {
        $i++;
        $del = array('id' => $val['id']);
        $this->del($del, $database, $this->collect_control);
      }
    }
    echo "删除" . $i . "条重复列表";
  }
  ...
  /**
   * 把采集到的房源列表插入到 collect_control
   * 2015.5.11 cc
   */
  public function add_collect_house_lists($data = array(), $database = 'db_city', $form_name = '')
  {
    $result = $this->add_data($data, $database, $this->collect_control);
    return $result;
  }

  /**
   * 查询collect_url_hash表中数据
   * 2015.5.15 cc
   */
  public function check_collect_url_hash($where = array(), $database = 'db_city')
  {
    $result = $this->get_data(array('form_name' => $this->collect_url_hash, 'where' => $where, 'select' => array('hash')), $database);
    return $result;
  }

  /**
   * 查询collect_control表中数据
   * 2015.5.11 cc
   */
  public function check_collect_house_lists($limit, $where = array(), $database = 'db_city')
  {
    $result = $this->get_data(array('form_name' => $this->collect_control, 'where' => $where, 'select' => array('url'), 'limit' => $limit), $database);
    return $result;
  }

  /**
   * 根据url type查询collect_control表中数据
   */
  public function check_collect_lists_byurl($lists, $database = 'db_city')
  {
    $where = array('hash' => md5($lists['url']));
    $data = $this->get_data(array('form_name' => $this->collect_url_hash, 'where' => $where, 'select' => array('hash')), $database);
    return $data;
  }

  /**
   * 删除collect_control表中数据
   * 2015.5.15 cc
   */
  public function del_collect_house_lists($data = array(), $database = 'db_city', $form_name = '')
  {
    $result = $this->del($data, $database, $this->collect_control);
    return $result;
  }

  /**
   * 把采集到的房源列表插入到 collect_url_hash
   * 2015.5.15 cc
   */
  public function add_collect_url_hash($data = array(), $database = 'db_city', $form_name = '')
  {
    $result = $this->add_data($data, $database, $this->collect_url_hash);
    return $result;
  }

  /**
   * 把采集到的房源列表插入到 picture_control
   * 2015.6.6 cc
   */
  public function add_picture_control($data = array(), $database = 'db_city', $form_name = '')
  {
    $result = $this->add_data($data, $database, $this->picture_control);
    return $result;
  }

  /**
   * 查询即将入库的房源所对应的电话是否在agent_blacklist表里已经存在
   */
  function check_agent_tel($where = array(), $database = 'db_city')
  {
  ...
  /**
   * 即将入库的房源所对应的经纪人加入agent_blacklist表里
   */
  function add_apent_broker($data, $database = 'db_city')
  {
    $result = $this->add_data($data, $database, agent_blacklist);
    return $result;
  }

  /**
   * 查询即将入库的二手房 房源是否重复采集 评判标准是：房主电话(telno1)+房源标题(house_title)
   */
  function check_house_only($where = array(), $database = 'db_city')
  {
    $result = $this->get_data(array('form_name' => $this->sell_house_collect, 'where' => $where, 'select' => array('telno1,house_title')), $database);
    return @$result[0]['house_title'];
  }


  /**
   * 查询即将入库的房源是否重复采集 评判标准是：房主电话(telno1)+房源标题(house_title)  check_rent_house_only
   */
  function check_rent_house_only($where = array(), $database = 'db_city')
  {
    $result = $this->get_data(array('form_name' => $this->rent_house_collect, 'where' => $where, 'select' => array('telno1,house_title')), $database);
    return @$result[0]['house_title'];
  }


  /**
   * 把采集到的房源插入到 sell_house_collect
   */
  public function add_collect_house($data = array(), $database = 'db_city', $form_name = '')
  {
    $result = $this->add_data($data, $database, $this->sell_house_collect);
    return $result;
  }

  /**
   * 把采集到的58房源房源详情存入临时表wuba_house_collect中
   * 2015.6.29 cc
   */
  public function add_wuba_house_collect($data = array(), $database = 'db_city', $form_name = '')
  {
    $result = $this->add_data($data, $database, $this->wuba_house_collect);
    return $result;
  }

  /**
   * 把采集到的房源插入到 rent_house_collect
   */
  public function add_rent_collect_house($data = array(), $database = 'db_city', $form_name = '')
  {
    $result = $this->add_data($data, $database, $this->rent_house_collect);
    return $result;
  }
  ...
  /**
   * 选取picture_control表中数据
   * 2015.6.6 cc
   */
  public function select_picture_control($where, $limit, $database = 'db_city')
  {
    $result = $this->get_data(array('form_name' => $this->picture_control, 'select' => array('picid', 'picurl', 'pictype'), 'where' => $where, 'limit' => $limit, 'order_by' => 'id'), $database);
    return $result;
  }

  /**
   * 选取wuba_house_collect表中电话图片地址
   * 2015.6.29 cc
   */
  public function select_wuba_telpic($where, $limit, $database = 'db_city')
  {
    $result = $this->get_data(array('form_name' => $this->wuba_house_collect, 'select' => array('id', 'text', 'telpic', 'type', 'telno'), 'where' => $where, 'limit' => $limit, 'order_by' => 'id'), $database);
    return $result;
  }
  ...
  {
    $filename = md5(basename($url)) . ".jpg";
    $localfile = "applications/mls_job/photo/" . $filename;
   ...
   * 2015.6.29 cc
   */
  public function get_telpic_url($url, $area, $type)
  {
    $filename = md5(basename($url . $type)) . ".jpg";
    $localfile = "applications/mls_job/telpic/" . $filename;
    $img = $this->get_html($url);
    if (!$img) {
      return false;
    }
...
//                        return false;
//                    }
//                    $_FILES['telpic']['name'] = $filename;
//                    $_FILES['telpic']['type'] = 'image/pjpeg';
//                    $_FILES['telpic']['tmp_name'] = $localfile;
//                    $_FILES['telpic']['error'] = 0;
//                    $_FILES['telpic']['size'] = filesize($localfile);
//                    $this->load->library('UploadFile', array('filename' => 'telpic'),'uf');
//                    $this->uf->setFileType("jpg|gif|png");
//                    $this->uf->setMaxSize('5120');
//                    $this->uf->setUploadType("ftp");
//                    $this->uf->setSaveDir("/".$area."mls/");
...
//                    @unlink($localfile);
  ...
  /**
   * 更新sell_house_collect中房源图片
   * 2015.6.6 cc
   */
  public function update_sell_house($arr, $data, $database = 'db_city', $form_name = '')
  {
    $result = $this->modify_data($arr, $data, $database, $this->sell_house_collect);
    return $result;
  }

  /**
   * 更新rent_house_collect中房源图片
   * 2015.6.6 cc
   */
  public function update_rent_house($arr, $data, $database = 'db_city', $form_name = '')
  {
    $result = $this->modify_data($arr, $data, $database, $this->rent_house_collect);
    return $result;
  }

  /**
   * 删除picture_control中数据
   * 2015.6.6 cc
   */
  public function del_picture_control($data = array(), $database = 'db_city', $form_name = '')
  {
    $result = $this->del($data, $database, $this->picture_control);
    return $result;
  }

  /**
   * 删除wuba_house_collect表中临时数据
   * 2015.6.30 cc
   */
  public function del_wuba_telpic($data = array(), $database = 'db_city', $form_name = '')
  {
    $result = $this->del($data, $database, $this->wuba_house_collect);
    return $result;
  }
   ...
   * 2015.6.9 cc
   */
  public function select_sell_ganji_house($like, $database = 'db_city')
  {
    $result = $this->get_data(array('form_name' => $this->sell_house_collect, 'select' => array('id', 'picurl'), 'like' => $like), $database);
    return $result;
  }
   ...
   * 2015.6.9 cc
   */
  public function select_rent_ganji_house($like, $database = 'db_city')
  {
    $result = $this->get_data(array('form_name' => $this->rent_house_collect, 'select' => array('id', 'picurl'), 'like' => $like), $database);
    return $result;
  }
   ...
   * 2015.6.13 cc
   */
  public function sell_365_house($data)
  {
    if (!empty($data) && isset($data)) {
      $where = array('hash' => md5($data['url']));
      $ress = $this->check_collect_url_hash($where, $database = 'db_city');
      if (empty($ress)) {
        switch ($data['fitment']) {
          case "豪华装":
            $data['serverco'] = 5;
        ...
        $data['picurl'] = !is_null($data['pics']) ? implode("*", $data['pics']) : "暂无资料";
        $data['createtime'] = time();
        $info = array(
          'house_title' => $data['address'],
          'district' => $data['district'],
          'block' => $data['streetname'],
          'house_name' => $data['blockshowname'],
          'house_addr' => $data['address'],
          'sell_type' => $data['infotype'], //1:住宅、2:别墅、3:写字楼、4:商铺、5:厂房仓库、车库
          'price' => $data['price'],
          'avgprice' => $data['averprice'],
        ...
        $hash['hash'] = md5($data['url']);
        $res = $this->add_collect_url_hash($hash, $database = 'db_city');
        return $this->add_collect_house($info, $database = 'db_city');
      }
    }
  }
   ...
   * 2015.6.13 cc
   */
  public function rent_365_house($data)
  {
    if (!empty($data) && isset($data)) {
      $where = array('hash' => md5($data['url']));
      $ress = $this->check_collect_url_hash($where, $database = 'db_city');
      if (empty($ress)) {
        ...
        $info = array(
          ...
          'house_name' => $data['blockshowname'],
          'house_addr' => $data['address'],
          'rent_type' => $data['infotype'], //1:住宅、2:别墅、3:写字楼、4:商铺、5:厂房仓库、车库
          'price' => $data['price'],
          'pricetype' => "面议",
          'forward' => $data['forward'],
          'room' => $data['room'],
        ...
        $hash['hash'] = md5($data['url']);
        $res = $this->add_collect_url_hash($hash, $database = 'db_city');
        return $this->add_rent_collect_house($info, $database = 'db_city');
      }
    }
  }
   ...
   * cc
   */
  public function select_sell_house_collect($where = array(), $database = 'db_city')
  {
    $result = $this->get_data(array('form_name' => $this->sell_house_collect, 'where' => $where, 'select' => array('createtime')), $database);
    return $result;
  }

  public function select_rent_house_collect($where = array(), $database = 'db_city')
  {
    $result = $this->get_data(array('form_name' => $this->rent_house_collect, 'where' => $where, 'select' => array('createtime')), $database);
    return $result;
  }
   ...
   * sell_house_collect
   */
  public function select_sell_info($where, $select, $database = 'db_city')
  {
    $result = $this->get_data(array('form_name' => $this->sell_house_collect, 'where' => $where, 'select' => $select, 'order_by' => 'id', 'limit' => 1), $database);
    return $result;
  }

  public function select_rent_info($where, $select, $database = 'db_city')
  {
    $result = $this->get_data(array('form_name' => $this->rent_house_collect, 'where' => $where, 'select' => $select, 'order_by' => 'id', 'limit' => 1), $database);
    return $result;
  }

  /**
   * 更新sell_house_collect中房源
   */
  public function update_sell_info_byid($id, $data, $database = 'db_city')
  {
    $this->db_city->where('id', $id);
    return $this->db_city->update('sell_house_collect', $data);
  }

  /**
   * 更新rent_house_collect中房源
   */
  public function update_rent_info_byid($id, $data, $database = 'db_city')
  {
    $this->db_city->where('id', $id);
    return $this->db_city->update('rent_house_collect', $data);
  }

  // 删除rent_house_collect表中数据
  public function del_collect_rent_house_info($data = array(), $database = 'db_city')
  {
    $result = $this->del($data, $database, $this->rent_house_collect);
    return $result;
  }

  // 删除sell_house_collect表中数据
  public function del_collect_sell_house_info($data = array(), $database = 'db_city')
  {
    $result = $this->del($data, $database, $this->sell_house_collect);
    return $result;
  }
   ...
   * cc
   */
  public function add_stat_collect($data = array(), $database = 'db_city', $form_name = '')
  {
    $result = $this->add_data($data, $database, $this->stat_collect);
    return $result;
  }

}
/* End of file autocollect_model.php */
/* Location: ./application/models/autocollect_model.php */
