...
class Sell_house_model extends House_base_model
{
  ...
  public function change_nature_by_agency_id2($agency_id = array(), $create_time)
  {
    $cond_where = '';
    if (!empty($agency_id)) {
      $agency_str = implode(',', $agency_id);
      $cond_where = "agency_id not in (" . $agency_str . ") and nature = 1 and createtime < '" . $create_time . "'";
    } else {
                    ...
                    if (is_full_array($house_info)) {
                        if ('0' == $house_info['is_district_public']) {
                            ...
                            if ($result) {
                                ...
                                $follow_arr['follow_way'] = 8;//跟进方式
                                $follow_arr['follow_type'] = 3;//跟进类型
                                $follow_arr['text'] = '区域公盘内委托人从 ' . $house_info['district_broker_name'] . '>> 无';//跟进内容
                                $follow_arr['date'] = date('Y-m-d H:i:s');//跟进时间
                                $follow_arr['type'] = 1;//类型
                                $this->add_follow($follow_arr);
                            }
