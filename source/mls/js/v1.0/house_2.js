...
/*
 ...
 $.ajax({
 url: MLS_URL + "/house_collections/change_contact_status/",
 type: "GET",
 dataType: "json",
...
function innerHeight() {//窗口改变大小的时候  计算高度
  if ($("#js_innerHouse").length > 0) {
    ...
    //$("#js_inner").css("height", _height - allH - 100 );
    $("#js_innerHouse").css("height", _height - allH);
...
//测试南京站
/*
 function show_tel_new(house_id,type,broker_id,oldurl,is_input)
 {
 house_id = parseInt(house_id);
 broker_id = parseInt(broker_id);
 $.ajax({
