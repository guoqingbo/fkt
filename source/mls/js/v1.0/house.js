        ...
        this.oncontextmenu = function (ev) {
          if (_url) {
            ...
            var _id = $(this).find(".checkbox").length ? $(this).find(".checkbox").val() : $(this).attr("info_id");
              ...
              //区域公盘按钮
              var is_send_district = $('#is_send_district' + _id).val();
              if (is_send_district == 1) {
                  $("#send_to_district").hide();
                  $("#cancel_to_district").show();
              } else {
                  $("#send_to_district").show();
                  $("#cancel_to_district").hide();
              }
...
/*
 ...
 {
 $.ajax({
 url: MLS_URL + "/house_collections_new/change_contact_status/",
 type: "GET",
 dataType: "json",
...
function modify_baomi_info(is_auth) {
  if (1 == is_auth) {
    $('#dong').hide();
    $('#dong_input').show();
      console.log($('#dong_input').nextAll(".iconfont"))
      $('#dong_input').nextAll(".iconfont").show();
    $('#unit').hide();
    $('#unit_input').show();
      $('#unit_input').nextAll(".iconfont").show();
    $('#door').hide();
    $('#door_input').show();
      $('#door_input').nextAll(".iconfont").show();
    $('#owner').hide();
    $('#owner_input').show();
    $('#telnos').hide();
    $('#telno1_input').show();
...
//房源提交保密信息
function submit_baomi_info(house_id, type, dong_type) {
  if (dong_type == 2) {
...
function modifyInfo(type) {
  ...
  $.ajax({
    ...
    success: function (data) {
      if ('is_seal' == data.result) {
        $("#dialog_do_permission_none_tip").html(data.seal_msg);
        openWin('js_pop_do_permission_none');
      } else {
        if ('yes_per_modify' == data.result) {
          location.href = "/" + type + "/modify/" + house_id;
        } else {
          $("#dialog_do_permission_none_tip").html(" 对不起，您没有访问权限!");
          openWin('js_pop_do_permission_none');
        }
      }
    }
