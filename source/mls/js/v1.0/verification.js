...
$(function () {
    ...
    $("#js_house_type_JDS").parent().click(function () {
        $(".js_s_h_info,.js_select_pirce,.js_item_hide,.js_s_info_WYF,.js_s_info_CQ,.js_s_info_XZ").hide();
        $(".js_s_BS_info,.js_show_pirce,.js_s_info_CQ,.js_s_info_XZ,.js_s_fenge_info").show();
    });
...
function sell_add(action, type, from) {
  var url = action == 'add' ? '/sell/add/' : '/sell/update';
  //运营管理后台，用户上传房源
  if (from == 'userhouse') {
      url = '/userhouse/index/add';
  }
  $.ajax({
    type: 'POST',
    url: url,
    ...
    success: function (data) {
      ...
      } else if ('/sell/add/' == url || '/userhouse/index/add' == url) {
        if (data['hosue_id'] > 0) {
          if ('/sell/add/' == url) {
            $.ajax({
              type: 'GET',
              ...
              if (data.is_reward === false) {
                ...
                if (data.coo_ziliao_check_1 === false) {
                  ...
                  if (data.coo_ziliao_check_2 === false) {
                    ...
                    $("#dialog_do_warnig_tip").html(data.error ? data.error : "系统错误");
                    openWin('js_pop_do_warning');
                  }
                }
              }
