...
//出售出租设置合作
function sharechange(flag, type, friend) { //flag检查房源是否存在标志 type出售出租类型 friend是否朋友圈
  var msg = "";
  var arr = new Array();
  var select_num = 0;
  var text = "";
  ...
  if (status_val_id_val != 1) {
    ...
    //判断是否发布到朋友圈 0 否 1是 2区域公盘
    if (friend == 2) {
        //发送区域公盘验证优化 1是否加入区域公盘.2.去重 3.房源区域筛选 4.检查是否上传房产证(暂时不做)
        //门店是否加入区域公盘
        ///var is_join_district = $('#is_join_district').val();
        //发送到区域公盘的房源是是否与门店所在区域公盘一致
        //var house_in_district_str = 'house_in_district' + $(".table").find("input:checked[name=items]").val();
        //var house_in_district = $("#" + house_in_district_str).val();
        //区域公盘内是否有该房源
        // var house_repeat_district_str = 'house_in_district' + $(".table").find("input:checked[name=items]").val();
        //  var house_repeat_district = $("#" + house_repeat_district_str).val();
        //是否上传了房产证
        // var is_have_certificate_str = 'is_have_certificate' + $(".table").find("input:checked[name=items]").val();
        // var is_have_certificate = $("#" + is_have_certificate_str).val();

        // if (friend == 2 && is_join_district==1 ) {
        //     $("#dialog_do_warnig_tip").html("该门店尚未加入区域公盘");
        //     openWin('js_pop_do_warning');
        //     return false;
        // }
        // if (friend == 2 && house_in_district==1 ) {
        //     $("#dialog_do_warnig_tip").html("该房源所在区域不在门店区域公盘");
        //     openWin('js_pop_do_warning');
        //     return false;
        // }
        // if (friend == 2 && house_repeat_district==1 ) {
        //     $("#dialog_do_warnig_tip").html("区域公盘已有该房源");
        //     openWin('js_pop_do_warning');
        //     return false;
        // }
        //      if (friend == 2 && is_have_certificate==1 ) {
        //          $("#upload_certificate_tip").html("该房源未上传房产证，请先上传房产证");
        //          openWin('upload_certificate');
        //          return false;
        //      }
        //发到区域公盘验证 1.去重 2.房源区域筛选 3.检查是否上传房产证
        var is_return = true;
        $.ajax({
            url: MLS_URL + "/" + type + "/check_district_house/",
            type: "GET",
            dataType: "json",
            async: false,
            data: {
            ...
            success: function (data) {
                if ('success' != data.msg) {
                    if (data.is_have_certificate == 0) {
                        // $("#upload_certificate_tip").html(data.msg);
                        // $("#thumbnails5").empty();
                        // openWin('upload_certificate');
                        $("#dialog_tip").html(data.msg);
                        openWin('pic_pop_tip');
                    } else {
...
function sharecancel(flag, type, house_id, friend) {
  ...
  if (flag == 0) {
    ...
    $.ajax({
      ...
      type: "GET",
      dataType: "json",
        data: {house_id_str: text, friend: friend},
      success: function (data) {
        if ('success' == data.msg) {
        ...
        } else if ('turn_other_house' == data.msg) {
            $("#dialog_do_warnig_tip").html("该房源在区域公盘的所属门店与您所在门店不一致，无法下架");
          openWin('js_pop_do_warning');
        } else {
...
//查看出租隐私内容
function show_rentbaomi_info(house_id, type, flag) {
  ...
  $.ajax({
    ...
    type: 'GET',
    dataType: 'JSON',
      data: {house_id: house_id, flag: flag},
    success: function (data) {
      //判断返回数据是否为空，不为空返回数据。
      if (data.id > 0) {
        $.ajax({
          ...
          success: function (msg) {
            if (msg.success) {
              ...
              //$('.link_btn_b').attr('style','display:none;');
                if (data.myself == 1 || data.join_district == 0) {
              $('#modify_baomi_button').show();
              $('#show_baomi_button').hide();
              //记录查看日志，并根据基本设置检测和跟进进程是否结束
                    var result = add_brower_log(type, house_id, '', data.myself);
                    if ('add_success' == result.msg && data.myself != 1) {
                    ...
                    $('#show_baomi_button').hide();
                    add_brower_log(type, house_id, 'distict', data.myself);
                }
            } else {
...
//查看出售的隐私内容
function show_baomi_info(house_id, type, district) {
  house_id = parseInt(house_id);
  if (type == 'sell') {
    var danwei = '万元';
  } else {
        ...
        $.ajax({
          ...
          success: function (msg) {
            if (msg.success) {
              ...
              //$('.link_btn_b').attr('style','display:none;');
                if (data.myself == 1 || data.join_district == 0) {//自己的房源或不是从区域公盘查看保密信息，显示修改按钮
                    $('#modify_baomi_button').show();
                    $('#show_baomi_button').hide();
                    var result = add_brower_log(type, house_id, '', data.myself);
...
//添加房源浏览记录
function add_brower_log(type, house_id, flag, myself) {
  var result = '';
  $.ajax({
    ...
    dataType: 'json',
    async: false,
      data: {house_id: house_id, flag: flag, myself: myself},
    success: function (data) {
      result = data;
    }
  });
...
//详情页添加跟进信息
function xq_openfollow(type, house_id, num, flag) {
  //经纪人所属用户组（是否认证）
  var group_id = $('#group_id').val();
  if ('1' == group_id) {
    $("#dialog_do_warnig_tip").html("您的帐号尚未认证");
  ...
  if (my_task) {
    if (type == 'sell' || type == 'rent') {
        var _url = '/' + type + '/house_follow/' + house_id + '/' + num + '/0/' + flag;
    } else {
        var _url = '/' + type + '/customer_follow/' + house_id + '/' + num + '/0/' + flag;
    }
  } else {
      var _url = '/' + type + '/house_follow/' + house_id + '/' + num + '/0/' + flag;
  }
...
//隐号拨打
function hidden_call_button(house_id, telno1, type) {
    var house_id = parseInt(house_id);

    $.ajax({
        url: MLS_URL + "/hidden_call/call_check/" + type,
        type: "POST",
        dataType: 'json',
        data: {house_id: house_id, telno1: telno1, type: type},
        success: function (data) {
            // 1.检查门店是否开通引号拨打
            // 2.检查门店开通号码是否全被占用
            // 3.检查号码是否停用
            ...
            if ('no_enable' == data.status) {
                $("#dialog_do_warnig_tip").html("您的门店尚未开通隐号拨打<br/>如需开通，请联系科地地产中心<br/>电话:0571-87675280");
                openWin('js_pop_do_warning');
                return false;
            } else if ('disable' == data.status) {
                $("#dialog_do_warnig_tip").html("账户余额：" + data.balance + "元<br/>您的账户已停用<br/>如需继续使用，请联系科地地产中心<br/>电话:0571-87675280");
                openWin('js_pop_do_warning');
                return false;
            } else if ('busy' == data.status) {
                $("#dialog_do_warnig_tip").html("您的门店现在无空闲号码，请稍后拨打，或联系地产中心开通更多号码<br/>电话:0571-87675280");
                openWin('js_pop_do_warning');
                return false;
            } else if ('balance_not_enough' == data.status) {
                $("#dialog_do_skip_tip").html("您的隐号拨打账户余额小于5元<br/>请联系科地地产中心及时充值，以免影响正常使用<br/>电话:0571-87675280");
                openWin('js_pop_do_skip');
                return false;
            } else if ('no_effective_phone' == data.status) {
                $("#dialog_do_warnig_tip").html("系统繁忙，暂无空闲虚拟号，请稍后拨打");
                openWin('js_pop_do_warning');
                return false;
            } else if ('fail_bind' == data.status) {
                $("#dialog_do_warnig_tip").html("虚拟号绑定失败，请联系地产中心<br/>电话:0571-87675280");
                openWin('js_pop_do_warning');
                return false;
            } else if ('normal' == data.status) {
                $("input[name='bindid']").val(data.bindid);
                $("#unbindnumber").html("确定要关闭该页面吗？</br>关闭后虚拟号(" + data.virtual_phone + ")将不可再用，若您正在通话中，则关闭不影响此次通话");
                $("#rent_monthly_tip").html(data.rent_monthly_tip);
                $("#dialog_do_per_talk_time").html("您的剩余通话时长还有" + data.per_talk_time + "分钟");
                $("#dialog_do_normal_tip").html(data.virtual_phone);
                $("#dialog_do_blind_time_tip").html(data.blind_time_str);
                $("#dialog_do_balance_tip").html("账户余额：" + data.balance + "元<br/>若有疑问请拨打电话:0571-87675280");
                openWin('js_pop_do_normal');
                return false;
            }
        }
    });
}
function unbindnumber() {
    var bindid = $("input[name='bindid']").val();
    console.log(bindid);
    $.ajax({
        url: MLS_URL + "/hidden_call/unbindnumber/",
        type: "POST",
        dataType: 'json',
        data: {bindid: bindid},
        success: function (data) {
            ...
            if ('success' == data.status) {
                ...
                closeWindowWin('js_pop_do_normal');
                return false;
            }
        }
    });
