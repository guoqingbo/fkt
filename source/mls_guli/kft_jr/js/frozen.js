...
function Scroll(el, options) {
	...
	if (this.options.role === 'slider') {
		...
		if (this.options.indicator) {
			...
			temp += '<li class="ui-slider-cont">/ '+cont+'</li></ul>';
			
			$(this.wrapper).after(temp);
...
Scroll.prototype = {
	...
	_end: function (e) {
		...
		//存在轮播图片文字的方法
		if( typeof showPicText === 'function' ){
			showPicText($(this.scroller.children[this.currentPage]).attr('id'));
		}
	},
	...
	_autoplay: function() {
		...
		//存在轮播图片文字的方法
		if( typeof showPicText === 'function' ){
			showPicText($(self.scroller.children[self.currentPage]).attr('id'));
		}
