...
function uploadSuccess(file, serverData) {
  ...
  if(resultData.success === 1) {
    $("#fileurl").val(resultData.result.imgUrl);
...
function uploadError(file, errorCode, message) {
  ...
  try {
    ...
    addImage("images/" + imageName, this.customSettings);
...
function addImage(src, customSettings) {
  var upload_infotype = customSettings.upload_infotype
  var upload_nail = customSettings.upload_nail
  if (src.indexOf('http://') >= 0) {
    var content = '<img class="img-upload" src="' + src + '">';
    $("#" + upload_nail).html(content);
  }
}
